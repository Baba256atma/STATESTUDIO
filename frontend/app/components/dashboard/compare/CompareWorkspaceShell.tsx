...
export type CompareWorkspaceShellProps = {
...
function moduleRow(module: CompareWorkspaceContextView["modules"][number]): React.ReactElement {
  return (
    <div
      key={module.id}
      data-nx-compare-module={module.id}
      style={{
        display: "flex",
        alignItems: "center",
...
export function CompareWorkspaceShell(props: CompareWorkspaceShellProps): React.ReactElement {
  const context = props.context;

  if (!context) {
    return (
      <div
        data-nx="compare-workspace-shell"
        data-nx-compare-state="empty"
        style={{
          flex: 1,
          minHeight: 0,
          padding: "16px 14px",
  ...
  return (
    <div
      data-nx="compare-workspace-shell"
      data-nx-compare-state="active"
      data-nx-compare-primary-id={context.primaryObjectId}
      data-nx-compare-status={context.comparisonStatus}
      style={{
        flex: 1,
        minHeight: 0,
        minWidth: 0,
    ...
    >
      ...
      <section style={{ flexShrink: 0, padding: "12px 14px", display: "flex", flexDirection: "column", gap: 10 }}>
        <div style={{ ...softCardStyle, padding: 12 }}>
          ...
          >
            {headerMetric("Primary", context.primaryObjectName)}
            {headerMetric("Target", context.targetObjectName)}
            <div style={{ gridColumn: "1 / -1" }}>
              {headerMetric("Status", context.comparisonStatusLabel)}
            </div>
          </div>
        </div>
...
export default CompareWorkspaceShell;
