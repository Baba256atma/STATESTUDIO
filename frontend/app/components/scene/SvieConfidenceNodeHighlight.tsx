...
export function SvieConfidenceNodeHighlight({
  visual,
  meshScale,
  selectedVisual,
  scannerHaloVisible,
}: Readonly<{
  visual: SvieConfidenceNodeVisualStyle;
  ...
  useFrame(({ clock }) => {
    ...
    const wobble =
      0.5 +
      0.5 *
        Math.sin(clock.elapsedTime * visual.pulseSpeed + Math.sin(clock.elapsedTime * 1.7) * 0.8);
  ...
  return (
    ...
    >
      <torusGeometry args={[0.82, 0.022, 10, 28]} />
