...
import type { SvieExecutiveFutureStoryConnectionVisualStyle } from "../../lib/scene/svie/svieExecutiveFutureStoryLayerContract.ts";
import {
...
const ExecutiveFutureStoryConnectionSegment = React.memo(function ExecutiveFutureStoryConnectionSegment({
  visual,
  objects,
  runtimeObjectPositionContext,
}: Readonly<{
  visual: SvieExecutiveFutureStoryConnectionVisualStyle;
  ...
  useFrame(({ clock }) => {
    ...
    material.opacity = visual.highlightOpacity + pulse * 0.08;
  });
...
export type SvieExecutiveFutureStoryOverlayProps = Readonly<{
  connectionVisuals: readonly SvieExecutiveFutureStoryConnectionVisualStyle[];
  objects: readonly unknown[];
  runtimeObjectPositionContext?: RuntimeObjectPositionContext;
}>;

export function SvieExecutiveFutureStoryOverlay({
  connectionVisuals,
  objects,
  runtimeObjectPositionContext,
}: SvieExecutiveFutureStoryOverlayProps): React.ReactElement | null {
  if (connectionVisuals.length === 0) return null;

  return (
    <>
      {connectionVisuals.map((visual) => (
        <ExecutiveFutureStoryConnectionSegment
          key={visual.id}
          visual={visual}
          objects={objects}
          runtimeObjectPositionContext={runtimeObjectPositionContext}
