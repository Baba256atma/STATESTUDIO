...
import type { ObjectKpiSummaryState } from "./kpiSummaryRuntime.ts";

export type KpiSummarySectionProps = Readonly<{
  summary: ObjectKpiSummaryState;
}>;

export function KpiSummarySection(props: KpiSummarySectionProps): React.ReactElement | null {
  const { summary } = props;
  if (!summary.visible) return null;

  return (
    <div
      data-nx="object-panel-kpi-summary"
      style={{ marginTop: 12, borderTop: `1px solid ${nx.borderSoft}`, paddingTop: 10 }}
    >
      <div style={objectPanelSectionLabel}>KPI Summary</div>
      {summary.emptyMessage ? (
        <div style={{ color: nx.muted, fontSize: 11, lineHeight: 1.45 }}>{summary.emptyMessage}</div>
      ) : (
        <div style={{ display: "grid", gap: 8 }}>
          {summary.items.map((item) => (
            <div key={item.kpiId} style={objectPanelInsightCardStyle}>
              ...
              {item.healthAvailable ? (
                ...
                >
                  ...
                  <div style={{ fontSize: 11, fontWeight: 800, color: nx.text }}>{item.progressLabel}</div>
                </div>
              ) : (
                <div style={{ marginTop: 4, color: nx.muted, fontSize: 11, lineHeight: 1.45 }}>
                  KPI health not available.
                </div>
              )}
            </div>
          ))}
