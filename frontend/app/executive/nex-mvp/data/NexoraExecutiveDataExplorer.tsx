...
function SourceIntelligenceView({
  source, intelligence, active, imports, onActivate, onViewOnStage,
  activeSourceContextId, onAdvisorContext, onRemoved,
}: Readonly<{
  ...
  return (
    <section data-testid="nexora-rdi3-source-intelligence" data-rdi3-source={source.sourceContextId} style={{ display: "grid", gap: "0.62rem", padding: "0.62rem", border: `1px solid ${cockpit.borderStrong}`, borderRadius: cockpit.radius.md, background: cockpit.panelSoft }}>
      <header style={{ display: "grid", gap: "0.18rem" }}>
        <span style={{ color: cockpit.lowMuted, fontSize: "0.55rem", letterSpacing: "0.12em", textTransform: "uppercase" }}>Executive Summary</span>
        <strong style={{ color: cockpit.text, fontSize: "0.78rem", overflowWrap: "anywhere" }}>{intelligence.sourceLabel}</strong>
        <span style={{ color: cockpit.muted, fontSize: "0.6rem" }}>Imported {intelligence.importedAt.slice(0, 10)} · {intelligence.recordCount} row · {intelligence.mappedObjectCount} objects</span>
      </header>
      ...
      <div>
        <span style={{ color: cockpit.lowMuted, fontSize: "0.55rem", letterSpacing: "0.1em", textTransform: "uppercase" }}>Affected</span>
        <div style={{ display: "flex", flexWrap: "wrap", gap: "0.3rem", marginTop: "0.35rem" }}>
          {intelligence.affectedObjects.map((entry) => <span key={entry.objectKey} style={{ border: `1px solid ${stateColor(entry.state)}`, borderRadius: cockpit.radius.pill, color: stateColor(entry.state), fontSize: "0.56rem", padding: "0.18rem 0.36rem" }}>{entry.objectLabel}</span>)}
        </div>
      </div>

      <div style={{ display: "grid", gap: "0.25rem" }}>
        <span style={{ color: cockpit.lowMuted, fontSize: "0.55rem", letterSpacing: "0.1em", textTransform: "uppercase" }}>Top Signals</span>
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: "0.35rem" }}>
        <button type="button" style={actionStyle(true)} onClick={() => {
          const target = intelligence.affectedObjects.find((entry) => entry.state === intelligence.overallState && entry.stageObjectId)?.stageObjectId;
          if (target) onViewOnStage(target);
        }}>View on Stage</button>
        <button type="button" style={actionStyle()} onClick={() => setComparing((value) => !value)}>Compare</button>
        <button type="button" style={actionStyle()} onClick={() => onAdvisorContext(createExecutiveSourceAdvisorContext(intelligence))}>Ask Nexora</button>
        {!active ? <button type="button" style={actionStyle()} onClick={onActivate}>Use as Active Source</button> : null}
        <button type="button" style={actionStyle()} onClick={() => setManagementOpen((value) => !value)}>More</button>
      </div>
      ...
      {managementOpen ? <section data-testid="nexora-rdi3-source-management" style={{ display: "grid", gap: "0.35rem", borderTop: `1px solid ${cockpit.border}`, paddingTop: "0.55rem" }}>
        {active ? <span style={{ color: cockpit.warning, fontSize: "0.61rem", lineHeight: 1.4 }}>This source currently supplies active business data. Replace it before removal, or cancel.</span> : <button type="button" style={actionStyle(false, true)} onClick={() => {
          const result = removeCsvRealDataImport({ workspaceId: source.workspaceId, sourceContextId: source.sourceContextId, activeSourceContextId });
          setManagementMessage(result.removed ? "Source removed." : "Source could not be removed.");
          if (result.removed) onRemoved();
        }}>Remove Source</button>}
        {managementMessage ? <span style={{ color: cockpit.muted, fontSize: "0.58rem" }}>{managementMessage}</span> : null}
      </section> : null}
    </section>
  );
}

function LiveSourceView({ connection, observations, activeSourceContextId, onActivate, onViewOnStage, onAdvisorContext }: Readonly<{
  ...
  return <section data-testid="nexora-rdi4-live-source" style={{ display: "grid", gap: "0.58rem", padding: "0.62rem", border: `1px solid ${cockpit.borderStrong}`, borderRadius: cockpit.radius.md, background: cockpit.panelSoft }}>
    ...
    <span style={{ color: cockpit.muted, fontSize: "0.6rem" }}>Last successful observation: {connection.lastSuccessfulObservationAt ? new Date(connection.lastSuccessfulObservationAt).toLocaleString() : "Not yet observed"}</span>
    {intelligence ? <><strong style={{ color: stateColor(intelligence.overallState), fontSize: "0.68rem" }}>{stateLabel(intelligence.overallState)} · {intelligence.mappedObjectCount} objects</strong><p style={{ color: cockpit.textSoft, fontSize: "0.65rem", lineHeight: 1.45, margin: 0 }}>{intelligence.interpretation}</p>{intelligence.topSignals.slice(0, 3).map((signal) => <span key={signal} style={{ color: cockpit.muted, fontSize: "0.59rem" }}>• {signal}</span>)}</> : <span style={{ color: cockpit.textSoft, fontSize: "0.64rem" }}>Connected is not Active. Refresh to create the first canonical observation.</span>}
    ...
    <section data-testid="nexora-pm2-automatic-monitoring" data-monitoring-enabled={monitoringPolicy?.enabled === true} data-monitoring-status={monitoringRuntime?.status ?? "idle"} style={{ display: "grid", gap: "0.38rem", borderTop: `1px solid ${cockpit.border}`, paddingTop: "0.5rem" }}>
...
export function NexoraExecutiveDataExplorer({ workspaceId, activeImport, activeLiveObservation, onImportCommitted, onLiveObservationActivated, onViewOnStage, onAdvisorContext }: NexoraExecutiveDataExplorerProps) {
  ...
  const selectedIdIsLive = liveConnections.some((entry) => entry.connectionId === selectedSourceId);
  const effectiveSelectedSourceId = imports.some((entry) => entry.sourceContextId === selectedSourceId)
    ? selectedSourceId
    : selectedIdIsLive
      ? null
    : activeImport?.workspaceId === workspaceId
      ? activeImport.sourceContextId
      : null;
  const selected = imports.find((entry) => entry.sourceContextId === effectiveSelectedSourceId) ?? null;
  ...
  return <div data-testid="nexora-rdi2-data-explorer" data-rdi3="executive-source-intelligence" data-rdi4="live-connector-foundation" data-rdi2-canonical-route="/executive" style={{ display: "flex", flexDirection: "column", gap: "0.75rem", padding: "0.85rem", minWidth: 0 }}>
    <header><p style={{ margin: 0, color: cockpit.lowMuted, fontSize: cockpit.type.status.size, letterSpacing: cockpit.type.status.tracking, textTransform: "uppercase" }}>Data Sources</p><p style={{ margin: "0.3rem 0 0", color: cockpit.textSoft, fontSize: "0.72rem" }}>Workspace · {workspaceId} · Imported & Connected</p></header>
    {!adding ? <button type="button" data-testid="nexora-rdi2-add-data" style={actionStyle(true)} onClick={() => setAdding(true)}>+ Add Data</button> : null}
    ...
    {adding && addKind === "csv" ? <CsvRealDataImportFlow workspaceId={workspaceId} onClose={() => { setAdding(false); setAddKind(null); }} onCompleted={(committed) => { onImportCommitted(committed); setSelectedSourceId(committed.sourceContextId); }} /> : null}
    ...
    <section aria-label="Existing Sources" style={{ display: "grid", gap: "0.4rem" }}>
      <p style={{ margin: 0, color: cockpit.lowMuted, fontSize: "0.58rem", letterSpacing: "0.1em", textTransform: "uppercase" }}>Existing Sources</p>
      {imports.length === 0 ? <p style={{ margin: 0, color: cockpit.muted, fontSize: "0.7rem", lineHeight: 1.45 }}>No validated real-data sources in this workspace.</p> : imports.map((entry) => {
        const active = activeImport?.sourceContextId === entry.sourceContextId;
        const selectedCard = effectiveSelectedSourceId === entry.sourceContextId;
        const intelligence = intelligenceBySource.get(entry.sourceContextId)!;
        return <button key={entry.sourceContextId} type="button" data-testid={`nexora-rdi3-source-${entry.importId}`} data-source-active={active} data-source-selected={selectedCard} onClick={() => setSelectedSourceId(entry.sourceContextId)} style={{ border: `1px solid ${selectedCard ? cockpit.borderStrong : cockpit.border}`, borderRadius: cockpit.radius.sm, background: selectedCard ? cockpit.accentSoft : cockpit.panelSoft, color: cockpit.text, cursor: "pointer", fontFamily: "inherit", padding: "0.5rem 0.58rem", textAlign: "left", minWidth: 0 }}>
          <span style={{ display: "flex", justifyContent: "space-between", gap: "0.4rem", alignItems: "baseline" }}><strong style={{ fontSize: "0.7rem", overflowWrap: "anywhere" }}>{entry.prepared.fileName}</strong><strong style={{ color: stateColor(intelligence.overallState), fontSize: "0.56rem", letterSpacing: "0.07em" }}>{stateLabel(intelligence.overallState)}</strong></span>
          <span style={{ display: "block", marginTop: "0.2rem", color: cockpit.muted, fontSize: "0.6rem" }}>{intelligence.recordCount} row · {intelligence.mappedObjectCount} objects{active ? " · Active" : ""}</span>
          <span style={{ display: "block", marginTop: "0.16rem", color: cockpit.lowMuted, fontSize: "0.56rem" }}>{intelligence.attentionCount} attention · {intelligence.criticalCount} critical</span>
        </button>;
      })}
    </section>

    <section aria-label="Connected Sources" style={{ display: "grid", gap: "0.4rem" }}>
      <p style={{ margin: 0, color: cockpit.lowMuted, fontSize: "0.58rem", letterSpacing: "0.1em", textTransform: "uppercase" }}>Connected</p>
      {liveConnections.length === 0 ? <p style={{ margin: 0, color: cockpit.muted, fontSize: "0.66rem" }}>No live sources connected in this workspace.</p> : liveConnections.map((connection) => { const history = listNexoraLiveObservations(workspaceId, connection.connectionId); const latest = history[history.length - 1] ?? null; const intelligence = latest ? projectExecutiveSourceIntelligence(liveProjection(latest)) : null; const active = activeLiveObservation?.sourceContextId === `live:${connection.connectionId}`; return <button key={connection.connectionId} type="button" data-testid={`nexora-rdi4-connection-${connection.providerId}`} data-source-active={active} onClick={() => setSelectedSourceId(connection.connectionId)} style={{ border: `1px solid ${selectedSourceId === connection.connectionId ? cockpit.borderStrong : cockpit.border}`, borderRadius: cockpit.radius.sm, background: selectedSourceId === connection.connectionId ? cockpit.accentSoft : cockpit.panelSoft, color: cockpit.text, cursor: "pointer", fontFamily: "inherit", padding: "0.5rem 0.58rem", textAlign: "left" }}><span style={{ display: "flex", justifyContent: "space-between", gap: "0.4rem" }}><strong style={{ fontSize: "0.69rem" }}>{connection.displayName}</strong><strong style={{ color: connection.status === "degraded" ? cockpit.warning : connection.status === "connected" ? cockpit.success : cockpit.muted, fontSize: "0.54rem", textTransform: "uppercase" }}>{connection.status}</strong></span><span style={{ display: "block", color: cockpit.muted, fontSize: "0.58rem", marginTop: "0.18rem" }}>{history.length} observation{history.length === 1 ? "" : "s"}{active ? " · Active" : ""}{intelligence ? ` · ${stateLabel(intelligence.overallState)}` : ""}</span></button>; })}
    </section>

    {selected && selectedIntelligence ? <SourceIntelligenceView key={selected.sourceContextId} source={selected} intelligence={selectedIntelligence} active={activeImport?.sourceContextId === selected.sourceContextId} activeSourceContextId={activeImport?.sourceContextId ?? null} imports={imports} onActivate={() => onImportCommitted(selected)} onViewOnStage={onViewOnStage} onAdvisorContext={onAdvisorContext} onRemoved={() => setSelectedSourceId(null)} /> : null}
    {selectedLive ? <LiveSourceView key={selectedLive.connectionId} connection={selectedLive} observations={listNexoraLiveObservations(workspaceId, selectedLive.connectionId)} activeSourceContextId={activeLiveObservation?.sourceContextId ?? activeImport?.sourceContextId ?? null} onActivate={onLiveObservationActivated} onViewOnStage={onViewOnStage} onAdvisorContext={onAdvisorContext} /> : null}
  </div>;
}
