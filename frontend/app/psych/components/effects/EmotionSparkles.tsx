...
function EmotionSparkles({ intensity = 0.5 }: EmotionSparklesProps): React.JSX.Element {
  ...
  const [layer] = useState(createEmotionSparkleGeometry);
  ...
  useFrame((_, delta) => {
    ...
    currentIntensityRef.current = THREE.MathUtils.lerp(currentIntensityRef.current, targetIntensity, safeDelta * 4);
...
export default React.memo(EmotionSparkles);
