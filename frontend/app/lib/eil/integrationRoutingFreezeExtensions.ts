/**
 * EIL-3:8 — Integration Routing Freeze Extensions.
 *
 * Descriptive extension-policy metadata for the frozen EIL-3 routing baseline.
...
import type {
  RoutingFreezeExtension,
  RoutingFreezeExtensionKey,
} from "./integrationRoutingFreezeTypes.ts";

const extension = (
  key: RoutingFreezeExtensionKey,
  canonicalName: string,
  description: string,
  ordinal: number,
): RoutingFreezeExtension =>
  Object.freeze({
    extensionId: `EIL-3:8/Extension/${key}` as const,
    canonicalKey: key,
    canonicalName,
    description,
 ...
 */
export const IntegrationRoutingFreezeExtensions: readonly RoutingFreezeExtension[] =
  Object.freeze([
    extension(
    ...
    extension(
      ...
      "Canonical EIL-3 identities must remain preserved across future phases.",
      5,
    ),
    extension(
      "NamespacePreservation",
      "Namespace preservation",
      "nexora.eil.integration-routing.* namespaces must remain preserved.",
      6,
    ),
    extension(
