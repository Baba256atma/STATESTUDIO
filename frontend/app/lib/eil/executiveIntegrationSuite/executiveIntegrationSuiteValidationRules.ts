/**
 * EIL-8:4 — Executive Integration Suite Validation Rules.
 *
...
import type { SuiteValidationCategoryKey } from "./executiveIntegrationSuiteValidationCategories.ts";
...
export interface ExecutiveIntegrationSuiteValidationRule {
  readonly ruleId: `EIL-8:4/Rule/${string}`;
  readonly canonicalKey: string;
  readonly canonicalName: string;
  readonly description: string;
  readonly categoryKey: SuiteValidationCategoryKey;
...
const rule = (
  ...
  description: string,
  categoryKey: SuiteValidationCategoryKey,
  order: number,
): ExecutiveIntegrationSuiteValidationRule =>
  Object.freeze({
    ruleId: `EIL-8:4/Rule/${key}` as const,
    canonicalKey: key,
    canonicalName,
    description,
    categoryKey,
    ...
    sourceReference: `${ExecutiveIntegrationSuiteModelIdentity.canonicalId}/validation/rules/${key}`,
 ...
 */
export const ExecutiveIntegrationSuiteValidationRules: readonly ExecutiveIntegrationSuiteValidationRule[] =
  Object.freeze([
    rule(
      "CanonicalPhaseId",
      "Canonical Phase ID",
      "Model phase ID must equal EIL-8:3.",
      "Identity",
      1,
    ),
    rule(
      "CanonicalModelId",
      "Canonical Model ID",
      "Model canonical ID must equal EIL-8:3/ExecutiveIntegrationSuiteModel.",
      "Identity",
      2,
    ),
    rule(
      ...
      "Model name must equal Executive Integration Suite Model.",
      "Identity",
      3,
    ),
    rule(
    ...
    rule(
      ...
      "Model namespace must equal nexora.eil.executive-integration-suite.model.",
      "Namespace",
      5,
    ),
    rule(
      ...
      "Validation namespace must equal nexora.eil.executive-integration-suite.validation.",
      "Namespace",
      6,
    ),
    rule(
    ...
    rule(
      ...
      "Model must depend exclusively on EIL-8:2 Registry.",
      "Dependency",
      9,
    ),
    rule(
      ...
      "Validation must depend exclusively on EIL-8:3 Model.",
      "Dependency",
      10,
    ),
    rule(
    ...
    rule(
      "ModuleCountSeven",
      "Module Count Seven",
      "Model must expose exactly seven module models.",
      "Inventory",
      13,
    ),
    rule(
    ...
    rule(
      "DerivedTotalForty",
      "Derived Total Forty",
      "Model inventory total must equal the sum of collection lengths (40).",
      "Inventory",
      16,
    ),
    rule(
