...
import type {
  StageBoundaryDecision,
  StageManifest,
  StageManifestValidationIssue,
  StageManifestValidationResult,
} from "./stageArchitectureTypes.ts";

function issue(code: string, message: string): StageManifestValidationIssue {
...
export function validateStageManifest(manifest: StageManifest): StageManifestValidationResult {
  const issues: StageManifestValidationIssue[] = [];
...
export function evaluateStageFileBoundary(input: {
  ...
  const allowed = input.allowedFiles.some(
    (entry) => normalizePath(entry).toLowerCase() === filePath.toLowerCase()
  );
  if (!allowed) {
    return Object.freeze({
      allowed: false,
...
export function assertStageManifestValid(manifest: StageManifest): StageManifestValidationResult {
