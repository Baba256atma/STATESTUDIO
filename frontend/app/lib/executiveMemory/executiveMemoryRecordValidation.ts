...
import {
  ...
  EXECUTIVE_MEMORY_RECORD_LIMITS,
  EXECUTIVE_MEMORY_RECORD_MANDATORY_FIELDS,
  EXECUTIVE_MEMORY_RECORD_SCHEMA_VERSION,
  EXECUTIVE_MEMORY_REFERENCE_TYPE_KEYS,
...
import type { ExecutiveMemoryValidationIssue, ExecutiveMemoryValidationResult } from "./executiveMemoryTypes.ts";
...
export function hasDuplicateReferenceIds(references: readonly ExecutiveMemoryReference[]): boolean {
  const ids = references.map((entry) => entry.referenceId);
  return new Set(ids).size !== ids.length;
}

export function isExecutiveMemoryReferenceType(value: string): boolean {
  return (EXECUTIVE_MEMORY_REFERENCE_TYPE_KEYS as readonly string[]).includes(value);
}

export function validateExecutiveMemoryMetadataContract(
...
export function validateExecutiveMemoryRecordShape(
  ...
  if (
    record.schemaVersion !== EXECUTIVE_MEMORY_RECORD_SCHEMA_VERSION &&
    typeof record.schemaVersion !== "string"
  ) {
    issues.push(issue("invalid_schema_version", "Schema version is invalid.", "schemaVersion"));
  }
  if (
    record.contractVersion !== EXECUTIVE_MEMORY_RECORD_CONTRACT_VERSION &&
...
export function validateExecutiveMemoryRecordBackwardCompatibility(
  record: ExecutiveMemoryRecord
): ExecutiveMemoryValidationResult {
  const issues: ExecutiveMemoryValidationIssue[] = [];
