...
import {
  EXECUTIVE_MEMORY_RECORD_IDENTITY,
  EXECUTIVE_MEMORY_RECORD_SELF_MANIFEST,
  ExecutiveMemoryRecordContracts,
} from "./executiveMemoryRecordContracts.ts";
import {
...
test("createExecutiveMemoryRecord builder composes valid records", () => {
  ...
  const record = createExecutiveMemoryRecord({
    ...
    body: { narrative: "Evidence narrative.", keyPoints: Object.freeze([]), readOnly: true as const },
    metadata,
...
test("rejects duplicate reference ids", () => {
  ...
  assert.equal(
    hasDuplicateReferenceIds(Object.freeze([duplicateReference, duplicateReference])),
    true
  );
  const metadata = createExecutiveMemoryMetadata({
...
test("validates schema compatibility flags on parsed payload", () => {
  const record = buildExecutiveMemoryRecordExample(FIXED_TIME);
  const payload = JSON.parse(serializeExecutiveMemoryRecord(record).json) as Record<string, unknown>;
