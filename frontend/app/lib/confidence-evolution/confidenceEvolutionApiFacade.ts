...
import {
  ...
  type ConfidenceEvolutionApiCalibrationSurface,
  type ConfidenceEvolutionApiCertificationResult,
  type ConfidenceEvolutionApiCertificationSurface,
  type ConfidenceEvolutionApiEvidenceReasonSurface,
  type ConfidenceEvolutionApiQuerySurface,
  type ConfidenceEvolutionApiRecordsSurface,
  type ConfidenceEvolutionApiResponse,
  type ConfidenceEvolutionApiTrendSurface,
...
export function createConfidenceEvolutionApiFacade(
  ...
  const records: ConfidenceEvolutionApiRecordsSurface = {
    createRecord: (input) => {
  ...
  const query: ConfidenceEvolutionApiQuerySurface = {
  ...
  const trend: ConfidenceEvolutionApiTrendSurface = {
  ...
  const evidenceReason: ConfidenceEvolutionApiEvidenceReasonSurface = {
  ...
  const calibration: ConfidenceEvolutionApiCalibrationSurface = {
    ...
    evaluateCalibration: (confidenceScore, reason, source, evidenceReferences) =>
      apiSuccess(
        ...
        evaluateConfidenceCalibration(confidenceScore, reason, source, evidenceReferences)
      ),
    calculateCalibrationScore: (confidenceScore, evidenceSupportScore) =>
      apiSuccess(
        "Calibration score calculated.",
        calculateConfidenceCalibrationScore(confidenceScore, evidenceSupportScore)
      ),
    calculateAccuracyScore: (confidenceScore, evidenceSupportScore) =>
      apiSuccess(
        "Accuracy score calculated.",
        calculateConfidenceAccuracyScore(confidenceScore, evidenceSupportScore)
      ),
  };

  const certification: ConfidenceEvolutionApiCertificationSurface = {
    runCertification: () => {
  ...
  return Object.freeze({
    ...
    evidenceReason: Object.freeze(evidenceReason),
