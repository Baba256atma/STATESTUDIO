/** ASSISTANT-6:7 — Exactly 18 immutable certification criteria. */
import { AssistantObjectContextManagementPlatform } from "./assistantObjectContextManagementPlatform.ts";
import type { AssistantObjectContextManagementCertificationCriterionMetadata } from "./assistantObjectContextManagementCertification.types.ts";
...
export const AssistantObjectContextManagementCertificationCriteria:
readonly AssistantObjectContextManagementCertificationCriterionMetadata[] =
  Object.freeze(
    criteria.map((criterion, index) => Object.freeze({
      criterionId:
        `ASSISTANT-6:7/Criterion/${String(index + 1).padStart(2, "0")}`,
      name: criterion.name,
      description: criterion.description,
      category: criterion.category,
      status: "Certified",
      validationTarget:
        AssistantObjectContextManagementPlatform.identity.id,
...
export const AssistantObjectContextManagementCertificationCategories =
  Object.freeze([...new Set(criteria.map(({ category }) => category))]);

export const AssistantObjectContextManagementCertificationMetadataFieldCount =
