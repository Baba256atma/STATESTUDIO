/** ASSISTANT-6:4 — Exactly 16 immutable non-executable gates. */
import { AssistantObjectContextManagementValidationRules } from "./assistantObjectContextManagementValidation.rules.ts";
import type { AssistantObjectContextManagementValidationGateMetadata } from "./assistantObjectContextManagementValidation.types.ts";

const gates = Object.freeze([
  {
    name: "Identity Gate",
    description:
      "Gate confirming Object & Context Management identity validation.",
  },
  {
    name: "Namespace Gate",
...
export const AssistantObjectContextManagementValidationGates:
readonly AssistantObjectContextManagementValidationGateMetadata[] =
  Object.freeze(
    gates.map((gate, index) => Object.freeze({
      gateId: `ASSISTANT-6:4/Gate/${String(index + 1).padStart(2, "0")}`,
      name: gate.name,
