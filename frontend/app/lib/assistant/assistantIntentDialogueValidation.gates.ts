/** ASSISTANT-3:4 — Exactly 16 immutable non-executable gates. */
import { AssistantIntentDialogueValidationRules } from "./assistantIntentDialogueValidation.rules.ts";
import type { AssistantIntentDialogueValidationGateMetadata } from "./assistantIntentDialogueValidation.types.ts";

const gates = Object.freeze([
  {
    name: "Identity Gate",
    description: "Gate confirming Intent & Dialogue identity validation.",
  },
  {
    name: "Namespace Gate",
...
export const AssistantIntentDialogueValidationGates:
readonly AssistantIntentDialogueValidationGateMetadata[] = Object.freeze(
  gates.map((gate, index) => Object.freeze({
    gateId: `ASSISTANT-3:4/Gate/${String(index + 1).padStart(2, "0")}`,
    name: gate.name,
