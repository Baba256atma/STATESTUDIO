/** ASSISTANT-2:4 — Exactly 16 immutable non-executable gates. */
import { AssistantExecutiveMemoryValidationRules } from "./assistantExecutiveMemoryValidation.rules.ts";
import type { AssistantExecutiveMemoryValidationGateMetadata } from "./assistantExecutiveMemoryValidation.types.ts";

const gates = Object.freeze([
  {
    name: "Identity Gate",
    description: "Gate confirming Executive Memory identity validation.",
  },
  {
    name: "Registry Gate",
...
export const AssistantExecutiveMemoryValidationGates:
readonly AssistantExecutiveMemoryValidationGateMetadata[] = Object.freeze(
  gates.map((gate, index) => Object.freeze({
    gateId: `ASSISTANT-2:4/Gate/${String(index + 1).padStart(2, "0")}`,
    name: gate.name,
