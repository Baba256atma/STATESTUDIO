import assert from "node:assert/strict";
import { readdirSync, readFileSync } from "node:fs";
import test from "node:test";
import { AssistantObjectContextManagementRegistry } from "./assistantObjectContextManagementRegistry.ts";

const files = [
  "assistantObjectContextManagementRegistry.collections.ts",
  "assistantObjectContextManagementRegistry.constants.ts",
  "assistantObjectContextManagementRegistry.entries.ts",
  "assistantObjectContextManagementRegistry.identity.ts",
  "assistantObjectContextManagementRegistry.metadata.ts",
  "assistantObjectContextManagementRegistry.test.ts",
  "assistantObjectContextManagementRegistry.ts",
  "assistantObjectContextManagementRegistry.types.ts",
];

test("ASSISTANT-6:2 consists of exactly eight Registry artifacts", () => {
  assert.deepEqual(
    readdirSync(new URL(".", import.meta.url))
      .filter((file) => files.includes(file)).sort(),
    files,
  );
});

test("ASSISTANT-6:2 publishes canonical Registry identity", () => {
  const registry = AssistantObjectContextManagementRegistry;
  assert.equal(
    registry.identity.id,
    "ASSISTANT-6:2/ObjectContextManagementRegistry",
  );
  assert.equal(
    registry.identity.namespace,
    "nexora.assistant.object-context-management.registry",
  );
  assert.equal(registry.identity.version, "1.0.0");
  assert.equal(registry.status, "Registry");
  assert.equal(registry.readiness, "ReadyForModel");
  assert.equal(
    registry.identity.sourceFoundation,
    "ASSISTANT-6:1/ObjectContextManagementFoundation",
  );
});

test("ASSISTANT-6:2 publishes exactly ten complete collections", () => {
  const registry = AssistantObjectContextManagementRegistry;
  assert.equal(Object.keys(registry.collections).length, 10);
  assert.equal(registry.constants.collectionCount, 10);
  assert.equal(registry.statistics.collectionCount, 10);
  assert.equal(registry.statistics.categoryCount, 10);
  assert.equal(registry.statistics.metadataCount, 8);
  assert.equal(registry.entries.length, registry.constants.entryCount);
  assert.equal(registry.statistics.entryCount, registry.entries.length);
  assert.equal(registry.entries.length, 86);
  assert.deepEqual(Object.keys(registry.collections), [
    "executiveObjectTypes",
    "contextTypes",
    "contextStates",
    "contextSessionTypes",
    "objectPriorities",
    "objectOutcomes",
    "contextStrategies",
    "contextPolicies",
    "contextLifecycleStates",
    "contextTags",
  ]);
});

test("ASSISTANT-6:2 entries are unique, stable, ordered, and immutable", () => {
  const registry = AssistantObjectContextManagementRegistry;
  const entries = registry.entries;
  assert.equal(
    new Set(entries.map(({ identifier }) => identifier)).size,
    entries.length,
  );
  assert.equal(entries.every(Object.isFrozen), true);
  assert.equal(entries.every(({ version }) => version === "1.0.0"), true);
  assert.equal(Object.isFrozen(registry.collections), true);
  for (const collection of Object.values(registry.collections)) {
    assert.deepEqual(
      collection.map(({ order }) => order),
      collection.map((_, index) => index + 1),
    );
  }
});

test("ASSISTANT-6:2 consumes Foundation only and has no prohibited behavior", () => {
  const registry = AssistantObjectContextManagementRegistry;
  const source = readFileSync(
    new URL(
      "./assistantObjectContextManagementRegistry.ts",
      import.meta.url,
    ),
    "utf8",
  );
  const importSources = [
    ...source.matchAll(/from ["'](\.\/[^"']+)["']/g),
  ].map((match) => match[1]);
  assert.deepEqual(importSources, [
    "./assistantObjectContextManagementRegistry.collections.ts",
    "./assistantObjectContextManagementRegistry.constants.ts",
    "./assistantObjectContextManagementRegistry.entries.ts",
    "./assistantObjectContextManagementRegistry.identity.ts",
    "./assistantObjectContextManagementRegistry.metadata.ts",
  ]);
  assert.equal(
    source.includes("assistantObjectContextManagementModel"),
    false,
  );
  assert.equal(
    source.includes("assistantWorkspaceOrchestration"),
    false,
  );
  assert.equal(source.includes("assistantExecutiveGuidance"), false);
  assert.equal(source.includes("assistantIntentDialogue"), false);
  assert.equal(source.includes("assistantExecutiveMemory"), false);
  assert.equal(source.includes("assistantConversation"), false);
  assert.deepEqual(registry.upstreamDependencies, [
    "ASSISTANT-6:1 Object & Context Management Foundation",
  ]);
  assert.equal(
    registry.metadata.sourceFoundation.identity.id,
    "ASSISTANT-6:1/ObjectContextManagementFoundation",
  );
  assert.equal(registry.runtime, false);
  assert.equal(registry.executionLogic, false);
  assert.equal(registry.objectCreation, false);
  assert.equal(registry.objectPersistence, false);
  assert.equal(registry.contextPersistence, false);
  assert.equal(registry.contextSynchronization, false);
  assert.equal(registry.persistence, false);
  assert.equal(registry.networking, false);
});
