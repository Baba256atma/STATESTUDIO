...
const files = [
  "assistantObjectContextManagementValidation.constants.ts",
  "assistantObjectContextManagementValidation.gates.ts",
  "assistantObjectContextManagementValidation.identity.ts",
  "assistantObjectContextManagementValidation.results.ts",
  "assistantObjectContextManagementValidation.rules.ts",
  "assistantObjectContextManagementValidation.test.ts",
  "assistantObjectContextManagementValidation.ts",
  "assistantObjectContextManagementValidation.types.ts",
];

test("ASSISTANT-6:4 consists of exactly eight Validation artifacts", () => {
  assert.deepEqual(
...
test("ASSISTANT-6:4 publishes canonical Validation identity", () => {
  ...
  assert.equal(
    ...
    "nexora.assistant.object-context-management.validation",
  );
...
test("ASSISTANT-6:4 publishes exactly 40 rules, 16 gates, and 8 categories", () => {
  const validation = AssistantObjectContextManagementValidation;
...
test("ASSISTANT-6:4 identities and metadata are immutable", () => {
  const validation = AssistantObjectContextManagementValidation;
  assert.equal(
...
test("ASSISTANT-6:4 consumes Model only and has no executable validation", () => {
  const validation = AssistantObjectContextManagementValidation;
  ...
  assert.equal(validation.objectCreation, false);
  assert.equal(validation.objectPersistence, false);
  assert.equal(validation.contextPersistence, false);
  assert.equal(validation.contextSynchronization, false);
