import assert from "node:assert/strict";
import { readdirSync, readFileSync } from "node:fs";
import test from "node:test";
import { AssistantExecutiveGuidanceRegistry } from "./assistantExecutiveGuidanceRegistry.ts";

const files = [
  "assistantExecutiveGuidanceRegistry.collections.ts",
  "assistantExecutiveGuidanceRegistry.constants.ts",
  "assistantExecutiveGuidanceRegistry.entries.ts",
  "assistantExecutiveGuidanceRegistry.identity.ts",
  "assistantExecutiveGuidanceRegistry.metadata.ts",
  "assistantExecutiveGuidanceRegistry.test.ts",
  "assistantExecutiveGuidanceRegistry.ts",
  "assistantExecutiveGuidanceRegistry.types.ts",
];

test("ASSISTANT-4:2 consists of exactly eight Registry artifacts", () => {
  assert.deepEqual(
    readdirSync(new URL(".", import.meta.url))
      .filter((file) => files.includes(file)).sort(),
    files,
  );
});

test("ASSISTANT-4:2 publishes canonical Registry identity", () => {
  const registry = AssistantExecutiveGuidanceRegistry;
  assert.equal(
    registry.identity.id,
    "ASSISTANT-4:2/ExecutiveGuidanceRegistry",
  );
  assert.equal(
    registry.identity.namespace,
    "nexora.assistant.executive-guidance.registry",
  );
  assert.equal(registry.identity.version, "1.0.0");
  assert.equal(registry.status, "Registry");
  assert.equal(registry.readiness, "ReadyForModel");
  assert.equal(
    registry.identity.sourceFoundation,
    "ASSISTANT-4:1/ExecutiveGuidanceFoundation",
  );
});

test("ASSISTANT-4:2 publishes exactly ten complete collections", () => {
  const registry = AssistantExecutiveGuidanceRegistry;
  assert.equal(Object.keys(registry.collections).length, 10);
  assert.equal(registry.constants.collectionCount, 10);
  assert.equal(registry.statistics.collectionCount, 10);
  assert.equal(registry.statistics.categoryCount, 10);
  assert.equal(registry.statistics.metadataCount, 8);
  assert.equal(registry.entries.length, registry.constants.entryCount);
  assert.equal(registry.statistics.entryCount, registry.entries.length);
  assert.equal(registry.entries.length, 86);
  assert.deepEqual(Object.keys(registry.collections), [
    "guidanceTypes",
    "guidanceObjectives",
    "guidanceStates",
    "guidanceSessionTypes",
    "guidancePriorities",
    "guidanceOutcomes",
    "guidanceStrategies",
    "guidancePolicies",
    "guidanceLifecycleStates",
    "guidanceTags",
  ]);
});

test("ASSISTANT-4:2 entries are unique, stable, ordered, and immutable", () => {
  const registry = AssistantExecutiveGuidanceRegistry;
  const entries = registry.entries;
  assert.equal(
    new Set(entries.map(({ identifier }) => identifier)).size,
    entries.length,
  );
  assert.equal(entries.every(Object.isFrozen), true);
  assert.equal(entries.every(({ version }) => version === "1.0.0"), true);
  assert.equal(Object.isFrozen(registry.collections), true);
  for (const collection of Object.values(registry.collections)) {
    assert.deepEqual(
      collection.map(({ order }) => order),
      collection.map((_, index) => index + 1),
    );
  }
});

test("ASSISTANT-4:2 consumes Foundation only and has no prohibited behavior", () => {
  const registry = AssistantExecutiveGuidanceRegistry;
  const source = readFileSync(
    new URL("./assistantExecutiveGuidanceRegistry.ts", import.meta.url),
    "utf8",
  );
  const importSources = [
    ...source.matchAll(/from ["'](\.\/[^"']+)["']/g),
  ].map((match) => match[1]);
  assert.deepEqual(importSources, [
    "./assistantExecutiveGuidanceRegistry.collections.ts",
    "./assistantExecutiveGuidanceRegistry.constants.ts",
    "./assistantExecutiveGuidanceRegistry.entries.ts",
    "./assistantExecutiveGuidanceRegistry.identity.ts",
    "./assistantExecutiveGuidanceRegistry.metadata.ts",
  ]);
  assert.equal(source.includes("assistantExecutiveGuidanceModel"), false);
  assert.equal(source.includes("assistantIntentDialogue"), false);
  assert.equal(source.includes("assistantExecutiveMemory"), false);
  assert.equal(source.includes("assistantConversation"), false);
  assert.deepEqual(registry.upstreamDependencies, [
    "ASSISTANT-4:1 Executive Guidance Foundation",
  ]);
  assert.equal(
    registry.metadata.sourceFoundation.identity.id,
    "ASSISTANT-4:1/ExecutiveGuidanceFoundation",
  );
  assert.equal(registry.runtime, false);
  assert.equal(registry.executionLogic, false);
  assert.equal(registry.recommendationGeneration, false);
  assert.equal(registry.coachingGeneration, false);
  assert.equal(registry.decisionGeneration, false);
  assert.equal(registry.persistence, false);
  assert.equal(registry.networking, false);
});
