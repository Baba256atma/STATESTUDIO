/** ASSISTANT-1:7 — Exactly 18 immutable certification criteria. */
import { AssistantConversationPlatform } from "./assistantConversationPlatform.ts";
import type { AssistantConversationCertificationCriterionMetadata } from "./assistantConversationCertification.types.ts";
...
export const AssistantConversationCertificationCriteria:
readonly AssistantConversationCertificationCriterionMetadata[] = Object.freeze(
  criteria.map((criterion, index) => Object.freeze({
    criterionId:
      `ASSISTANT-1:7/Criterion/${String(index + 1).padStart(2, "0")}`,
    name: criterion.name,
    description: criterion.description,
    category: criterion.category,
    status: "Certified",
    target: AssistantConversationPlatform.identity.id,
