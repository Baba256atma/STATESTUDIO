/** ASSISTANT-3:8 — Canonical lock, architectural locks, and frozen registry. */
import { AssistantIntentDialogueCertification } from "./assistantIntentDialogueCertification.ts";
import type {
  AssistantIntentDialogueFreezeArchitecturalLockMetadata,
  AssistantIntentDialogueFreezeRegistryEntryMetadata,
} from "./assistantIntentDialogueFreeze.types.ts";
...
export const AssistantIntentDialogueFreezeCanonicalLock = Object.freeze({
  lockIdentifier: "ASSISTANT-3-INTENT-DIALOGUE-UNDERSTANDING-LOCKED",
  name: "Assistant Intent & Dialogue Understanding Architecture Lock",
  description:
    "Permanent immutable lock for the certified Intent & Dialogue Understanding architecture.",
...
const architecturalLocks = Object.freeze([
  {
    ...
    description:
      "Lock canonical Intent & Dialogue Understanding identities permanently.",
    protectedTarget: certification.identity.id,
  },
  {
    name: "Namespace Locked",
    description:
      "Lock Intent & Dialogue Understanding namespace declarations permanently.",
    protectedTarget: certification.identity.namespace,
  },
  {
    name: "Version Locked",
    description:
      "Lock Intent & Dialogue Understanding version metadata permanently.",
  ...
  },
  {
    ...
    description:
      "Lock Intent & Dialogue Understanding architectural boundaries permanently.",
    protectedTarget: "ASSISTANT-3-INTENT-DIALOGUE-UNDERSTANDING-LOCKED",
  },
] as const);

export const AssistantIntentDialogueFreezeArchitecturalLocks:
readonly AssistantIntentDialogueFreezeArchitecturalLockMetadata[] =
  Object.freeze(
    architecturalLocks.map((lock, index) => Object.freeze({
      lockId: `ASSISTANT-3:8/Lock/${String(index + 1).padStart(2, "0")}`,
      name: lock.name,
...
export const AssistantIntentDialogueFreezeArchitectureRegistry:
readonly AssistantIntentDialogueFreezeRegistryEntryMetadata[] = Object.freeze(
  registryEntries.map((entry, index) => Object.freeze({
    entryId: `ASSISTANT-3:8/Registry/${String(index + 1).padStart(2, "0")}`,
    name: entry.name,
