/** ASSISTANT-5:7 — Exactly 18 immutable certification criteria. */
import { AssistantWorkspaceOrchestrationPlatform } from "./assistantWorkspaceOrchestrationPlatform.ts";
import type { AssistantWorkspaceOrchestrationCertificationCriterionMetadata } from "./assistantWorkspaceOrchestrationCertification.types.ts";
...
export const AssistantWorkspaceOrchestrationCertificationCriteria:
readonly AssistantWorkspaceOrchestrationCertificationCriterionMetadata[] =
  Object.freeze(
    criteria.map((criterion, index) => Object.freeze({
      criterionId:
        `ASSISTANT-5:7/Criterion/${String(index + 1).padStart(2, "0")}`,
      name: criterion.name,
      description: criterion.description,
      category: criterion.category,
      status: "Certified",
      validationTarget: AssistantWorkspaceOrchestrationPlatform.identity.id,
...
export const AssistantWorkspaceOrchestrationCertificationCategories =
  Object.freeze([...new Set(criteria.map(({ category }) => category))]);

export const AssistantWorkspaceOrchestrationCertificationMetadataFieldCount =
