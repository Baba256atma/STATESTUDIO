/** ASSISTANT-4:7 — Exactly 18 immutable certification criteria. */
import { AssistantExecutiveGuidancePlatform } from "./assistantExecutiveGuidancePlatform.ts";
import type { AssistantExecutiveGuidanceCertificationCriterionMetadata } from "./assistantExecutiveGuidanceCertification.types.ts";
...
export const AssistantExecutiveGuidanceCertificationCriteria:
readonly AssistantExecutiveGuidanceCertificationCriterionMetadata[] =
  Object.freeze(
    criteria.map((criterion, index) => Object.freeze({
      criterionId:
        `ASSISTANT-4:7/Criterion/${String(index + 1).padStart(2, "0")}`,
      name: criterion.name,
      description: criterion.description,
      category: criterion.category,
      status: "Certified",
      validationTarget: AssistantExecutiveGuidancePlatform.identity.id,
...
export const AssistantExecutiveGuidanceCertificationCategories = Object.freeze(
  [...new Set(criteria.map(({ category }) => category))],
);

export const AssistantExecutiveGuidanceCertificationMetadataFieldCount =
