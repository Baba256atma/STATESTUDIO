/** ASSISTANT-7:4 — Exactly 16 immutable non-executable gates. */
import { AssistantExecutiveActionPlanningValidationRules } from "./assistantExecutiveActionPlanningValidation.rules.ts";
import type { AssistantExecutiveActionPlanningValidationGateMetadata } from "./assistantExecutiveActionPlanningValidation.types.ts";

const gates = Object.freeze([
  {
    name: "Identity Gate",
    description:
      "Gate confirming Executive Action Planning identity validation.",
  },
  {
    name: "Namespace Gate",
  ...
  {
    name: "Canonical Inventory Gate",
    description:
      "Gate confirming Canonical Inventory Rule compliance validation.",
  },
  {
...
export const AssistantExecutiveActionPlanningValidationGates:
readonly AssistantExecutiveActionPlanningValidationGateMetadata[] =
  Object.freeze(
    gates.map((gate, index) => Object.freeze({
      gateId: `ASSISTANT-7:4/Gate/${String(index + 1).padStart(2, "0")}`,
      name: gate.name,
