/** ASSISTANT-5:4 — Exactly 16 immutable non-executable gates. */
import { AssistantWorkspaceOrchestrationValidationRules } from "./assistantWorkspaceOrchestrationValidation.rules.ts";
import type { AssistantWorkspaceOrchestrationValidationGateMetadata } from "./assistantWorkspaceOrchestrationValidation.types.ts";

const gates = Object.freeze([
  {
    name: "Identity Gate",
    description: "Gate confirming Workspace Orchestration identity validation.",
  },
  {
    name: "Namespace Gate",
...
export const AssistantWorkspaceOrchestrationValidationGates:
readonly AssistantWorkspaceOrchestrationValidationGateMetadata[] =
  Object.freeze(
    gates.map((gate, index) => Object.freeze({
      gateId: `ASSISTANT-5:4/Gate/${String(index + 1).padStart(2, "0")}`,
      name: gate.name,
