...
const files = [
  "assistantIntentDialogueManifest.constants.ts",
  "assistantIntentDialogueManifest.identity.ts",
  "assistantIntentDialogueManifest.inventory.ts",
  "assistantIntentDialogueManifest.metadata.ts",
  "assistantIntentDialogueManifest.summary.ts",
  "assistantIntentDialogueManifest.test.ts",
  "assistantIntentDialogueManifest.ts",
  "assistantIntentDialogueManifest.types.ts",
];

test("ASSISTANT-3:5 consists of exactly eight Manifest artifacts", () => {
  assert.deepEqual(
...
test("ASSISTANT-3:5 publishes canonical Manifest identity", () => {
  const manifest = AssistantIntentDialogueManifest;
  assert.equal(
  ...
  assert.equal(
    ...
    "nexora.assistant.intent-dialogue.manifest",
  );
...
test("ASSISTANT-3:5 preserves Validation-derived inventories only", () => {
  const manifest = AssistantIntentDialogueManifest;
  assert.equal(
...
test("ASSISTANT-3:5 inventory, compatibility, and readiness metadata are complete", () => {
  const manifest = AssistantIntentDialogueManifest;
...
test("ASSISTANT-3:5 consumes Validation only and has no prohibited behavior", () => {
  const manifest = AssistantIntentDialogueManifest;
  const source = readFileSync(
    new URL("./assistantIntentDialogueManifest.ts", import.meta.url),
    "utf8",
  );
  const importSources = [
  ...
  assert.equal(manifest.intentClassification, false);
  assert.equal(manifest.nlp, false);
  assert.equal(manifest.naturalLanguageParsing, false);
  assert.equal(manifest.dialogueExecution, false);
