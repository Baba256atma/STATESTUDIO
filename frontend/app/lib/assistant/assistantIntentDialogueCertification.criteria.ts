/** ASSISTANT-3:7 — Exactly 18 immutable certification criteria. */
import { AssistantIntentDialoguePlatform } from "./assistantIntentDialoguePlatform.ts";
import type { AssistantIntentDialogueCertificationCriterionMetadata } from "./assistantIntentDialogueCertification.types.ts";
...
export const AssistantIntentDialogueCertificationCriteria:
readonly AssistantIntentDialogueCertificationCriterionMetadata[] =
  Object.freeze(
    criteria.map((criterion, index) => Object.freeze({
      criterionId:
        `ASSISTANT-3:7/Criterion/${String(index + 1).padStart(2, "0")}`,
      name: criterion.name,
      description: criterion.description,
      category: criterion.category,
      status: "Certified",
      validationTarget: AssistantIntentDialoguePlatform.identity.id,
...
export const AssistantIntentDialogueCertificationCategories = Object.freeze(
  [...new Set(criteria.map(({ category }) => category))],
);

export const AssistantIntentDialogueCertificationMetadataFieldCount =
