...
import PublicIndex, * as PublicExports from "./userJourneyExperiencePublicIndex.ts";

test("NEX-4:9 consists of exactly two Public Index files", () => {
  const files = readdirSync(new URL(".", import.meta.url))
    .filter((name) => name.startsWith("userJourneyExperiencePublicIndex"));
  assert.deepEqual(files.sort(), [
    "userJourneyExperiencePublicIndex.test.ts",
    "userJourneyExperiencePublicIndex.ts",
  ]);
});

test("NEX-4:9 exposes exactly twelve exports and nine namespace sections", () => {
...
test("NEX-4:9 publishes a unique deterministic immutable API Registry", () => {
...
test("NEX-4:9 imports only Freeze and is ReadyForConsumer", () => {
  const source = readFileSync(
    new URL("./userJourneyExperiencePublicIndex.ts", import.meta.url),
    "utf8",
  );
  ...
  assert.equal(PublicIndex.identity.namespace, "nexora.nex.user-journey-experience.public-index");
  ...
  assert.equal(PublicIndex.uiImplementation, false);
  assert.equal(PublicIndex.uxBehavior, false);
  assert.equal(PublicIndex.navigationLogic, false);
  assert.equal(PublicIndex.workflows, false);
