...
import { integrateConsensusIntelligenceWithCognition } from "./integrateConsensusIntelligenceWithCognition";
...
function resetConsensusIntelligenceTestStacks(): void {
...
function minimalCognition(org = "consensus-intelligence-org"): AdaptiveGovernanceIntelligenceSnapshot {
  return {
...
function seedConsensusIntelligenceRuntime(
  ...
  for (let i = 0; i < 4; i += 1) {
    evaluateInstitutionalMemoryAccumulation({
      organizationId,
      cognitionSnapshot: { ...cognition, signature: `ci-seed-${i}` },
...
function consensusEvalInput(
  org: string,
  cognition: AdaptiveGovernanceIntelligenceSnapshot,
  now: number,
  overrides?: Partial<Parameters<typeof evaluateExecutiveConsensusIntelligence>[0]>
) {
  return {
    organizationId: org,
    cognitionSnapshot: cognition,
...
describe("executive consensus intelligence D9:7:1", () => {
  beforeEach(() => {
    resetConsensusIntelligenceTestStacks();
  });

  it("generates consensus snapshots when unified reflective runtime is present", () => {
    const org = "ci-verify-org";
    const cognition = minimalCognition(org);
    seedConsensusIntelligenceRuntime(org, cognition);

    const result = evaluateExecutiveConsensusIntelligence(
      consensusEvalInput(org, cognition, 110_000)
    );

    expect(result.evaluated).toBe(true);
    expect(getConsensusIntelligenceStore(org).getState().reasoningPerspectives.length).toBeGreaterThan(
      0
    );
    expect(getConsensusIntelligenceStore(org).getState().consensusRecords.length).toBeGreaterThan(0);
    expect(result.snapshot?.observationCount).toBeGreaterThan(0);
  });

  it("detects governance-resilience consensus reinforcement", () => {
    const org = "ci-governance-resilience-org";
    const cognition = minimalCognition(org);
    seedConsensusIntelligenceRuntime(org, cognition);
    ...
    const result = evaluateExecutiveConsensusIntelligence(
      consensusEvalInput(org, cognition, 111_000, {
        unifiedSelfReflectiveSnapshot: reflective
          ? {
              ...
              summary: {
                ...
                governanceAlignment: "self_regulated",
    ...
    expect(result.evaluated).toBe(true);
    expect(
      getConsensusIntelligenceStore(org).getState().consensusRecords.some((r) =>
        r.consensusSignals.includes("governance_resilience_alignment")
      )
    ).toBe(true);
  });

  it("detects operational speed vs stabilization divergence", () => {
    const org = "ci-divergence-org";
    const cognition = minimalCognition(org);
    seedConsensusIntelligenceRuntime(org, cognition);
    ...
    const result = evaluateExecutiveConsensusIntelligence(
      consensusEvalInput(org, cognition, 112_000, {
    ...
    expect(result.evaluated).toBe(true);
    expect(
      getConsensusIntelligenceStore(org).getState().consensusRecords.some(
        (r) =>
          r.divergentPerspectives.includes("operational_speed") ||
          r.consensusSignals.includes("orchestration_disagreement")
      )
    ).toBe(true);
  });

  it("skips when unified reflective depth is insufficient", () => {
    const org = "ci-isolated-org";
    const cognition = minimalCognition(org);
    seedConsensusIntelligenceRuntime(org, cognition);
    ...
    const result = evaluateExecutiveConsensusIntelligence(
      consensusEvalInput(org, cognition, 113_000, {
        unifiedSelfReflectiveSnapshot: reflective
          ? { ...reflective, activeSubsystems: Object.freeze([]) }
          : null,
      })
    );

    expect(result.skipped).toBe(true);
    expect(result.reason).toBe("insufficient_reflective_depth");
  });

  it("dedupes duplicate consensus evaluations on unchanged signature", () => {
    const org = "ci-dedupe-org";
    const cognition = minimalCognition(org);
    seedConsensusIntelligenceRuntime(org, cognition);

    const first = integrateConsensusIntelligenceWithCognition({
      organizationId: org,
      cognitionSnapshot: cognition,
      fragilityElevated: true,
      now: 114_000,
    });
    const second = integrateConsensusIntelligenceWithCognition({
      organizationId: org,
      cognitionSnapshot: cognition,
      fragilityElevated: true,
      now: 114_100,
  ...
  it("keeps bounded consensus intelligence memory under caps", () => {
    const org = "ci-bounded-org";
    const cognition = minimalCognition(org);
    seedConsensusIntelligenceRuntime(org, cognition);

    for (let i = 0; i < 20; i += 1) {
      evaluateExecutiveConsensusIntelligence(
        consensusEvalInput(org, { ...cognition, signature: `ci-bounded-${i}` }, 115_000 + i * 600)
      );
    }

    const state = getConsensusIntelligenceStore(org).getState();
    expect(state.consensusRecords.length).toBeLessThanOrEqual(10);
    expect(state.snapshots.length).toBeLessThanOrEqual(8);
    expect(state.reasoningPerspectives.length).toBeLessThanOrEqual(12);
  });

  it("blocks recursive consensus intelligence evaluation", () => {
    expect(beginConsensusIntelligenceEvaluation()).toBe(true);
    expect(beginConsensusIntelligenceEvaluation()).toBe(true);
    expect(beginConsensusIntelligenceEvaluation()).toBe(false);
    endConsensusIntelligenceEvaluation();
    endConsensusIntelligenceEvaluation();
  });

  it("emits consensus intelligence contract fields", () => {
    const org = "ci-contract-org";
    const cognition = minimalCognition(org);
    seedConsensusIntelligenceRuntime(org, cognition);

    const result = evaluateExecutiveConsensusIntelligence(
      consensusEvalInput(org, cognition, 116_000)
    );

    expect(result.evaluated).toBe(true);
    const record = result.snapshot?.recentConsensusRecords[0];
    expect(record).toBeDefined();
    expect(record!.consensusId.length).toBeGreaterThan(0);
    expect(record!.consensusState.length).toBeGreaterThan(0);
    expect(record!.consensusStrength.length).toBeGreaterThan(0);
    expect(record!.alignedPerspectives.length).toBeGreaterThan(0);
    expect(record!.consensusSignals.length).toBeGreaterThan(0);
    expect(record!.confidence).toBeGreaterThanOrEqual(0.48);
    expect(record!.generatedAt).toBe(116_000);
    expect(selectLatestStrategicConsensusSnapshot(org)?.reasoningPerspectives.length).toBeGreaterThan(
      0
    );
  });
});
