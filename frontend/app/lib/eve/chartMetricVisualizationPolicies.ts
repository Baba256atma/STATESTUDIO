const policyNames = Object.freeze([
  ...
  "Inventory derivation policy", "Dependency isolation policy",
  "Metadata-only policy", "Canonical Inventory Rule policy",
] as const);

export const ChartMetricVisualizationRegistryPolicies = Object.freeze(policyNames.map(
  (name, index) => Object.freeze({
    id: `EVE-5:2/Policy/${index + 1}` as const,
    name,
