...
import { getPriorityArbitrationStore, resetPriorityArbitrationStores } from "./priorityArbitrationStore";
...
function minimalCognition(org = "priority-arbitration-org"): AdaptiveGovernanceIntelligenceSnapshot {
  return {
...
function seedPriorityArbitrationRuntime(
  organizationId: string,
  cognition: AdaptiveGovernanceIntelligenceSnapshot
) {
  for (let i = 0; i < 3; i += 1) {
    const snapshot = { ...cognition, signature: `pa-seed-${i}` };
  ...
  for (let i = 0; i < 4; i += 1) {
    evaluateInstitutionalMemoryAccumulation({
      organizationId,
      cognitionSnapshot: { ...cognition, signature: `pa-temporal-${i}` },
...
describe("strategic priority arbitration D9:5:3", () => {
  beforeEach(() => {
    resetAllStacks();
  });

  it("arbitrates competing enterprise priorities deterministically", () => {
    const org = "pa-accumulate-org";
    const cognition = minimalCognition(org);
    seedPriorityArbitrationRuntime(org, cognition);

    const result = evaluateStrategicPriorityArbitration({
      organizationId: org,
      cognitionSnapshot: cognition,
      fragilityElevated: true,
      pressureTopologyStressed: true,
    ...
    expect(getPriorityArbitrationStore(org).getState().executiveArbitrations.length).toBeGreaterThan(0);
    expect(getPriorityArbitrationStore(org).getState().decisionTradeoffs.length).toBeGreaterThan(0);
  });

  it("forms resilience vs speed conflicting tradeoff under pressure", () => {
    const org = "pa-resilience-speed-org";
    const cognition = minimalCognition(org);
    seedPriorityArbitrationRuntime(org, cognition);
    ...
    expect(result.evaluated).toBe(true);
    const arbitrations = getPriorityArbitrationStore(org).getState().executiveArbitrations;
    expect(
      arbitrations.some(
        (a) =>
          a.tradeoffType === "conflicting" &&
  ...
  it("dedupes duplicate arbitration evaluations on unchanged signature", () => {
    const org = "pa-dedupe-org";
    const cognition = minimalCognition(org);
    seedPriorityArbitrationRuntime(org, cognition);

    const first = integratePriorityArbitrationWithCognition({
      organizationId: org,
      cognitionSnapshot: cognition,
      fragilityElevated: true,
      pressureTopologyStressed: true,
    ...
    const second = integratePriorityArbitrationWithCognition({
      organizationId: org,
      cognitionSnapshot: cognition,
      fragilityElevated: true,
      pressureTopologyStressed: true,
  ...
  it("suppresses arbitration when dependency depth is insufficient", () => {
    const org = "pa-shallow-org";
    const cognition = minimalCognition(org);
    ...
    expect(result.reason).toBe("insufficient_arbitration_depth");
  });

  it("keeps bounded arbitration memory under caps", () => {
    const org = "pa-bounded-org";
    const cognition = minimalCognition(org);
    seedPriorityArbitrationRuntime(org, cognition);
    ...
    for (let i = 0; i < 20; i += 1) {
      evaluateStrategicPriorityArbitration({
        organizationId: org,
        cognitionSnapshot: { ...cognition, signature: `pa-bounded-${i}` },
        fragilityElevated: true,
        pressureTopologyStressed: true,
        now: 25_000 + i * 600,
      });
    ...
    const state = getPriorityArbitrationStore(org).getState();
    expect(state.executiveArbitrations.length).toBeLessThanOrEqual(10);
    expect(state.snapshots.length).toBeLessThanOrEqual(8);
  });

  it("blocks recursive priority arbitration evaluation", () => {
    expect(beginPriorityArbitrationEvaluation()).toBe(true);
    expect(beginPriorityArbitrationEvaluation()).toBe(true);
    expect(beginPriorityArbitrationEvaluation()).toBe(false);
    endPriorityArbitrationEvaluation();
    endPriorityArbitrationEvaluation();
  });

  it("emits executive priority arbitration contract fields", () => {
    const org = "pa-contract-org";
    const cognition = minimalCognition(org);
    seedPriorityArbitrationRuntime(org, cognition);
    ...
    expect(result.evaluated).toBe(true);
    const arbitration = result.snapshot?.recentExecutiveArbitrations[0];
    expect(arbitration).toBeDefined();
    expect(arbitration!.arbitrationId.length).toBeGreaterThan(0);
    expect(arbitration!.competingPriorities.length).toBeGreaterThanOrEqual(2);
    expect(arbitration!.balancingSignals.length).toBeGreaterThan(0);
    expect(arbitration!.confidence).toBeGreaterThanOrEqual(0.5);
    expect(arbitration!.generatedAt).toBe(26_000);
  });
});
