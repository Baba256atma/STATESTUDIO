...
function minimalCognition(org = "stability-optimization-org"): AdaptiveGovernanceIntelligenceSnapshot {
  return {
...
function seedStabilityOptimizationRuntime(
  organizationId: string,
  cognition: AdaptiveGovernanceIntelligenceSnapshot
) {
  for (let i = 0; i < 3; i += 1) {
    const snapshot = { ...cognition, signature: `so-seed-${i}` };
  ...
  for (let i = 0; i < 4; i += 1) {
    evaluateInstitutionalMemoryAccumulation({
      organizationId,
      cognitionSnapshot: { ...cognition, signature: `so-temporal-${i}` },
...
describe("strategic stability optimization D9:5:9", () => {
  beforeEach(() => {
    resetAllStacks();
  });

  it("evaluates resilience pathways deterministically", () => {
    const org = "so-accumulate-org";
    const cognition = minimalCognition(org);
    seedStabilityOptimizationRuntime(org, cognition);

    const result = evaluateStrategicStabilityOptimization({
      organizationId: org,
      cognitionSnapshot: cognition,
      fragilityElevated: true,
      pressureTopologyStressed: true,
    ...
    expect(getStabilityOptimizationStore(org).getState().stabilityOptimizations.length).toBeGreaterThan(0);
    expect(getStabilityOptimizationStore(org).getState().resiliencePathways.length).toBeGreaterThan(0);
  });

  it("forms sustainability reinforcement with resilience signals", () => {
    const org = "so-sustainability-org";
    const cognition = minimalCognition(org);
    seedStabilityOptimizationRuntime(org, cognition);
    ...
    expect(result.evaluated).toBe(true);
    const optimizations = getStabilityOptimizationStore(org).getState().stabilityOptimizations;
    expect(
      optimizations.some(
        (o) =>
          o.resilienceSignals.includes("governance_continuity") ||
          o.optimizationState === "resilient" ||
          o.optimizationStrength === "strong"
      )
    ).toBe(true);
    expect(getStabilityOptimizationStore(org).getState().sustainabilitySignals.length).toBeGreaterThan(0);
  });

  it("dedupes duplicate optimization evaluations on unchanged signature", () => {
    const org = "so-dedupe-org";
    const cognition = minimalCognition(org);
    seedStabilityOptimizationRuntime(org, cognition);

    const first = integrateStabilityOptimizationWithCognition({
      organizationId: org,
      cognitionSnapshot: cognition,
      fragilityElevated: true,
      pressureTopologyStressed: true,
    ...
    const second = integrateStabilityOptimizationWithCognition({
      organizationId: org,
      cognitionSnapshot: cognition,
      fragilityElevated: true,
      pressureTopologyStressed: true,
  ...
  it("suppresses optimization when orchestration depth is insufficient", () => {
    const org = "so-shallow-org";
    const cognition = minimalCognition(org);
    ...
    expect(result.reason).toBe("insufficient_optimization_depth");
  });

  it("keeps bounded stability optimization memory under caps", () => {
    const org = "so-bounded-org";
    const cognition = minimalCognition(org);
    seedStabilityOptimizationRuntime(org, cognition);
    ...
    for (let i = 0; i < 20; i += 1) {
      evaluateStrategicStabilityOptimization({
        organizationId: org,
        cognitionSnapshot: { ...cognition, signature: `so-bounded-${i}` },
        fragilityElevated: true,
        pressureTopologyStressed: true,
        now: 29_000 + i * 600,
      });
    ...
    const state = getStabilityOptimizationStore(org).getState();
    expect(state.stabilityOptimizations.length).toBeLessThanOrEqual(10);
    expect(state.snapshots.length).toBeLessThanOrEqual(8);
  });

  it("blocks recursive stability optimization evaluation", () => {
    expect(beginStabilityOptimizationEvaluation()).toBe(true);
    expect(beginStabilityOptimizationEvaluation()).toBe(true);
    expect(beginStabilityOptimizationEvaluation()).toBe(false);
    endStabilityOptimizationEvaluation();
    endStabilityOptimizationEvaluation();
  });

  it("emits strategic stability optimization contract fields", () => {
    const org = "so-contract-org";
    const cognition = minimalCognition(org);
    seedStabilityOptimizationRuntime(org, cognition);
    ...
    expect(result.evaluated).toBe(true);
    const optimization = result.snapshot?.recentStabilityOptimizations[0];
    expect(optimization).toBeDefined();
    expect(optimization!.optimizationId.length).toBeGreaterThan(0);
    expect(optimization!.optimizationState.length).toBeGreaterThan(0);
    expect(optimization!.optimizationStrength.length).toBeGreaterThan(0);
    expect(optimization!.resilienceSignals.length).toBeGreaterThan(0);
    expect(optimization!.sustainabilityRisks).toBeDefined();
    expect(optimization!.confidence).toBeGreaterThanOrEqual(0.48);
    expect(optimization!.generatedAt).toBe(30_000);
  });
});
