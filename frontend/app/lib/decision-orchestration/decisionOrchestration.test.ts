...
import { getDecisionOrchestrationStore, resetDecisionOrchestrationStores } from "./decisionOrchestrationStore";
...
function minimalCognition(org = "decision-orchestration-org"): AdaptiveGovernanceIntelligenceSnapshot {
  return {
...
function seedDecisionOrchestrationRuntime(
  organizationId: string,
  cognition: AdaptiveGovernanceIntelligenceSnapshot
) {
  for (let i = 0; i < 3; i += 1) {
    const snapshot = { ...cognition, signature: `do-seed-${i}` };
  ...
  for (let i = 0; i < 4; i += 1) {
    evaluateInstitutionalMemoryAccumulation({
      organizationId,
      cognitionSnapshot: { ...cognition, signature: `do-temporal-${i}` },
...
describe("executive decision orchestration D9:5:1", () => {
  beforeEach(() => {
    resetAllStacks();
  });

  it("organizes strategic decision orchestrations deterministically", () => {
    const org = "do-accumulate-org";
    const cognition = minimalCognition(org);
    seedDecisionOrchestrationRuntime(org, cognition);

    const result = evaluateExecutiveDecisionOrchestration({
      organizationId: org,
      cognitionSnapshot: cognition,
      fragilityElevated: true,
      pressureTopologyStressed: true,
    ...
    expect(getDecisionOrchestrationStore(org).getState().strategicOrchestrations.length).toBeGreaterThan(
      0
    );
    expect(getDecisionOrchestrationStore(org).getState().responseDependencies.length).toBeGreaterThan(0);
  });

  it("forms stabilization sequence with governance-before-pressure dependency", () => {
    const org = "do-stabilization-org";
    const cognition = minimalCognition(org);
    seedDecisionOrchestrationRuntime(org, cognition);
    ...
    expect(result.evaluated).toBe(true);
    const orchestrations = getDecisionOrchestrationStore(org).getState().strategicOrchestrations;
    expect(
      orchestrations.some(
        (o) =>
          o.actionSequence.includes("governance_alignment") &&
          o.actionSequence.includes("pressure_reduction")
      )
    ).toBe(true);
    const deps = getDecisionOrchestrationStore(org).getState().responseDependencies;
    expect(
      deps.some(
        (d) =>
          d.prerequisiteCategory === "governance_alignment" &&
          d.dependentCategory === "pressure_reduction"
      )
    ).toBe(true);
  });

  it("dedupes duplicate orchestration evaluations on unchanged signature", () => {
    const org = "do-dedupe-org";
    const cognition = minimalCognition(org);
    seedDecisionOrchestrationRuntime(org, cognition);

    const first = integrateDecisionOrchestrationWithCognition({
      organizationId: org,
      cognitionSnapshot: cognition,
      fragilityElevated: true,
      pressureTopologyStressed: true,
    ...
    const second = integrateDecisionOrchestrationWithCognition({
      organizationId: org,
      cognitionSnapshot: cognition,
      fragilityElevated: true,
      pressureTopologyStressed: true,
  ...
  it("suppresses orchestration when depth is insufficient", () => {
    const org = "do-shallow-org";
    const cognition = minimalCognition(org);
  ...
  it("keeps bounded orchestration memory under caps", () => {
    const org = "do-bounded-org";
    const cognition = minimalCognition(org);
    seedDecisionOrchestrationRuntime(org, cognition);
    ...
    for (let i = 0; i < 20; i += 1) {
      evaluateExecutiveDecisionOrchestration({
        organizationId: org,
        cognitionSnapshot: { ...cognition, signature: `do-bounded-${i}` },
        fragilityElevated: true,
        pressureTopologyStressed: true,
        now: 24_000 + i * 600,
      });
    ...
    const state = getDecisionOrchestrationStore(org).getState();
    expect(state.strategicOrchestrations.length).toBeLessThanOrEqual(10);
    expect(state.snapshots.length).toBeLessThanOrEqual(8);
  });

  it("blocks recursive decision orchestration evaluation", () => {
    expect(beginDecisionOrchestrationEvaluation()).toBe(true);
    expect(beginDecisionOrchestrationEvaluation()).toBe(true);
    expect(beginDecisionOrchestrationEvaluation()).toBe(false);
    endDecisionOrchestrationEvaluation();
    endDecisionOrchestrationEvaluation();
  });

  it("emits strategic decision orchestration contract fields", () => {
    const org = "do-contract-org";
    const cognition = minimalCognition(org);
    seedDecisionOrchestrationRuntime(org, cognition);
    ...
    expect(result.evaluated).toBe(true);
    const orchestration = result.snapshot?.recentStrategicOrchestrations[0];
    expect(orchestration).toBeDefined();
    expect(orchestration!.orchestrationId.length).toBeGreaterThan(0);
    expect(orchestration!.actionSequence.length).toBeGreaterThanOrEqual(2);
    expect(orchestration!.confidence).toBeGreaterThanOrEqual(0.5);
    expect(orchestration!.generatedAt).toBe(25_000);
  });
});
