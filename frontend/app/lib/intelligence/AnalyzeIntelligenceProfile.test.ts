...
test("exports INT-1 analyze contract completion tag", () => {
  assert.equal(INT1_ANALYZE_CONTRACT_COMPLETE_TAG, "[INT1_ANALYZE_CONTRACT_COMPLETE]");
  assert.equal(ANALYZE_INTELLIGENCE_CONTRACT_DIAGNOSTIC, "[ANALYZE_INTELLIGENCE_CONTRACT]");
  assert.equal(ANALYZE_INTELLIGENCE_CONTRACT_READY_DIAGNOSTIC, "[ANALYZE_INTELLIGENCE_CONTRACT_READY]");
});

test("builds canonical analyze intelligence profile from executive snapshot", () => {
  ...
  assert.equal(profile.diagnostics.includes(ANALYZE_INTELLIGENCE_CONTRACT_DIAGNOSTIC), true);
  assert.equal(profile.diagnostics.includes(ANALYZE_INTELLIGENCE_CONTRACT_READY_DIAGNOSTIC), true);
  ...
  assert.equal(Object.isFrozen(profile.confidence), true);
});
...
test("maps executive snapshot into trend scenario and confidence exposures", () => {
  const profile = buildAnalyzeIntelligenceProfile({ sceneJson: SAMPLE_SCENE });
  ...
  assert.ok(profile.confidence.summary.includes("Analyze confidence composite"));
});
