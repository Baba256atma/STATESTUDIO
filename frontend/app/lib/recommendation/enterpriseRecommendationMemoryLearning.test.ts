/**
 * D7:5:5 — Strategic recommendation memory + learning intelligence tests.
 */
...
function buildTopology() {
  ...
  const built = buildOperationalUniverseTopology({ topologyId: "topo-learning", objects });
  assert.ok(built.ok);
...
function learningInput(stack: ReturnType<typeof buildLearningStack>) {
  return {
...
test("deterministic learning analysis", () => {
  const stack = buildLearningStack();
  const r1 = evaluateRecommendationLearning({
    ...learningInput(stack),
    learningContext: { memoryLeverageFactor: 0.1 },
  });
  const r2 = evaluateRecommendationLearning({
    ...learningInput(stack),
    learningContext: { memoryLeverageFactor: 0.1 },
  });
...
test("historical-outcome modeling", () => {
  const stack = buildLearningStack();
  const memories = deriveRecommendationMemorySignals({
    ...learningInput(stack),
    memoryLeverageFactor: 0.1,
  });
  const outcomes = analyzeHistoricalOutcomes({
    memories,
    recommendationState: stack.recommendations,
    comparisonState: stack.comparison,
    adaptationState: stack.adaptation,
  ...
  const stability = calculateLearningStabilityScore({
    memories,
    confidenceState: stack.confidence,
  });
  ...
  for (const m of memories) {
    assert.ok(m.memoryStrength <= 0.92);
  }
});

test("recommendation-history consistency", () => {
  const stack = buildLearningStack();
  const memories = deriveRecommendationMemorySignals(learningInput(stack));
  const outcomes = analyzeHistoricalOutcomes({
    memories,
    recommendationState: stack.recommendations,
    comparisonState: stack.comparison,
  ...
  const p1 = analyzePatternLearning({
    memories,
    comparisonState: stack.comparison,
    trajectoryState: stack.trajectory,
    divergenceState: stack.divergence,
  ...
  const p2 = analyzePatternLearning({
    memories,
    comparisonState: stack.comparison,
    trajectoryState: stack.trajectory,
    divergenceState: stack.divergence,
  ...
  const e1 = analyzeExecutiveStrategicMemory({
    memories,
  ...
  const e2 = analyzeExecutiveStrategicMemory({
    memories,
    outcomeRecords: outcomes,
    trajectoryState: stack.trajectory,
    momentumState: stack.momentum,
...
test("validated-pathway testing", () => {
  const stack = buildLearningStack();
  const result = evaluateRecommendationLearning(learningInput(stack));
  assert.ok(result.ok);
  if (!result.ok) return;
  assert.ok(
    result.snapshot.state.historicalOutcomeRecords.some(
      (r) =>
        r.outcomeType === "successful_recovery_pathway" ||
        r.outcomeType === "validated_stabilization" ||
        r.outcomeType === "resilience_learning_evolution"
    )
  );
  assert.ok(result.snapshot.state.patternLearningRecords.length > 0);
});

test("replay-safe memory snapshots", () => {
  const stack = buildLearningStack();
  const result = evaluateRecommendationLearning(learningInput(stack));
  assert.ok(result.ok);
  if (!result.ok) return;
  const frozen = freezeStrategicRecommendationMemorySnapshot(result.snapshot);
  assert.throws(() => {
    (frozen.state as { learningStabilityScore: number }).learningStabilityScore = 0;
  });
});

test("governance guard rail enforcement", () => {
  assert.ok(containsFalseCertaintyText("guaranteed learning outcome"));
  const guard = guardEvaluateRecommendationLearning({
    ...
    memories: [
      {
        memoryId: "memory::bad",
        originatingRecommendationId: "rec-bad",
  ...
  if (guard.ok) return;
  assert.equal(guard.code, "invalid_memory_region");

  const semanticsGuard = guardLearningExecutiveSemantics({
    headline: "Autonomous learning will rewrite recommendation policies",
    summary: "Historical pattern",
  });
  assert.equal(semanticsGuard.ok, false);
  if (semanticsGuard.ok) return;
  assert.equal(semanticsGuard.code, "uncontrolled_self_learning");
});

test("immutable memory state preservation", () => {
  const stack = buildLearningStack();
  const frozenComparison = JSON.stringify(stack.comparison);
  evaluateRecommendationLearning(learningInput(stack));
  assert.equal(JSON.stringify(stack.comparison), frozenComparison);
});

test("executive-readable learning semantics", () => {
  const stack = buildLearningStack();
  const result = evaluateRecommendationLearning({ ...learningInput(stack), tick: 8 });
  assert.ok(result.ok);
  if (!result.ok) return;
  assert.match(
    result.snapshot.semantics.headline,
    /recovery|logistics|stabilization|resilience|fragility|pattern|memory|historical/i
  );
  assert.ok(!result.snapshot.semantics.headline.includes("Learning recursion optimization exceeded"));
  assert.equal(result.snapshot.state.learningAmbiguityDisclaimer, LEARNING_AMBIGUITY_DISCLAIMER);
  assert.equal(
    result.snapshot.state.nonAutonomousLearningDisclaimer,
    NON_AUTONOMOUS_LEARNING_DISCLAIMER
  );

  const manual = buildExecutiveRecommendationLearningSemantics({ state: result.snapshot.state });
  assert.ok(manual.summary.includes("Indicative"));
});

test("integrated memory panel contract", () => {
  const stack = buildLearningStack();
  const result = evaluateRecommendationLearning(learningInput(stack));
  assert.ok(result.ok);
  if (!result.ok) return;
  assert.equal(result.panelContract.topologyId, stack.topology.topologyId);
  assert.equal(result.panelContract.learningAmbiguityDisclaimer, LEARNING_AMBIGUITY_DISCLAIMER);
  assert.equal(
    result.panelContract.nonAutonomousLearningDisclaimer,
    NON_AUTONOMOUS_LEARNING_DISCLAIMER
  );
  assert.ok(result.panelContract.memories.length > 0);
});

test("rejects duplicate memory build fingerprint", () => {
  const stack = buildLearningStack();
  const first = evaluateRecommendationLearning({ ...learningInput(stack), tick: 0 });
  assert.ok(first.ok);
  if (!first.ok) return;
  const fp = buildMemoryContentFingerprint({
  ...
  const second = evaluateRecommendationLearning({
    ...learningInput(stack),
    tick: 0,
    priorMemoryFingerprints: [fp],
  });
  assert.equal(second.ok, false);
  if (second.ok) return;
  assert.equal(second.guard.code, "duplicate_memory_build");
});

test("learning stability classification", () => {
  const label = classifyExecutiveLearningLabel({
    learningStabilityScore: 0.65,
    patternRecurrenceScore: 0.2,
    validationConfidenceScore: 0.7,
  });
  assert.equal(label, "validated");
});
