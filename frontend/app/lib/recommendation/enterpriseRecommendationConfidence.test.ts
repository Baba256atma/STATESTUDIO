/**
 * D7:5:2 — Strategic recommendation confidence intelligence tests.
 */
...
function buildTopology() {
  ...
  const built = buildOperationalUniverseTopology({ topologyId: "topo-confidence", objects });
  assert.ok(built.ok);
...
function confidenceInput(stack: ReturnType<typeof buildConfidenceStack>) {
  return {
...
test("deterministic confidence analysis", () => {
  const stack = buildConfidenceStack();
  const r1 = evaluateRecommendationConfidence({
    ...confidenceInput(stack),
    confidenceContext: { confidenceLeverageFactor: 0.1 },
  });
  const r2 = evaluateRecommendationConfidence({
    ...confidenceInput(stack),
    confidenceContext: { confidenceLeverageFactor: 0.1 },
  });
...
test("confidence scoring modeling", () => {
  const stack = buildConfidenceStack();
  const signals = deriveRecommendationConfidenceSignals(confidenceInput(stack));
  const overall = calculateOverallConfidenceScore({
    signals,
    recommendationState: stack.recommendations,
  });
  ...
  for (const signal of signals) {
    assert.ok(signal.evidenceStrength <= 0.92);
  }
});

test("uncertainty consistency validation", () => {
  const stack = buildConfidenceStack();
  const signals = deriveRecommendationConfidenceSignals(confidenceInput(stack));
  const u1 = analyzeRecommendationUncertainty({
    topology: stack.topology,
    signals,
    recommendationState: stack.recommendations,
    foresightState: stack.foresight,
  ...
  const u2 = analyzeRecommendationUncertainty({
    topology: stack.topology,
    signals,
    recommendationState: stack.recommendations,
    foresightState: stack.foresight,
...
test("evidence-strength testing", () => {
  const stack = buildConfidenceStack();
  const signals = deriveRecommendationConfidenceSignals(confidenceInput(stack));
  const e1 = analyzeEvidenceStrength({
    signals,
    recommendationState: stack.recommendations,
    foresightState: stack.foresight,
    recoveryOpportunityState: stack.recoveryOpportunity,
  ...
  const e2 = analyzeEvidenceStrength({
    signals,
    recommendationState: stack.recommendations,
    foresightState: stack.foresight,
    recoveryOpportunityState: stack.recoveryOpportunity,
...
test("replay-safe confidence snapshots", () => {
  const stack = buildConfidenceStack();
  const result = evaluateRecommendationConfidence(confidenceInput(stack));
  assert.ok(result.ok);
  if (!result.ok) return;
  const frozen = freezeRecommendationConfidenceSnapshot(result.snapshot);
  assert.throws(() => {
    (frozen.state as { overallConfidenceScore: number }).overallConfidenceScore = 0;
  });
});

test("governance guard rail enforcement", () => {
  assert.ok(containsFalseCertaintyText("guaranteed confidence outcome"));
  ...
  if (guard.ok) return;
  assert.equal(guard.code, "invalid_confidence_region");

  const semanticsGuard = guardConfidenceExecutiveSemantics({
    headline: "Confidence is guaranteed",
    summary: "Evidence strength",
  });
  assert.equal(semanticsGuard.ok, false);
});

test("immutable confidence state preservation", () => {
  const stack = buildConfidenceStack();
  const frozenRecs = JSON.stringify(stack.recommendations);
  evaluateRecommendationConfidence(confidenceInput(stack));
  assert.equal(JSON.stringify(stack.recommendations), frozenRecs);
});

test("executive-readable confidence semantics", () => {
  const stack = buildConfidenceStack();
  const result = evaluateRecommendationConfidence({ ...confidenceInput(stack), tick: 8 });
  assert.ok(result.ok);
  if (!result.ok) return;
  assert.match(
    result.snapshot.semantics.headline,
    /confidence|recovery|divergence|manufacturing|evidence|uncertainty|recommendation/i
  );
  assert.ok(!result.snapshot.semantics.headline.includes("Confidence recursion variance exceeded"));
  assert.equal(result.snapshot.state.uncertaintyDisclaimer, CONFIDENCE_UNCERTAINTY_DISCLAIMER);

  const manual = buildExecutiveRecommendationConfidenceSemantics({ state: result.snapshot.state });
  assert.ok(manual.summary.includes("Indicative"));
});

test("integrated confidence panel contract", () => {
  const stack = buildConfidenceStack();
  const result = evaluateRecommendationConfidence(confidenceInput(stack));
  assert.ok(result.ok);
  if (!result.ok) return;
  assert.equal(result.panelContract.topologyId, stack.topology.topologyId);
  assert.equal(result.panelContract.uncertaintyDisclaimer, CONFIDENCE_UNCERTAINTY_DISCLAIMER);
  assert.ok(result.panelContract.signals.length > 0);
});

test("rejects duplicate confidence build fingerprint", () => {
  const stack = buildConfidenceStack();
  const first = evaluateRecommendationConfidence({ ...confidenceInput(stack), tick: 0 });
  assert.ok(first.ok);
  if (!first.ok) return;
  const fp = buildConfidenceContentFingerprint({
    ...
    recommendationFingerprint: stableStringify({
      ...
      confidence: stack.recommendations.recommendationConfidenceScore,
  ...
  const second = evaluateRecommendationConfidence({
    ...confidenceInput(stack),
    tick: 0,
    priorConfidenceFingerprints: [fp],
  });
  assert.equal(second.ok, false);
  if (second.ok) return;
  assert.equal(second.guard.code, "duplicate_confidence_build");
});

test("recommendation confidence label classification", () => {
  const label = classifyRecommendationConfidenceLabel({
    overallConfidenceScore: 0.65,
    uncertaintyAmplificationScore: 0.2,
    evidenceStabilityScore: 0.6,
  });
