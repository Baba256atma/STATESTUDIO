/**
 * D7:5:6 — Executive strategic governance intelligence tests.
 */
...
function buildTopology() {
  ...
  const built = buildOperationalUniverseTopology({ topologyId: "topo-governance", objects });
  assert.ok(built.ok);
...
function governanceInput(stack: ReturnType<typeof buildGovernanceStack>) {
  return {
...
test("deterministic governance analysis", () => {
  const stack = buildGovernanceStack();
  const r1 = evaluateStrategicGovernance({
    ...governanceInput(stack),
    governanceContext: { governanceLeverageFactor: 0.1 },
  });
  const r2 = evaluateStrategicGovernance({
    ...governanceInput(stack),
    governanceContext: { governanceLeverageFactor: 0.1 },
  });
...
test("governance-alignment modeling", () => {
  const stack = buildGovernanceStack();
  const signals = deriveStrategicGovernanceSignals({
    ...governanceInput(stack),
    governanceLeverageFactor: 0.1,
  });
  const alignment = analyzeGovernanceAlignment({
  ...
  const stability = calculateGovernanceStabilityScore({
    governanceSignals: signals,
    confidenceState: stack.confidence,
    memoryState: stack.memory,
  });
  ...
  for (const g of signals) {
    assert.ok(g.governanceStrength <= 0.92);
  }
});

test("recommendation-safety consistency", () => {
  const stack = buildGovernanceStack();
  const signals = deriveStrategicGovernanceSignals(governanceInput(stack));
  const alignment = analyzeGovernanceAlignment({
    governanceSignals: signals,
    recommendationState: stack.recommendations,
    confidenceState: stack.confidence,
  ...
  const s1 = analyzeRecommendationSafety({
    governanceSignals: signals,
    comparisonState: stack.comparison,
    memoryState: stack.memory,
    confidenceState: stack.confidence,
  ...
  const s2 = analyzeRecommendationSafety({
    governanceSignals: signals,
    comparisonState: stack.comparison,
    memoryState: stack.memory,
    confidenceState: stack.confidence,
  ...
  const o1 = analyzeExecutiveOversight({
    governanceSignals: signals,
  ...
  const o2 = analyzeExecutiveOversight({
    governanceSignals: signals,
    alignmentRecords: alignment,
    safetyRecords: s2,
    trajectoryState: stack.trajectory,
...
test("executive oversight testing", () => {
  const stack = buildGovernanceStack();
  const result = evaluateStrategicGovernance(governanceInput(stack));
  assert.ok(result.ok);
  if (!result.ok) return;
  assert.ok(result.snapshot.state.executiveOversightRecords.length >= 6);
  assert.ok(result.snapshot.state.governanceAlignmentRecords.length > 0);
});

test("replay-safe governance snapshots", () => {
  const stack = buildGovernanceStack();
  const result = evaluateStrategicGovernance(governanceInput(stack));
  assert.ok(result.ok);
  if (!result.ok) return;
  const frozen = freezeExecutiveStrategicGovernanceSnapshot(result.snapshot);
  assert.throws(() => {
    (frozen.state as { governanceStabilityScore: number }).governanceStabilityScore = 0;
  });
});

test("governance guard rail enforcement", () => {
  assert.ok(containsFalseCertaintyText("guaranteed governance outcome"));
  ...
  if (guard.ok) return;
  assert.equal(guard.code, "invalid_governance_region");

  const semanticsGuard = guardGovernanceExecutiveSemantics({
    headline: "Autonomous governance enforcement will override executive authority",
    summary: "Governance review",
  });
  assert.equal(semanticsGuard.ok, false);
  if (semanticsGuard.ok) return;
  assert.equal(semanticsGuard.code, "autonomous_governance_enforcement");
});

test("immutable governance state preservation", () => {
  const stack = buildGovernanceStack();
  const frozenMemory = JSON.stringify(stack.memory);
  evaluateStrategicGovernance(governanceInput(stack));
  assert.equal(JSON.stringify(stack.memory), frozenMemory);
});

test("executive-readable governance semantics", () => {
  const stack = buildGovernanceStack();
  const result = evaluateStrategicGovernance({ ...governanceInput(stack), tick: 8 });
  assert.ok(result.ok);
  if (!result.ok) return;
  assert.match(
    result.snapshot.semantics.headline,
    /governance|logistics|oversight|recommendation|divergence|recovery|aligned|safeguard/i
  );
  assert.ok(!result.snapshot.semantics.headline.includes("Governance recursion escalation exceeded"));
  assert.equal(result.snapshot.state.governanceAmbiguityDisclaimer, GOVERNANCE_AMBIGUITY_DISCLAIMER);
  assert.equal(result.snapshot.state.nonEnforcementDisclaimer, NON_ENFORCEMENT_DISCLAIMER);

  const manual = buildExecutiveStrategicGovernanceSemantics({ state: result.snapshot.state });
  assert.ok(manual.summary.includes("Indicative"));
});

test("integrated governance panel contract", () => {
  const stack = buildGovernanceStack();
  const result = evaluateStrategicGovernance(governanceInput(stack));
  assert.ok(result.ok);
  if (!result.ok) return;
  assert.equal(result.panelContract.topologyId, stack.topology.topologyId);
  assert.equal(result.panelContract.governanceAmbiguityDisclaimer, GOVERNANCE_AMBIGUITY_DISCLAIMER);
  assert.equal(result.panelContract.nonEnforcementDisclaimer, NON_ENFORCEMENT_DISCLAIMER);
  assert.ok(result.panelContract.governanceSignals.length > 0);
});

test("rejects duplicate governance build fingerprint", () => {
  const stack = buildGovernanceStack();
  const first = evaluateStrategicGovernance({ ...governanceInput(stack), tick: 0 });
  assert.ok(first.ok);
  if (!first.ok) return;
  const fp = buildGovernanceContentFingerprint({
    topologyFingerprint: stack.topology.fingerprint,
    memoryFingerprint: stableStringify({
      label: stack.memory.executiveLearningLabel,
      stability: stack.memory.learningStabilityScore,
    }),
  ...
  const second = evaluateStrategicGovernance({
    ...governanceInput(stack),
    tick: 0,
    priorGovernanceFingerprints: [fp],
  });
  assert.equal(second.ok, false);
  if (second.ok) return;
  assert.equal(second.guard.code, "duplicate_governance_build");
});

test("governance stability classification", () => {
  const label = classifyExecutiveGovernanceLabel({
    governanceStabilityScore: 0.65,
    recommendationSafetyScore: 0.55,
    oversightRequirementScore: 0.2,
  });
