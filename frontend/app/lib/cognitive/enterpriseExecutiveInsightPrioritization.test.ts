/**
 * D7:6:4 — Executive insight prioritization intelligence tests.
 */
...
import { analyzeInsightUrgency } from "./insightUrgencyAnalysis.ts";
...
function buildTopology() {
  ...
  const built = buildOperationalUniverseTopology({ topologyId: "topo-insight-priority", objects });
  assert.ok(built.ok);
...
function buildInsightPrioritizationStack() {
...
function insightPrioritizationInput(stack: ReturnType<typeof buildInsightPrioritizationStack>) {
  return {
...
test("deterministic insight prioritization", () => {
  const stack = buildInsightPrioritizationStack();
  const r1 = evaluateExecutiveInsightPrioritization({
    ...insightPrioritizationInput(stack),
    insightPrioritizationContext: { insightLeverageFactor: 0.1 },
  });
  const r2 = evaluateExecutiveInsightPrioritization({
    ...insightPrioritizationInput(stack),
    insightPrioritizationContext: { insightLeverageFactor: 0.1 },
  });
...
test("strategic-value modeling", () => {
  const stack = buildInsightPrioritizationStack();
  const insights = deriveExecutiveInsightPrioritySignals({
    cognitiveLoadState: stack.cognitiveLoad,
    attentionRoutingState: stack.attentionRouting,
    cognitiveUxState: stack.cognitiveUx,
    orchestrationState: stack.orchestration,
    ...
    recoveryOpportunityState: stack.recoveryOpportunity,
    resilienceState: stack.resilience,
    insightLeverageFactor: 0.1,
  });
  const value = analyzeStrategicValue({
  ...
  const score = calculateStrategicInsightScore({
    insightSignals: insights,
    cognitiveLoadState: stack.cognitiveLoad,
    confidenceState: stack.confidence,
  });
  ...
  for (const i of insights) {
    assert.ok(i.priorityStrength <= 0.92);
  }
});

test("urgency consistency validation", () => {
  const stack = buildInsightPrioritizationStack();
  const insights = deriveExecutiveInsightPrioritySignals({
    cognitiveLoadState: stack.cognitiveLoad,
    attentionRoutingState: stack.attentionRouting,
  ...
  const u1 = analyzeInsightUrgency({
    insightSignals: insights,
    cognitiveLoadState: stack.cognitiveLoad,
    cognitiveUxState: stack.cognitiveUx,
    attentionRoutingState: stack.attentionRouting,
  ...
  const u2 = analyzeInsightUrgency({
    insightSignals: insights,
    cognitiveLoadState: stack.cognitiveLoad,
    cognitiveUxState: stack.cognitiveUx,
    attentionRoutingState: stack.attentionRouting,
  ...
  const s1 = analyzeExecutiveInsightIntelligence({
    insightSignals: insights,
    valueRecords: value,
    urgencyRecords: u1,
    trajectoryState: stack.trajectory,
  ...
  const s2 = analyzeExecutiveInsightIntelligence({
    insightSignals: insights,
    valueRecords: value,
    urgencyRecords: u2,
    trajectoryState: stack.trajectory,
  ...
  assert.equal(
    u1.map((r) => r.recordId).join("|"),
    u2.map((r) => r.recordId).join("|")
  );
  assert.equal(
...
test("executive insight testing", () => {
  const stack = buildInsightPrioritizationStack();
  const result = evaluateExecutiveInsightPrioritization(insightPrioritizationInput(stack));
  assert.ok(result.ok);
  if (!result.ok) return;
  assert.ok(result.snapshot.state.executiveInsightRecords.length >= 6);
  assert.ok(result.snapshot.state.strategicValueRecords.length > 0);
});

test("replay-safe prioritization snapshots", () => {
  const stack = buildInsightPrioritizationStack();
  const result = evaluateExecutiveInsightPrioritization(insightPrioritizationInput(stack));
  assert.ok(result.ok);
  if (!result.ok) return;
  const frozen = freezeExecutiveInsightPrioritizationSnapshot(result.snapshot);
  assert.throws(() => {
    (frozen.state as { strategicInsightScore: number }).strategicInsightScore = 0;
  });
});

test("governance guard rail enforcement", () => {
  assert.ok(containsFalseCertaintyText("guaranteed insight outcome"));
  const guard = guardEvaluateExecutiveInsightPrioritization({
    topologyId: "topo",
    regionIds: ["finance"],
    insightSignals: [
      {
        insightId: "insight::bad",
        affectedRegionIds: ["unknown"],
        priorityState: "visible",
        priorityStrength: 0.5,
      },
    ],
  });
  assert.equal(guard.ok, false);
  if (guard.ok) return;
  assert.equal(guard.code, "invalid_insight_region");

  const semanticsGuard = guardInsightPrioritizationExecutiveSemantics({
    headline: "Inflate urgency via hidden prioritization bias and artificial executive steering",
    summary: "Insight prioritization review",
  });
  assert.equal(semanticsGuard.ok, false);
  if (semanticsGuard.ok) return;
  assert.equal(semanticsGuard.code, "manipulative_urgency_inflation");
});

test("immutable prioritization state preservation", () => {
  const stack = buildInsightPrioritizationStack();
  const frozenLoad = JSON.stringify(stack.cognitiveLoad);
  evaluateExecutiveInsightPrioritization(insightPrioritizationInput(stack));
  assert.equal(JSON.stringify(stack.cognitiveLoad), frozenLoad);
});

test("executive-readable prioritization semantics", () => {
  const stack = buildInsightPrioritizationStack();
  const result = evaluateExecutiveInsightPrioritization({
    ...insightPrioritizationInput(stack),
    tick: 8,
  });
  assert.ok(result.ok);
  ...
  assert.match(
    ...
    /operational|recovery|coordination|fragility|insight|priority|divergence|logistics|manufacturing|governance|predictive|stability|intelligence|equilibrium|resilience/i
  );
  assert.ok(
    !result.snapshot.semantics.headline.includes("Insight recursion prioritization exceeded")
  );
  assert.equal(
    result.snapshot.state.prioritizationAmbiguityDisclaimer,
    PRIORITIZATION_AMBIGUITY_DISCLAIMER
  );
  assert.equal(
    result.snapshot.state.nonManipulationPrioritizationDisclaimer,
    NON_MANIPULATION_PRIORITIZATION_DISCLAIMER
  );

  const manual = buildExecutiveInsightPrioritizationSemantics({ state: result.snapshot.state });
  assert.ok(manual.summary.includes("Indicative"));
});

test("integrated prioritization panel contract", () => {
  const stack = buildInsightPrioritizationStack();
  const result = evaluateExecutiveInsightPrioritization(insightPrioritizationInput(stack));
  assert.ok(result.ok);
  if (!result.ok) return;
  ...
  assert.equal(
    result.panelContract.prioritizationAmbiguityDisclaimer,
    PRIORITIZATION_AMBIGUITY_DISCLAIMER
  );
  assert.equal(
    result.panelContract.nonManipulationPrioritizationDisclaimer,
    NON_MANIPULATION_PRIORITIZATION_DISCLAIMER
  );
  assert.ok(result.panelContract.insightSignals.length > 0);
});

test("rejects duplicate prioritization build fingerprint", () => {
  const stack = buildInsightPrioritizationStack();
  const first = evaluateExecutiveInsightPrioritization({
    ...insightPrioritizationInput(stack),
    tick: 0,
  });
  assert.ok(first.ok);
  if (!first.ok) return;
  const fp = buildInsightPrioritizationContentFingerprint({
    ...
    attentionRoutingFingerprint: stableStringify({
  ...
  const second = evaluateExecutiveInsightPrioritization({
    ...insightPrioritizationInput(stack),
    tick: 0,
    priorInsightPrioritizationFingerprints: [fp],
  });
  assert.equal(second.ok, false);
  if (second.ok) return;
  assert.equal(second.guard.code, "duplicate_prioritization_build");
});

test("insight prioritization classification", () => {
  const label = classifyExecutiveInsightPrioritizationLabel({
    insightSignals: [
      {
        insightId: "i1",
        affectedRegionIds: ["logistics"],
        priorityState: "background",
        priorityStrength: 0.6,
      },
    ],
    strategicInsightScore: 0.7,
    strategicValueScore: 0.35,
    urgencyEscalationScore: 0.3,
  });
  assert.equal(label, "background");
});
