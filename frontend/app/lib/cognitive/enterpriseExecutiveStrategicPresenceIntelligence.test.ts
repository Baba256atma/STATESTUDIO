/**
 * D7:6:8 — Executive strategic presence intelligence tests.
 */
...
import { analyzePresenceFragmentation } from "./presenceFragmentationAnalysis.ts";
...
function buildTopology() {
  ...
  const built = buildOperationalUniverseTopology({ topologyId: "topo-presence", objects });
  assert.ok(built.ok);
...
function presenceInput(stack: ReturnType<typeof buildPresenceStack>) {
  return {
...
test("deterministic presence orchestration", () => {
  const stack = buildPresenceStack();
  const r1 = evaluateExecutiveStrategicPresence({
    ...presenceInput(stack),
    presenceContext: { presenceLeverageFactor: 0.1 },
  });
  const r2 = evaluateExecutiveStrategicPresence({
    ...presenceInput(stack),
    presenceContext: { presenceLeverageFactor: 0.1 },
  });
...
test("situational-awareness modeling", () => {
  const stack = buildPresenceStack();
  const presences = deriveExecutiveStrategicPresenceSignals({
    immersionState: stack.immersion,
    timelineState: stack.timeline,
    narrativeState: stack.narrative,
    insightPrioritizationState: stack.insightPrioritization,
    ...
    recoveryOpportunityState: stack.recoveryOpportunity,
    resilienceState: stack.resilience,
    presenceLeverageFactor: 0.1,
  });
  const layers = analyzeSituationalAwarenessLayers({
    presenceSignals: presences,
    immersionState: stack.immersion,
    timelineState: stack.timeline,
    narrativeState: stack.narrative,
    foresightState: stack.foresight,
  ...
  const continuity = calculateSituationalContinuityScore({
    presenceSignals: presences,
    immersionState: stack.immersion,
    narrativeState: stack.narrative,
    timelineState: stack.timeline,
  });
  assert.ok(presences.length > 0);
  assert.ok(layers.length > 0);
  assert.ok(continuity >= 0 && continuity <= 1);
  for (const p of presences) {
    assert.ok(p.presenceStrength <= 0.92);
  }
});
...
test("continuity consistency validation", () => {
  ...
  const f1 = analyzePresenceFragmentation({
    presenceSignals: presences,
    immersionState: stack.immersion,
    timelineState: stack.timeline,
    narrativeState: stack.narrative,
  ...
  const f2 = analyzePresenceFragmentation({
    presenceSignals: presences,
    immersionState: stack.immersion,
    timelineState: stack.timeline,
    narrativeState: stack.narrative,
  ...
  const c1 = analyzeExecutiveContinuity({
    presenceSignals: presences,
    layerRecords: layers,
  ...
  const c2 = analyzeExecutiveContinuity({
    presenceSignals: presences,
    layerRecords: layers,
...
test("executive awareness testing", () => {
  const stack = buildPresenceStack();
  const result = evaluateExecutiveStrategicPresence(presenceInput(stack));
  assert.ok(result.ok);
  if (!result.ok) return;
  assert.ok(result.snapshot.state.executiveContinuityRecords.length >= 6);
  assert.ok(result.snapshot.state.situationalAwarenessLayerRecords.length > 0);
});

test("replay-safe presence snapshots", () => {
  const stack = buildPresenceStack();
  const result = evaluateExecutiveStrategicPresence(presenceInput(stack));
  assert.ok(result.ok);
  if (!result.ok) return;
  const frozen = freezeExecutiveStrategicPresenceSnapshot(result.snapshot);
  assert.throws(() => {
    (frozen.state as { situationalContinuityScore: number }).situationalContinuityScore = 0;
  });
});

test("governance guard rail enforcement", () => {
  assert.ok(containsFalseCertaintyText("guaranteed presence outcome"));
  const guard = guardEvaluateExecutiveStrategicPresence({
    topologyId: "topo",
    regionIds: ["finance"],
    presenceSignals: [
      {
        presenceId: "presence::bad",
        affectedRegionIds: ["unknown"],
        presenceState: "aware",
        presenceStrength: 0.5,
      },
    ],
  });
  assert.equal(guard.ok, false);
  if (guard.ok) return;
  assert.equal(guard.code, "invalid_presence_region");

  const semanticsGuard = guardExecutiveStrategicPresenceSemantics({
    headline: "Manipulative engagement via addictive awareness loops and hidden behavioral tracking",
    summary: "Presence review",
  });
  assert.equal(semanticsGuard.ok, false);
  if (semanticsGuard.ok) return;
  assert.equal(semanticsGuard.code, "manipulative_engagement_system");
});

test("immutable presence state preservation", () => {
  const stack = buildPresenceStack();
  const frozenImmersion = JSON.stringify(stack.immersion);
  evaluateExecutiveStrategicPresence(presenceInput(stack));
  assert.equal(JSON.stringify(stack.immersion), frozenImmersion);
});

test("executive-readable presence semantics", () => {
  const stack = buildPresenceStack();
  const result = evaluateExecutiveStrategicPresence({ ...presenceInput(stack), tick: 8 });
  assert.ok(result.ok);
  if (!result.ok) return;
  assert.match(
    result.snapshot.semantics.headline,
    /operational|recovery|coordination|fragility|awareness|presence|logistics|manufacturing|governance|predictive|stability|intelligence|equilibrium|resilience|volatility|continuity|strategic|situational/i
  );
  assert.ok(
    !result.snapshot.semantics.headline.includes("Presence recursion continuity exceeded")
  );
  assert.equal(result.snapshot.state.presenceAmbiguityDisclaimer, PRESENCE_AMBIGUITY_DISCLAIMER);
  assert.equal(
    result.snapshot.state.nonManipulationPresenceDisclaimer,
    NON_MANIPULATION_PRESENCE_DISCLAIMER
  );

  const manual = buildExecutiveStrategicPresenceSemantics({ state: result.snapshot.state });
  assert.ok(manual.summary.includes("Indicative"));
});

test("integrated presence panel contract", () => {
  const stack = buildPresenceStack();
  const result = evaluateExecutiveStrategicPresence(presenceInput(stack));
  assert.ok(result.ok);
  if (!result.ok) return;
  assert.equal(result.panelContract.topologyId, stack.topology.topologyId);
  assert.equal(result.panelContract.presenceAmbiguityDisclaimer, PRESENCE_AMBIGUITY_DISCLAIMER);
  assert.equal(
    result.panelContract.nonManipulationPresenceDisclaimer,
    NON_MANIPULATION_PRESENCE_DISCLAIMER
  );
  assert.ok(result.panelContract.presenceSignals.length > 0);
});

test("rejects duplicate presence build fingerprint", () => {
  const stack = buildPresenceStack();
  const first = evaluateExecutiveStrategicPresence({ ...presenceInput(stack), tick: 0 });
  assert.ok(first.ok);
  if (!first.ok) return;
  const fp = buildPresenceContentFingerprint({
  ...
  const second = evaluateExecutiveStrategicPresence({
    ...presenceInput(stack),
    tick: 0,
    priorPresenceFingerprints: [fp],
  });
  assert.equal(second.ok, false);
  if (second.ok) return;
  assert.equal(second.guard.code, "duplicate_presence_build");
});

test("presence classification", () => {
  const label = classifyExecutivePresenceLabel({
    presenceSignals: [
      {
        presenceId: "p1",
        affectedRegionIds: ["logistics"],
        presenceState: "aware",
        presenceStrength: 0.6,
      },
    ],
    situationalContinuityScore: 0.6,
    multiLayerAwarenessScore: 0.35,
    presenceFragmentationScore: 0.3,
  });
  assert.equal(label, "aware");
});
