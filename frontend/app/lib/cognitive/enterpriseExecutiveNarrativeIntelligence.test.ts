/**
 * D7:6:5 — Executive narrative intelligence tests.
 */
...
import { analyzeNarrativeCoherence } from "./narrativeCoherenceAnalysis.ts";
...
function buildTopology() {
  ...
  const built = buildOperationalUniverseTopology({ topologyId: "topo-narrative", objects });
  assert.ok(built.ok);
...
function narrativeInput(stack: ReturnType<typeof buildNarrativeStack>) {
  return {
...
test("deterministic narrative synthesis", () => {
  const stack = buildNarrativeStack();
  const r1 = evaluateExecutiveNarratives({
    ...narrativeInput(stack),
    narrativeContext: { narrativeLeverageFactor: 0.1 },
  });
  const r2 = evaluateExecutiveNarratives({
    ...narrativeInput(stack),
    narrativeContext: { narrativeLeverageFactor: 0.1 },
  });
...
test("strategic-context modeling", () => {
  const stack = buildNarrativeStack();
  const narratives = deriveExecutiveNarrativeSignals({
    insightPrioritizationState: stack.insightPrioritization,
    cognitiveLoadState: stack.cognitiveLoad,
    advisoryState: stack.advisory,
    explainabilityState: stack.explainability,
    ...
    recoveryOpportunityState: stack.recoveryOpportunity,
    resilienceState: stack.resilience,
    narrativeLeverageFactor: 0.1,
  });
  const context = analyzeStrategicContext({
  ...
  const clarity = calculateNarrativeClarityScore({
    narrativeSignals: narratives,
    explainabilityState: stack.explainability,
    advisoryState: stack.advisory,
  });
  ...
  for (const n of narratives) {
    assert.ok(n.narrativeStrength <= 0.92);
  }
});

test("narrative-coherence consistency validation", () => {
  const stack = buildNarrativeStack();
  const narratives = deriveExecutiveNarrativeSignals({
    insightPrioritizationState: stack.insightPrioritization,
    cognitiveLoadState: stack.cognitiveLoad,
  ...
  const c1 = analyzeNarrativeCoherence({
    narrativeSignals: narratives,
    insightPrioritizationState: stack.insightPrioritization,
    cognitiveLoadState: stack.cognitiveLoad,
    explainabilityState: stack.explainability,
  ...
  const c2 = analyzeNarrativeCoherence({
    narrativeSignals: narratives,
    insightPrioritizationState: stack.insightPrioritization,
    cognitiveLoadState: stack.cognitiveLoad,
    explainabilityState: stack.explainability,
  ...
  const u1 = analyzeExecutiveUnderstanding({
    narrativeSignals: narratives,
  ...
  const u2 = analyzeExecutiveUnderstanding({
    narrativeSignals: narratives,
    contextRecords: context,
    coherenceRecords: c2,
    trajectoryState: stack.trajectory,
...
test("executive understanding testing", () => {
  const stack = buildNarrativeStack();
  const result = evaluateExecutiveNarratives(narrativeInput(stack));
  assert.ok(result.ok);
  if (!result.ok) return;
  assert.ok(result.snapshot.state.executiveUnderstandingRecords.length >= 6);
  assert.ok(result.snapshot.state.strategicContextRecords.length > 0);
});

test("replay-safe narrative snapshots", () => {
  const stack = buildNarrativeStack();
  const result = evaluateExecutiveNarratives(narrativeInput(stack));
  assert.ok(result.ok);
  if (!result.ok) return;
  const frozen = freezeExecutiveNarrativeSnapshot(result.snapshot);
  assert.throws(() => {
    (frozen.state as { narrativeClarityScore: number }).narrativeClarityScore = 0;
  });
});

test("governance guard rail enforcement", () => {
  assert.ok(containsFalseCertaintyText("guaranteed narrative outcome"));
  const guard = guardEvaluateExecutiveNarratives({
    topologyId: "topo",
    regionIds: ["finance"],
    narrativeSignals: [
      {
        narrativeId: "narrative::bad",
  ...
  if (guard.ok) return;
  assert.equal(guard.code, "invalid_narrative_region");

  const semanticsGuard = guardExecutiveNarrativeSemantics({
    headline: "Manipulative storytelling via emotional persuasion and fabricated narrative claims",
    summary: "Narrative review",
  });
  assert.equal(semanticsGuard.ok, false);
  if (semanticsGuard.ok) return;
  assert.equal(semanticsGuard.code, "manipulative_storytelling");
});

test("immutable narrative state preservation", () => {
  const stack = buildNarrativeStack();
  const frozenInsight = JSON.stringify(stack.insightPrioritization);
  evaluateExecutiveNarratives(narrativeInput(stack));
  assert.equal(JSON.stringify(stack.insightPrioritization), frozenInsight);
});

test("executive-readable narrative semantics", () => {
  const stack = buildNarrativeStack();
  const result = evaluateExecutiveNarratives({ ...narrativeInput(stack), tick: 8 });
  assert.ok(result.ok);
  if (!result.ok) return;
  assert.match(
    result.snapshot.semantics.headline,
    /operational|recovery|coordination|fragility|narrative|logistics|manufacturing|governance|predictive|stability|intelligence|equilibrium|resilience|volatility|dependency|synchronization/i
  );
  assert.ok(
    !result.snapshot.semantics.headline.includes("Narrative recursion synthesis exceeded")
  );
  assert.equal(result.snapshot.state.narrativeAmbiguityDisclaimer, NARRATIVE_AMBIGUITY_DISCLAIMER);
  assert.equal(
    result.snapshot.state.nonManipulationNarrativeDisclaimer,
    NON_MANIPULATION_NARRATIVE_DISCLAIMER
  );

  const manual = buildExecutiveNarrativeSemantics({ state: result.snapshot.state });
  assert.ok(manual.summary.includes("Indicative"));
});

test("integrated narrative panel contract", () => {
  const stack = buildNarrativeStack();
  const result = evaluateExecutiveNarratives(narrativeInput(stack));
  assert.ok(result.ok);
  if (!result.ok) return;
  assert.equal(result.panelContract.topologyId, stack.topology.topologyId);
  assert.equal(result.panelContract.narrativeAmbiguityDisclaimer, NARRATIVE_AMBIGUITY_DISCLAIMER);
  assert.equal(
    result.panelContract.nonManipulationNarrativeDisclaimer,
    NON_MANIPULATION_NARRATIVE_DISCLAIMER
  );
  assert.ok(result.panelContract.narrativeSignals.length > 0);
});

test("rejects duplicate narrative build fingerprint", () => {
  const stack = buildNarrativeStack();
  const first = evaluateExecutiveNarratives({ ...narrativeInput(stack), tick: 0 });
  assert.ok(first.ok);
  if (!first.ok) return;
  const fp = buildNarrativeContentFingerprint({
  ...
  const second = evaluateExecutiveNarratives({
    ...narrativeInput(stack),
    tick: 0,
    priorNarrativeFingerprints: [fp],
  });
  assert.equal(second.ok, false);
  if (second.ok) return;
  assert.equal(second.guard.code, "duplicate_narrative_build");
});

test("narrative classification", () => {
  const label = classifyExecutiveNarrativeLabel({
    narrativeSignals: [
      {
        narrativeId: "n1",
        affectedRegionIds: ["logistics"],
        narrativeState: "clear",
        narrativeStrength: 0.6,
      },
    ],
    narrativeClarityScore: 0.7,
    strategicContextScore: 0.35,
    narrativeFragmentationScore: 0.3,
  });
