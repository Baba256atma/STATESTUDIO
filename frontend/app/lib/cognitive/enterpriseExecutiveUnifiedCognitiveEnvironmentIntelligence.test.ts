/**
 * D7:6:9 — Unified executive cognitive environment intelligence tests.
 */
...
import { analyzeCognitiveEnvironmentFragmentation } from "./cognitiveEnvironmentFragmentationAnalysis.ts";
...
function buildTopology() {
  ...
  const built = buildOperationalUniverseTopology({ topologyId: "topo-environment", objects });
  assert.ok(built.ok);
...
function buildEnvironmentStack() {
...
function environmentInput(stack: ReturnType<typeof buildEnvironmentStack>) {
  return {
...
test("deterministic environment orchestration", () => {
  const stack = buildEnvironmentStack();
  const r1 = evaluateUnifiedExecutiveEnvironment({
    ...environmentInput(stack),
    environmentContext: { environmentLeverageFactor: 0.1 },
  });
  const r2 = evaluateUnifiedExecutiveEnvironment({
    ...environmentInput(stack),
    environmentContext: { environmentLeverageFactor: 0.1 },
  });
...
test("cross-cognitive synchronization modeling", () => {
  const stack = buildEnvironmentStack();
  const environments = deriveUnifiedExecutiveEnvironmentSignals({
    presenceState: stack.presence,
    immersionState: stack.immersion,
    timelineState: stack.timeline,
    narrativeState: stack.narrative,
    ...
    recoveryOpportunityState: stack.recoveryOpportunity,
    resilienceState: stack.resilience,
    environmentLeverageFactor: 0.1,
  });
  const syncRecords = analyzeCrossCognitiveSynchronization({
    environmentSignals: environments,
    presenceState: stack.presence,
    immersionState: stack.immersion,
    timelineState: stack.timeline,
    narrativeState: stack.narrative,
  ...
  const coherence = calculateEnvironmentCoherenceScore({
    environmentSignals: environments,
    presenceState: stack.presence,
    immersionState: stack.immersion,
    timelineState: stack.timeline,
  ...
  for (const e of environments) {
    assert.ok(e.environmentStrength <= 0.92);
  }
});

test("continuity consistency validation", () => {
  const stack = buildEnvironmentStack();
  const environments = deriveUnifiedExecutiveEnvironmentSignals({
    presenceState: stack.presence,
    immersionState: stack.immersion,
  ...
  const f1 = analyzeCognitiveEnvironmentFragmentation({
    environmentSignals: environments,
    presenceState: stack.presence,
    immersionState: stack.immersion,
    timelineState: stack.timeline,
  ...
  const f2 = analyzeCognitiveEnvironmentFragmentation({
    environmentSignals: environments,
    presenceState: stack.presence,
    immersionState: stack.immersion,
    timelineState: stack.timeline,
  ...
  const c1 = analyzeExecutiveEnvironmentContinuity({
    environmentSignals: environments,
    syncRecords,
  ...
  const c2 = analyzeExecutiveEnvironmentContinuity({
    environmentSignals: environments,
    syncRecords,
...
test("unified executive cognition testing", () => {
  const stack = buildEnvironmentStack();
  const result = evaluateUnifiedExecutiveEnvironment(environmentInput(stack));
  assert.ok(result.ok);
  if (!result.ok) return;
  assert.ok(result.snapshot.state.executiveEnvironmentContinuityRecords.length >= 6);
  assert.ok(result.snapshot.state.crossCognitiveSynchronizationRecords.length > 0);
});

test("replay-safe environment snapshots", () => {
  const stack = buildEnvironmentStack();
  const result = evaluateUnifiedExecutiveEnvironment(environmentInput(stack));
  assert.ok(result.ok);
  if (!result.ok) return;
  const frozen = freezeUnifiedExecutiveCognitiveEnvironmentSnapshot(result.snapshot);
  assert.throws(() => {
    (frozen.state as { environmentCoherenceScore: number }).environmentCoherenceScore = 0;
  });
});

test("governance guard rail enforcement", () => {
  assert.ok(containsFalseCertaintyText("guaranteed environment outcome"));
  const guard = guardEvaluateUnifiedExecutiveEnvironment({
    topologyId: "topo",
    regionIds: ["finance"],
    environmentSignals: [
      {
        environmentId: "environment::bad",
  ...
  if (guard.ok) return;
  assert.equal(guard.code, "invalid_environment_region");

  const semanticsGuard = guardUnifiedExecutiveCognitiveEnvironmentSemantics({
    headline:
      "Manipulative cognition environment via addictive executive loops and hidden psychological orchestration",
    summary: "Environment review",
  });
  assert.equal(semanticsGuard.ok, false);
  if (semanticsGuard.ok) return;
  assert.equal(semanticsGuard.code, "manipulative_cognition_environment");
});

test("immutable environment state preservation", () => {
  const stack = buildEnvironmentStack();
  const frozenPresence = JSON.stringify(stack.presence);
  evaluateUnifiedExecutiveEnvironment(environmentInput(stack));
  assert.equal(JSON.stringify(stack.presence), frozenPresence);
});

test("executive-readable environment semantics", () => {
  const stack = buildEnvironmentStack();
  const result = evaluateUnifiedExecutiveEnvironment({ ...environmentInput(stack), tick: 8 });
  assert.ok(result.ok);
  if (!result.ok) return;
  assert.match(
    result.snapshot.semantics.headline,
    /operational|recovery|coordination|fragility|awareness|cognitive|environment|logistics|manufacturing|governance|predictive|stability|intelligence|equilibrium|resilience|volatility|continuity|strategic|synchronized|synchronization/i
  );
  assert.ok(
    !result.snapshot.semantics.headline.includes("Environment recursion coherence exceeded")
  );
  assert.equal(
    result.snapshot.state.environmentAmbiguityDisclaimer,
    ENVIRONMENT_AMBIGUITY_DISCLAIMER
  );
  assert.equal(
    result.snapshot.state.nonManipulationEnvironmentDisclaimer,
    NON_MANIPULATION_ENVIRONMENT_DISCLAIMER
  );

  const manual = buildUnifiedExecutiveCognitiveEnvironmentSemantics({
    state: result.snapshot.state,
  });
  assert.ok(manual.summary.includes("Indicative"));
});

test("integrated environment panel contract", () => {
  const stack = buildEnvironmentStack();
  const result = evaluateUnifiedExecutiveEnvironment(environmentInput(stack));
  assert.ok(result.ok);
  if (!result.ok) return;
  ...
  assert.equal(
    result.panelContract.environmentAmbiguityDisclaimer,
    ENVIRONMENT_AMBIGUITY_DISCLAIMER
  );
  assert.equal(
    result.panelContract.nonManipulationEnvironmentDisclaimer,
    NON_MANIPULATION_ENVIRONMENT_DISCLAIMER
  );
  assert.ok(result.panelContract.environmentSignals.length > 0);
});

test("rejects duplicate environment build fingerprint", () => {
  const stack = buildEnvironmentStack();
  const first = evaluateUnifiedExecutiveEnvironment({ ...environmentInput(stack), tick: 0 });
  assert.ok(first.ok);
  if (!first.ok) return;
  const fp = buildEnvironmentContentFingerprint({
  ...
  const second = evaluateUnifiedExecutiveEnvironment({
    ...environmentInput(stack),
    tick: 0,
    priorEnvironmentFingerprints: [fp],
  });
  assert.equal(second.ok, false);
  if (second.ok) return;
  assert.equal(second.guard.code, "duplicate_environment_build");
});

test("environment classification", () => {
  const label = classifyExecutiveEnvironmentLabel({
    environmentSignals: [
      {
        environmentId: "e1",
        affectedRegionIds: ["logistics"],
        environmentState: "coherent",
        environmentStrength: 0.6,
      },
    ],
    environmentCoherenceScore: 0.6,
    crossCognitiveSyncScore: 0.6,
    environmentFragmentationScore: 0.3,
  });
