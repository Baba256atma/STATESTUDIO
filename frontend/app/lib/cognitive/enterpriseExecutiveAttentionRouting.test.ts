/**
 * D7:6:2 — Executive attention routing intelligence tests.
 */
...
import { analyzeAttentionFragmentation } from "./attentionFragmentationAnalysis.ts";
...
function buildTopology() {
  ...
  const built = buildOperationalUniverseTopology({ topologyId: "topo-attention-routing", objects });
  assert.ok(built.ok);
...
function buildAttentionRoutingStack() {
...
function attentionRoutingInput(stack: ReturnType<typeof buildAttentionRoutingStack>) {
  return {
...
test("deterministic attention routing", () => {
  const stack = buildAttentionRoutingStack();
  const r1 = evaluateExecutiveAttentionRouting({
    ...attentionRoutingInput(stack),
    attentionRoutingContext: { routingLeverageFactor: 0.1 },
  });
  const r2 = evaluateExecutiveAttentionRouting({
    ...attentionRoutingInput(stack),
    attentionRoutingContext: { routingLeverageFactor: 0.1 },
  });
...
test("dynamic priority-flow modeling", () => {
  const stack = buildAttentionRoutingStack();
  const routes = deriveExecutiveAttentionRoutingSignals({
    cognitiveUxState: stack.cognitiveUx,
    orchestrationState: stack.orchestration,
    consensusState: stack.consensus,
    advisoryState: stack.advisory,
    ...
    trajectoryState: stack.trajectory,
    cascadeState: stack.cascade,
    routingLeverageFactor: 0.1,
  });
  const flow = analyzeDynamicPriorityFlow({
    routingSignals: routes,
    cognitiveUxState: stack.cognitiveUx,
    orchestrationState: stack.orchestration,
    consensusState: stack.consensus,
    governanceState: stack.governance,
  ...
  const stability = calculateFocusStabilityScore({
    routingSignals: routes,
    cognitiveUxState: stack.cognitiveUx,
    orchestrationState: stack.orchestration,
  });
  ...
  for (const r of routes) {
    assert.ok(r.routingStrength <= 0.92);
  }
});

test("attention-fragmentation consistency validation", () => {
  const stack = buildAttentionRoutingStack();
  const routes = deriveExecutiveAttentionRoutingSignals({
    cognitiveUxState: stack.cognitiveUx,
    orchestrationState: stack.orchestration,
  ...
  const f1 = analyzeAttentionFragmentation({
    routingSignals: routes,
    cognitiveUxState: stack.cognitiveUx,
    orchestrationState: stack.orchestration,
    advisoryState: stack.advisory,
  ...
  const f2 = analyzeAttentionFragmentation({
    routingSignals: routes,
    cognitiveUxState: stack.cognitiveUx,
    orchestrationState: stack.orchestration,
    advisoryState: stack.advisory,
  ...
  const focus1 = analyzeExecutiveFocusOrchestration({
    routingSignals: routes,
    flowRecords: flow,
  ...
  const focus2 = analyzeExecutiveFocusOrchestration({
    routingSignals: routes,
    flowRecords: flow,
  ...
  assert.equal(
    focus1.map((r) => r.recordId).join("|"),
    focus2.map((r) => r.recordId).join("|")
  );
});

test("executive focus testing", () => {
  const stack = buildAttentionRoutingStack();
  const result = evaluateExecutiveAttentionRouting(attentionRoutingInput(stack));
  assert.ok(result.ok);
  if (!result.ok) return;
  assert.ok(result.snapshot.state.executiveFocusOrchestrationRecords.length >= 6);
  assert.ok(result.snapshot.state.dynamicPriorityFlowRecords.length > 0);
});

test("replay-safe routing snapshots", () => {
  const stack = buildAttentionRoutingStack();
  const result = evaluateExecutiveAttentionRouting(attentionRoutingInput(stack));
  assert.ok(result.ok);
  if (!result.ok) return;
  const frozen = freezeExecutiveAttentionRoutingSnapshot(result.snapshot);
  assert.throws(() => {
    (frozen.state as { focusStabilityScore: number }).focusStabilityScore = 0;
  });
});

test("governance guard rail enforcement", () => {
  assert.ok(containsFalseCertaintyText("guaranteed routing outcome"));
  const guard = guardEvaluateExecutiveAttentionRouting({
    topologyId: "topo",
    regionIds: ["finance"],
    routingSignals: [
      {
        routingId: "routing::bad",
  ...
  if (guard.ok) return;
  assert.equal(guard.code, "invalid_routing_region");

  const semanticsGuard = guardAttentionRoutingExecutiveSemantics({
    headline: "Cognitive manipulation via dark pattern urgency will force attention",
    summary: "Attention routing review",
  });
...
test("immutable routing state preservation", () => {
  const stack = buildAttentionRoutingStack();
  const frozenCognitiveUx = JSON.stringify(stack.cognitiveUx);
  evaluateExecutiveAttentionRouting(attentionRoutingInput(stack));
  assert.equal(JSON.stringify(stack.cognitiveUx), frozenCognitiveUx);
});

test("executive-readable routing semantics", () => {
  const stack = buildAttentionRoutingStack();
  const result = evaluateExecutiveAttentionRouting({ ...attentionRoutingInput(stack), tick: 8 });
  assert.ok(result.ok);
  if (!result.ok) return;
  assert.match(
    result.snapshot.semantics.headline,
    /operational|recovery|coordination|fragility|attention|routing|divergence|logistics|manufacturing|governance|stabilization|priority|intelligence/i
  );
  assert.ok(
    !result.snapshot.semantics.headline.includes("Attention recursion routing exceeded")
  );
  assert.equal(result.snapshot.state.routingAmbiguityDisclaimer, ROUTING_AMBIGUITY_DISCLAIMER);
  assert.equal(
    result.snapshot.state.nonManipulationRoutingDisclaimer,
    NON_MANIPULATION_ROUTING_DISCLAIMER
  );

  const manual = buildExecutiveAttentionRoutingSemantics({ state: result.snapshot.state });
  assert.ok(manual.summary.includes("Indicative"));
});

test("integrated attention routing panel contract", () => {
  const stack = buildAttentionRoutingStack();
  const result = evaluateExecutiveAttentionRouting(attentionRoutingInput(stack));
  assert.ok(result.ok);
  if (!result.ok) return;
  assert.equal(result.panelContract.topologyId, stack.topology.topologyId);
  assert.equal(result.panelContract.routingAmbiguityDisclaimer, ROUTING_AMBIGUITY_DISCLAIMER);
  assert.equal(
    result.panelContract.nonManipulationRoutingDisclaimer,
    NON_MANIPULATION_ROUTING_DISCLAIMER
  );
  assert.ok(result.panelContract.attentionRoutes.length > 0);
});

test("rejects duplicate routing build fingerprint", () => {
  const stack = buildAttentionRoutingStack();
  const first = evaluateExecutiveAttentionRouting({ ...attentionRoutingInput(stack), tick: 0 });
  assert.ok(first.ok);
  if (!first.ok) return;
  const fp = buildAttentionRoutingContentFingerprint({
  ...
  const second = evaluateExecutiveAttentionRouting({
    ...attentionRoutingInput(stack),
    tick: 0,
    priorAttentionRoutingFingerprints: [fp],
  });
  assert.equal(second.ok, false);
  if (second.ok) return;
  assert.equal(second.guard.code, "duplicate_routing_build");
});

test("attention routing classification", () => {
  const label = classifyExecutiveAttentionRoutingLabel({
    routingSignals: [
      {
        routingId: "r1",
    ...
    focusStabilityScore: 0.7,
    strategicUrgencyScore: 0.4,
    attentionFragmentationScore: 0.3,
  });
