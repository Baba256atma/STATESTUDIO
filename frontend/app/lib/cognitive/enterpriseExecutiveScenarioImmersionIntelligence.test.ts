/**
 * D7:6:7 — Executive scenario immersion intelligence tests.
 */
...
import { analyzeImmersiveCognition } from "./immersiveCognitionAnalysis.ts";
...
function buildTopology() {
  ...
  const built = buildOperationalUniverseTopology({ topologyId: "topo-immersion", objects });
  assert.ok(built.ok);
...
function immersionInput(stack: ReturnType<typeof buildImmersionStack>) {
  return {
...
test("deterministic immersion orchestration", () => {
  const stack = buildImmersionStack();
  const r1 = evaluateExecutiveScenarioImmersion({
    ...immersionInput(stack),
    immersionContext: { immersionLeverageFactor: 0.1 },
  });
  const r2 = evaluateExecutiveScenarioImmersion({
    ...immersionInput(stack),
    immersionContext: { immersionLeverageFactor: 0.1 },
  });
...
test("multi-layer scenario modeling", () => {
  const stack = buildImmersionStack();
  const immersions = deriveExecutiveScenarioImmersionSignals({
    timelineState: stack.timeline,
    narrativeState: stack.narrative,
    insightPrioritizationState: stack.insightPrioritization,
    foresightState: stack.foresight,
    ...
    recoveryOpportunityState: stack.recoveryOpportunity,
    resilienceState: stack.resilience,
    immersionLeverageFactor: 0.1,
  });
  const layers = analyzeMultiLayerScenarios({
    immersionSignals: immersions,
    timelineState: stack.timeline,
    narrativeState: stack.narrative,
    foresightState: stack.foresight,
    cognitiveLoadState: stack.cognitiveLoad,
  ...
  const clarity = calculateImmersionClarityScore({
    immersionSignals: immersions,
    narrativeState: stack.narrative,
    timelineState: stack.timeline,
  });
  assert.ok(immersions.length > 0);
  assert.ok(layers.length > 0);
  assert.ok(clarity >= 0 && clarity <= 1);
  for (const i of immersions) {
    assert.ok(i.immersionStrength <= 0.92);
  }
});

test("scenario-coherence consistency validation", () => {
  const stack = buildImmersionStack();
  const immersions = deriveExecutiveScenarioImmersionSignals({
    timelineState: stack.timeline,
    narrativeState: stack.narrative,
  ...
  const c1 = analyzeImmersiveCognition({
    immersionSignals: immersions,
    timelineState: stack.timeline,
    narrativeState: stack.narrative,
    cognitiveLoadState: stack.cognitiveLoad,
  ...
  const c2 = analyzeImmersiveCognition({
    immersionSignals: immersions,
    timelineState: stack.timeline,
    narrativeState: stack.narrative,
    cognitiveLoadState: stack.cognitiveLoad,
  ...
  const e1 = analyzeExecutiveScenarioExploration({
    immersionSignals: immersions,
    layerRecords: layers,
    cognitionRecords: c1,
    trajectoryState: stack.trajectory,
  ...
  const e2 = analyzeExecutiveScenarioExploration({
    immersionSignals: immersions,
    layerRecords: layers,
    cognitionRecords: c2,
    trajectoryState: stack.trajectory,
...
test("executive exploration testing", () => {
  const stack = buildImmersionStack();
  const result = evaluateExecutiveScenarioImmersion(immersionInput(stack));
  assert.ok(result.ok);
  if (!result.ok) return;
  assert.ok(result.snapshot.state.executiveScenarioExplorationRecords.length >= 6);
  assert.ok(result.snapshot.state.scenarioEvolutionLayerRecords.length > 0);
});

test("replay-safe immersion snapshots", () => {
  const stack = buildImmersionStack();
  const result = evaluateExecutiveScenarioImmersion(immersionInput(stack));
  assert.ok(result.ok);
  if (!result.ok) return;
  const frozen = freezeExecutiveScenarioImmersionSnapshot(result.snapshot);
  assert.throws(() => {
    (frozen.state as { immersionClarityScore: number }).immersionClarityScore = 0;
  });
});

test("governance guard rail enforcement", () => {
  assert.ok(containsFalseCertaintyText("guaranteed immersion outcome"));
  const guard = guardEvaluateExecutiveScenarioImmersion({
    topologyId: "topo",
    regionIds: ["finance"],
    immersionSignals: [
      {
        immersionId: "immersion::bad",
        affectedRegionIds: ["unknown"],
        immersionState: "observational",
        immersionStrength: 0.5,
      },
    ],
  });
  assert.equal(guard.ok, false);
  if (guard.ok) return;
  assert.equal(guard.code, "invalid_immersion_region");

  const semanticsGuard = guardExecutiveScenarioImmersionSemantics({
    headline: "Manipulative emotional immersion via addictive scenario systems and fabricated operational future",
    summary: "Immersion review",
  });
  assert.equal(semanticsGuard.ok, false);
  if (semanticsGuard.ok) return;
  assert.equal(semanticsGuard.code, "manipulative_emotional_immersion");
});

test("immutable immersion state preservation", () => {
  const stack = buildImmersionStack();
  const frozenTimeline = JSON.stringify(stack.timeline);
  evaluateExecutiveScenarioImmersion(immersionInput(stack));
  assert.equal(JSON.stringify(stack.timeline), frozenTimeline);
});

test("executive-readable immersion semantics", () => {
  const stack = buildImmersionStack();
  const result = evaluateExecutiveScenarioImmersion({ ...immersionInput(stack), tick: 8 });
  assert.ok(result.ok);
  if (!result.ok) return;
  assert.match(
    result.snapshot.semantics.headline,
    /operational|recovery|coordination|fragility|scenario|immersion|logistics|manufacturing|governance|predictive|stability|intelligence|equilibrium|resilience|volatility|dependency|horizon|exploration/i
  );
  assert.ok(
    !result.snapshot.semantics.headline.includes("Immersion recursion exceeded")
  );
  assert.equal(result.snapshot.state.immersionAmbiguityDisclaimer, IMMERSION_AMBIGUITY_DISCLAIMER);
  assert.equal(
    result.snapshot.state.nonManipulationImmersionDisclaimer,
    NON_MANIPULATION_IMMERSION_DISCLAIMER
  );

  const manual = buildExecutiveScenarioImmersionSemantics({ state: result.snapshot.state });
  assert.ok(manual.summary.includes("Indicative"));
});

test("integrated immersion panel contract", () => {
  const stack = buildImmersionStack();
  const result = evaluateExecutiveScenarioImmersion(immersionInput(stack));
  assert.ok(result.ok);
  if (!result.ok) return;
  assert.equal(result.panelContract.topologyId, stack.topology.topologyId);
  assert.equal(result.panelContract.immersionAmbiguityDisclaimer, IMMERSION_AMBIGUITY_DISCLAIMER);
  assert.equal(
    result.panelContract.nonManipulationImmersionDisclaimer,
    NON_MANIPULATION_IMMERSION_DISCLAIMER
  );
  assert.ok(result.panelContract.immersionSignals.length > 0);
});

test("rejects duplicate immersion build fingerprint", () => {
  const stack = buildImmersionStack();
  const first = evaluateExecutiveScenarioImmersion({ ...immersionInput(stack), tick: 0 });
  assert.ok(first.ok);
  if (!first.ok) return;
  const fp = buildImmersionContentFingerprint({
  ...
  const second = evaluateExecutiveScenarioImmersion({
    ...immersionInput(stack),
    tick: 0,
    priorImmersionFingerprints: [fp],
  });
  assert.equal(second.ok, false);
  if (second.ok) return;
  assert.equal(second.guard.code, "duplicate_immersion_build");
});

test("immersion classification", () => {
  const label = classifyExecutiveImmersionLabel({
    immersionSignals: [
      {
        immersionId: "i1",
        affectedRegionIds: ["logistics"],
        immersionState: "observational",
        immersionStrength: 0.6,
      },
    ],
    immersionClarityScore: 0.6,
    multiLayerScenarioScore: 0.35,
    immersionOverloadScore: 0.3,
  });
  assert.equal(label, "observational");
});
