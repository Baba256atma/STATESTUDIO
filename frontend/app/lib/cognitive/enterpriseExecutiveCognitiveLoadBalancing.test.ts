/**
 * D7:6:3 — Executive cognitive load balancing intelligence tests.
 */
...
import { analyzeOverloadDistribution } from "./overloadDistributionAnalysis.ts";
...
function buildTopology() {
  ...
  const built = buildOperationalUniverseTopology({ topologyId: "topo-cognitive-load", objects });
  assert.ok(built.ok);
...
function buildCognitiveLoadStack() {
...
function cognitiveLoadInput(stack: ReturnType<typeof buildCognitiveLoadStack>) {
  return {
...
test("deterministic cognitive-load balancing", () => {
  const stack = buildCognitiveLoadStack();
  const r1 = evaluateExecutiveCognitiveLoad({
    ...cognitiveLoadInput(stack),
    cognitiveLoadContext: { loadLeverageFactor: 0.1 },
  });
  const r2 = evaluateExecutiveCognitiveLoad({
    ...cognitiveLoadInput(stack),
    cognitiveLoadContext: { loadLeverageFactor: 0.1 },
  });
...
test("signal-density modeling", () => {
  const stack = buildCognitiveLoadStack();
  const loads = deriveExecutiveCognitiveLoadSignals({
    attentionRoutingState: stack.attentionRouting,
    cognitiveUxState: stack.cognitiveUx,
    orchestrationState: stack.orchestration,
    advisoryState: stack.advisory,
    ...
    trajectoryState: stack.trajectory,
    cascadeState: stack.cascade,
    loadLeverageFactor: 0.1,
  });
  const density = analyzeSignalDensity({
    loadSignals: loads,
    attentionRoutingState: stack.attentionRouting,
    cognitiveUxState: stack.cognitiveUx,
    orchestrationState: stack.orchestration,
  ...
  const balance = calculateCognitiveBalanceScore({
    loadSignals: loads,
    cognitiveUxState: stack.cognitiveUx,
    attentionRoutingState: stack.attentionRouting,
  });
  ...
  for (const l of loads) {
    assert.ok(l.loadStrength <= 0.92);
  }
});

test("overload-distribution consistency validation", () => {
  const stack = buildCognitiveLoadStack();
  const loads = deriveExecutiveCognitiveLoadSignals({
    attentionRoutingState: stack.attentionRouting,
    cognitiveUxState: stack.cognitiveUx,
  ...
  const d1 = analyzeOverloadDistribution({
    loadSignals: loads,
    cognitiveUxState: stack.cognitiveUx,
    attentionRoutingState: stack.attentionRouting,
    orchestrationState: stack.orchestration,
  ...
  const d2 = analyzeOverloadDistribution({
    loadSignals: loads,
    cognitiveUxState: stack.cognitiveUx,
    attentionRoutingState: stack.attentionRouting,
    orchestrationState: stack.orchestration,
  ...
  const s1 = analyzeExecutiveStability({
    loadSignals: loads,
    densityRecords: density,
    distributionRecords: d1,
    trajectoryState: stack.trajectory,
    momentumState: stack.momentum,
    equilibriumState: stack.equilibrium,
    divergenceState: stack.divergence,
  });
  const s2 = analyzeExecutiveStability({
    loadSignals: loads,
    densityRecords: density,
    distributionRecords: d2,
    trajectoryState: stack.trajectory,
    momentumState: stack.momentum,
    equilibriumState: stack.equilibrium,
    divergenceState: stack.divergence,
...
test("executive stability testing", () => {
  const stack = buildCognitiveLoadStack();
  const result = evaluateExecutiveCognitiveLoad(cognitiveLoadInput(stack));
  assert.ok(result.ok);
  if (!result.ok) return;
  assert.ok(result.snapshot.state.executiveStabilityRecords.length >= 6);
  assert.ok(result.snapshot.state.signalDensityRecords.length > 0);
});

test("replay-safe cognitive-load snapshots", () => {
  const stack = buildCognitiveLoadStack();
  const result = evaluateExecutiveCognitiveLoad(cognitiveLoadInput(stack));
  assert.ok(result.ok);
  if (!result.ok) return;
  const frozen = freezeExecutiveCognitiveLoadBalancingSnapshot(result.snapshot);
  assert.throws(() => {
    (frozen.state as { cognitiveBalanceScore: number }).cognitiveBalanceScore = 0;
  });
});

test("governance guard rail enforcement", () => {
  assert.ok(containsFalseCertaintyText("guaranteed load outcome"));
  ...
  if (guard.ok) return;
  assert.equal(guard.code, "invalid_load_region");

  const semanticsGuard = guardCognitiveLoadExecutiveSemantics({
    headline: "Psychological manipulation via addictive urgency systems",
    summary: "Cognitive load review",
  });
  assert.equal(semanticsGuard.ok, false);
  if (semanticsGuard.ok) return;
  assert.equal(semanticsGuard.code, "psychological_manipulation_detected");
});

test("immutable load state preservation", () => {
  const stack = buildCognitiveLoadStack();
  const frozenRouting = JSON.stringify(stack.attentionRouting);
  evaluateExecutiveCognitiveLoad(cognitiveLoadInput(stack));
  assert.equal(JSON.stringify(stack.attentionRouting), frozenRouting);
});

test("executive-readable cognitive-load semantics", () => {
  const stack = buildCognitiveLoadStack();
  const result = evaluateExecutiveCognitiveLoad({ ...cognitiveLoadInput(stack), tick: 8 });
  assert.ok(result.ok);
  if (!result.ok) return;
  assert.match(
    result.snapshot.semantics.headline,
    /operational|recovery|coordination|fragility|cognitive|load|divergence|logistics|manufacturing|governance|predictive|stability|intelligence|equilibrium/i
  );
  assert.ok(
    !result.snapshot.semantics.headline.includes("Load-balancing recursion exceeded")
  );
  assert.equal(result.snapshot.state.loadAmbiguityDisclaimer, LOAD_AMBIGUITY_DISCLAIMER);
  assert.equal(
    result.snapshot.state.nonManipulationLoadDisclaimer,
    NON_MANIPULATION_LOAD_DISCLAIMER
  );

  const manual = buildExecutiveCognitiveLoadBalancingSemantics({ state: result.snapshot.state });
  assert.ok(manual.summary.includes("Indicative"));
});

test("integrated cognitive-load panel contract", () => {
  const stack = buildCognitiveLoadStack();
  const result = evaluateExecutiveCognitiveLoad(cognitiveLoadInput(stack));
  assert.ok(result.ok);
  if (!result.ok) return;
  assert.equal(result.panelContract.topologyId, stack.topology.topologyId);
  assert.equal(result.panelContract.loadAmbiguityDisclaimer, LOAD_AMBIGUITY_DISCLAIMER);
  assert.equal(
    result.panelContract.nonManipulationLoadDisclaimer,
    NON_MANIPULATION_LOAD_DISCLAIMER
  );
  assert.ok(result.panelContract.loadSignals.length > 0);
});

test("rejects duplicate cognitive-load build fingerprint", () => {
  const stack = buildCognitiveLoadStack();
  const first = evaluateExecutiveCognitiveLoad({ ...cognitiveLoadInput(stack), tick: 0 });
  assert.ok(first.ok);
  if (!first.ok) return;
  const fp = buildCognitiveLoadContentFingerprint({
  ...
  const second = evaluateExecutiveCognitiveLoad({
    ...cognitiveLoadInput(stack),
    tick: 0,
    priorCognitiveLoadFingerprints: [fp],
  });
  assert.equal(second.ok, false);
  if (second.ok) return;
  assert.equal(second.guard.code, "duplicate_load_build");
});

test("cognitive load classification", () => {
  const label = classifyExecutiveCognitiveLoadLabel({
    loadSignals: [
      {
        loadId: "l1",
    ...
    cognitiveBalanceScore: 0.7,
    signalDensityScore: 0.35,
    overloadEscalationScore: 0.3,
  });
