/**
 * NEA-2:8 — Channel Connectors Freeze Extensions.
 *
 * Immutable extension policy metadata for the frozen Channel Connectors.
 * Policy metadata only — no runtime extension behavior.
 *
 * Ownership: owned exclusively by NEA-2:8.
 */

export const CHANNEL_CONNECTOR_FREEZE_ALLOWED_EXTENSIONS = Object.freeze([
...
export const CHANNEL_CONNECTOR_FREEZE_FORBIDDEN_EXTENSIONS = Object.freeze([
  ...
  "Mutation of connector identity registry",
  "Runtime freeze, certification, or validation logic",
  "Runtime connectors, networking, persistence, or AI surfaces",
] as const);

/** Canonical immutable extension policy. */
export const ChannelConnectorFreezeExtensionPolicy = Object.freeze({
  policyId: "NEA-2:8/FreezeExtensionPolicy",
  sourcePhase: "NEA-2:8" as const,
  allowedExtensions: CHANNEL_CONNECTOR_FREEZE_ALLOWED_EXTENSIONS,
  forbiddenExtensions: CHANNEL_CONNECTOR_FREEZE_FORBIDDEN_EXTENSIONS,
  allowedExtensionCount: CHANNEL_CONNECTOR_FREEZE_ALLOWED_EXTENSIONS.length,
  forbiddenExtensionCount:
    CHANNEL_CONNECTOR_FREEZE_FORBIDDEN_EXTENSIONS.length,
  backwardCompatibility: Object.freeze({
    ...
    description:
      "Consumers must treat NEA-2:8 as an immutable baseline; breaking change requires a major-version successor.",
  }),
