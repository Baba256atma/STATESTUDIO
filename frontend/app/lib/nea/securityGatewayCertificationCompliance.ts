/**
 * NEA-4:7 — Security Gateway Certification Compliance.
 *
 * Immutable architectural compliance declarations for Security Gateway.
 * Declarative only. No runtime evaluation.
 *
 * Ownership: owned exclusively by NEA-4:7.
 */

import {
  SecurityGatewayPlatform,
  SecurityGatewayPlatformId,
} from "./securityGatewayPlatform.ts";
import type { SecurityGatewayComplianceDeclaration } from "./securityGatewayCertificationTypes.ts";

const platform = SecurityGatewayPlatform;

const compliance = (
  key: string,
  complianceName: string,
  description: string,
  order: number,
): SecurityGatewayComplianceDeclaration =>
  Object.freeze({
    complianceId: `NEA-4:7/Compliance/${key}`,
    complianceName,
    description,
    compliant: true as const,
    platformReference: `${SecurityGatewayPlatformId}/${key}`,
    executesRuntime: false as const,
    metadataOnly: true as const,
    immutable: true as const,
    deterministicOrder: order,
  });

/** Exactly ten compliance declarations. */
export const SecurityGatewayCertificationComplianceDeclarations: readonly SecurityGatewayComplianceDeclaration[] =
  Object.freeze([
    compliance(
      "PhaseChain",
      "Phase Chain",
      "NEA-4:1 through NEA-4:6 phase chain is composed through Platform namespace.",
      1,
    ),
    compliance(
      "CanonicalReferences",
      "Canonical References",
      "All upstream surfaces are referenced; none are reconstructed.",
      2,
    ),
    compliance(
      "SecurityIdentityRegistry",
      "Security Identity Registry",
      "Security identities are certified through the Registry collections surface.",
      3,
    ),
    compliance(
      "SecurityPolicyRegistry",
      "Security Policy Registry",
      "Security policies are certified through the Registry collections surface.",
      4,
    ),
    compliance(
      "PermissionRegistry",
      "Permission Registry",
      "Permissions are certified through the Registry collections surface.",
      5,
    ),
    compliance(
      "Ownership",
      "Ownership",
      "Ownership remains unique per phase; Platform does not claim upstream ownership.",
      6,
    ),
    compliance(
      "Inventories",
      "Inventories",
      "Inventories are derived from canonical collections without hardcoding.",
      7,
    ),
    compliance(
      "NamespaceComposition",
      "Namespace Composition",
      "Platform namespace includes foundation, registry, model, validation, manifest, and platform.",
      8,
    ),
    compliance(
      "PublicSurface",
      "Public Surface",
      "Each phase exposes a controlled eight-export public surface.",
      9,
    ),
    compliance(
      "DependencyDirection",
      "Dependency Direction",
      "Dependency direction is Certification → Platform → Manifest → Validation → Model → Registry → Foundation.",
      10,
    ),
  ]);

/** Canonical immutable compliance catalog. */
export const SecurityGatewayCertificationComplianceCatalog = Object.freeze({
  catalogId: "NEA-4:7/ComplianceCatalog",
  sourcePhase: "NEA-4:7" as const,
  platformId: SecurityGatewayPlatformId,
  declarations: SecurityGatewayCertificationComplianceDeclarations,
  complianceCount:
    SecurityGatewayCertificationComplianceDeclarations.length,
  allCompliant: SecurityGatewayCertificationComplianceDeclarations.every(
    (item) => item.compliant === true,
  ),
  platformManifestOnly: platform.dependency.manifestOnly,
  platformImmutable: platform.immutable,
  executesRuntime: false as const,
  metadataOnly: true as const,
  immutable: true as const,
  deterministic: true as const,
});
