...
function minimalCognition(org = "drift-org"): AdaptiveGovernanceIntelligenceSnapshot {
  return {
...
function seedFullTemporalStack(
  organizationId: string,
  cognition: AdaptiveGovernanceIntelligenceSnapshot,
  options?: { fragility?: boolean }
) {
  for (let i = 0; i < 3; i += 1) {
    const snapshot = { ...cognition, signature: `drift-seed-${i}` };
...
describe("temporal drift projection D9:3:4", () => {
  beforeEach(() => {
  ...
  it("projects adaptive_growth when resilience recovery patterns dominate", () => {
    const org = "adaptive-growth-org";
    const cognition = minimalCognition(org);
    seedFullTemporalStack(org, cognition, { fragility: false });
    ...
    const result = evaluateTemporalDriftProjection({
      organizationId: org,
      cognitionSnapshot: cognition,
      fragilityElevated: false,
      continuityPreserved: true,
    ...
    const adaptive = getTemporalDriftProjectionStore(org)
      .getState()
      .projections.find((p) => p.trajectoryDirection === "adaptive_growth");
    expect(adaptive).toBeDefined();
    expect(adaptive!.supportingSignals.length).toBeGreaterThan(0);
  });

  it("projects degrading direction when fragility escalates", () => {
    const org = "degrading-org";
    const cognition = minimalCognition(org);
    seedFullTemporalStack(org, cognition, { fragility: true });
    ...
    expect(result.evaluated).toBe(true);
    const degrading = result.snapshot?.recentProjections.find(
  ...
  it("dedupes duplicate drift evaluations on unchanged signature", () => {
    const org = "drift-dedupe-org";
    const cognition = minimalCognition(org);
    seedFullTemporalStack(org, cognition);

    const first = integrateTemporalDriftProjectionWithCognition({
      organizationId: org,
      cognitionSnapshot: cognition,
      fragilityElevated: true,
      now: 10_000,
    });
    const second = integrateTemporalDriftProjectionWithCognition({
      organizationId: org,
      cognitionSnapshot: cognition,
      fragilityElevated: true,
      now: 10_100,
  ...
  it("keeps bounded projection memory under caps", () => {
    const org = "drift-bounded-org";
    const cognition = minimalCognition(org);
    seedFullTemporalStack(org, cognition);

    for (let i = 0; i < 20; i += 1) {
      evaluateTemporalDriftProjection({
        organizationId: org,
        cognitionSnapshot: { ...cognition, signature: `drift-bounded-${i}` },
        fragilityElevated: i % 2 === 0,
        now: 11_000 + i * 600,
      });
    }

    const state = getTemporalDriftProjectionStore(org).getState();
    expect(state.projections.length).toBeLessThanOrEqual(12);
    expect(state.snapshots.length).toBeLessThanOrEqual(8);
  });

  it("blocks recursive drift evaluation", () => {
    expect(beginTemporalDriftEvaluation()).toBe(true);
    expect(beginTemporalDriftEvaluation()).toBe(true);
    expect(beginTemporalDriftEvaluation()).toBe(false);
    endTemporalDriftEvaluation();
    endTemporalDriftEvaluation();
  });

  it("emits trajectory projection contract fields", () => {
    const org = "drift-contract-org";
    const cognition = minimalCognition(org);
    seedFullTemporalStack(org, cognition, { fragility: false });
    ...
    expect(result.evaluated).toBe(true);
    const projection = result.snapshot?.recentProjections[0];
    expect(projection).toBeDefined();
    expect(projection!.projectionId.length).toBeGreaterThan(0);
    expect(projection!.confidence).toBeGreaterThanOrEqual(0.45);
    expect(projection!.generatedAt).toBe(12_000);
  });
});
