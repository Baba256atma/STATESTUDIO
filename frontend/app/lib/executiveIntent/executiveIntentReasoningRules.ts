...
import type { IntentConfidenceResult } from "./executiveIntentConfidenceTypes.ts";
import type { IntentConflictResult } from "./executiveIntentConflictTypes.ts";
import type { IntentDependencyResult } from "./executiveIntentDependencyTypes.ts";
import type { IntentEvolutionResult } from "./executiveIntentEvolutionTypes.ts";
import type { IntentExtractionResult } from "./executiveIntentExtractionTypes.ts";
...
import type {
  ExecutiveIntentReadinessAssessment,
  ExecutiveIntentReadinessState,
  ExecutiveIntentReasoningEvidence,
  ExecutiveIntentReasoningHighlights,
  ExecutiveIntentReasoningHighlightKey,
  ExecutiveIntentReasoningIssue,
  ExecutiveIntentReasoningIssueKey,
  ExecutiveIntentReasoningSection,
  ExecutiveIntentReasoningSectionKey,
  ExecutiveIntentReasoningSummary,
  ExecutiveIntentReasoningUnknown,
} from "./executiveIntentReasoningTypes.ts";

export const EXECUTIVE_INTENT_REASONING_RULES_VERSION = "APP-3/11-RULES-1" as const;
...
export type ReasoningRuleId = (typeof REASONING_RULE_IDS)[number];
...
function buildSection(input: Readonly<{
...
export function buildReasoningSections(input: Readonly<{
  ...
  conflict: IntentConflictResult | null;
  dependency: IntentDependencyResult | null;
  evolution: IntentEvolutionResult | null;
  confidence: IntentConfidenceResult | null;
}>): readonly ExecutiveIntentReasoningSection[] {
  const {
    extraction,
    state,
    semanticModel,
  ...
  return Object.freeze([
    ...
    buildSection({
      ...
      content:
        classification?.summary.primaryClassLabel ?? "Primary classification unavailable.",
    ...
    buildSection({
      ...
      content:
        classification && classification.summary.secondaryClassLabels.length > 0
    ...
    buildSection({
      ...
      content:
        semanticModel && semanticModel.knownInformation.length > 0
    ...
    buildSection({
      ...
      content:
        semanticModel && semanticModel.unknowns.length > 0
...
export function buildReasoningUnknowns(
  semanticModel: ExecutiveIntentSemanticModel | null
): readonly ExecutiveIntentReasoningUnknown[] {
  if (!semanticModel) return Object.freeze([]);
  return Object.freeze(
    semanticModel.unknowns.map((entry) =>
      Object.freeze({
...
export function buildReasoningEvidence(input: Readonly<{
  extraction: IntentExtractionResult | null;
  semanticModel: ExecutiveIntentSemanticModel | null;
  classification: IntentClassificationResult | null;
  conflict: IntentConflictResult | null;
  ...
  evolution: IntentEvolutionResult | null;
  confidence: IntentConfidenceResult | null;
}>): readonly ExecutiveIntentReasoningEvidence[] {
  const evidence: ExecutiveIntentReasoningEvidence[] = [];
...
export function buildReasoningIssues(input: Readonly<{
  semanticModel: ExecutiveIntentSemanticModel | null;
  classification: IntentClassificationResult | null;
  conflict: IntentConflictResult | null;
  dependency: IntentDependencyResult | null;
  evolution: IntentEvolutionResult | null;
  confidence: IntentConfidenceResult | null;
}>): readonly ExecutiveIntentReasoningIssue[] {
  const issues: ExecutiveIntentReasoningIssue[] = [];
  ...
  if (input.semanticModel?.flags.missingTarget || input.semanticModel?.flags.missingMeasure) {
    pushIssue(
      ...
      "Semantic model indicates missing target or measure information.",
      "warning",
      "APP-3/5",
      false
    );
  ...
  if (
    input.semanticModel?.timeHorizon.kind === "unknown" ||
    input.semanticModel?.timeHorizon.label.toLowerCase().includes("unknown")
  ) {
    pushIssue(
      "missing_deadline",
      "Missing Deadline",
      "Time horizon is not explicitly defined.",
      "warning",
      "APP-3/5",
      false
    );
  ...
  if (input.conflict?.flags.hasConflict) {
    pushIssue(
      ...
      input.conflict.summary.headline,
      "warning",
      "APP-3/7",
      Boolean(input.conflict.flags.requiresExecutiveReview)
    );
  }
  ...
  if (
    ...
    pushIssue(
      "unstable_evolution",
      "Unstable Evolution",
      input.evolution?.summary.headline ?? "Evolution history is unstable.",
      "warning",
      "APP-3/9",
      input.evolution?.status === "broken"
    );
  ...
  if (
    ...
    pushIssue(
      ...
      input.confidence.summary.headline,
      "warning",
      "APP-3/10",
      false
    );
  ...
  if (
    ...
    pushIssue(
      ...
      `Classification status is ${input.classification.status}.`,
      "warning",
      "APP-3/6",
      !input.classification.primaryClass
    );
  }
  if (
    input.semanticModel &&
    ...
    pushIssue(
      "unknown_constraints",
      "Unknown Constraints",
      "Constraints may be present but are not explicitly captured.",
      "info",
      "APP-3/5",
      false
    );
...
export function buildReasoningHighlights(input: Readonly<{
  semanticModel: ExecutiveIntentSemanticModel | null;
  conflict: IntentConflictResult | null;
  dependency: IntentDependencyResult | null;
  evolution: IntentEvolutionResult | null;
  ...
  const items: ExecutiveIntentReasoningHighlights["items"][number][] = [];
  ...
  if (
    input.semanticModel?.primaryGoal &&
    ...
    pushHighlight(
      ...
      input.semanticModel.summary.primaryGoalLabel
    );
  }
  if (
    input.evolution &&
    ...
    pushHighlight(
      "stable_strategy",
      "Stable Strategy",
      input.evolution.summary.headline
    );
  }
  if (
    input.semanticModel?.primaryGoal &&
  ...
  if (
    input.dependency?.flags.hasDependencies &&
    input.dependency.summary.highestStrength === "critical"
  ) {
    pushHighlight(
      "critical_dependency",
      "Critical Dependency",
      input.dependency.summary.headline
    );
  }
  if (input.unknownCount >= 3) {
    pushHighlight(
      "multiple_unknowns",
      "Multiple Unknowns",
      `${input.unknownCount} explicit unknowns are recorded.`
    );
  }
  if (input.evolution && input.evolution.timeline.events.length > 5) {
    pushHighlight(
      "recent_strategy_shift",
      "Recent Strategy Shift",
      `${input.evolution.timeline.events.length} evolution events recorded.`
    );
  }
...
export function buildReadinessAssessment(input: Readonly<{
  extraction: IntentExtractionResult | null;
  state: IntentResolutionResult | null;
  semanticModel: ExecutiveIntentSemanticModel | null;
  confidence: IntentConfidenceResult | null;
  ...
  const extractionAvailable = Boolean(
    input.extraction?.primaryIntent && input.extraction.status !== "failed"
  );

  let state: ExecutiveIntentReadinessState = "unknown";
  ...
  if (input.state?.state.flags.isArchived) {
    ...
    input.confidence?.flags.readyForReasoning &&
  ...
  const readyForAssistant =
    state === "ready" || (state === "needs_clarification" && blockingIssueCount === 0);
...
export function buildReasoningSummary(input: Readonly<{
  extraction: IntentExtractionResult | null;
  semanticModel: ExecutiveIntentSemanticModel | null;
  classification: IntentClassificationResult | null;
  confidence: IntentConfidenceResult | null;
...
export function collectReasoningRulesApplied(): readonly string[] {
  return Object.freeze([...REASONING_RULE_IDS]);
}

export function collectEnginesConsumed(input: Readonly<{
  extraction: IntentExtractionResult | null;
  state: IntentResolutionResult | null;
  semanticModel: ExecutiveIntentSemanticModel | null;
  classification: IntentClassificationResult | null;
  ...
  const engines: string[] = ["APP-3/1"];
  if (input.extraction) engines.push("APP-3/4");
  if (input.state) engines.push("APP-3/2");
  if (input.semanticModel) engines.push("APP-3/5");
  if (input.classification) engines.push("APP-3/6");
  if (input.conflict) engines.push("APP-3/7");
  if (input.dependency) engines.push("APP-3/8");
  if (input.evolution) engines.push("APP-3/9");
  if (input.confidence) engines.push("APP-3/10");
