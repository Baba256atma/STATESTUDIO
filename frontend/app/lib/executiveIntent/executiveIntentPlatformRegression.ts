...
export const EXECUTIVE_INTENT_REGRESSION_VERSION = "APP-3/14-REGRESSION-1" as const;

export type ExecutiveIntentRegressionPhaseResult = Readonly<{
  phaseId: string;
...
export type ExecutiveIntentRegressionResult = Readonly<{
  resultId: string;
  passed: boolean;
  phases: readonly ExecutiveIntentRegressionPhaseResult[];
  passedCount: number;
  failedCount: number;
  skippedCount: number;
  timestamp: string;
...
function buildPipelineBundle(text: string) {
...
function verifyConsumerSource(path: string, forbidden: readonly string[]): boolean {
  const source = readFileSync(new URL(path, import.meta.url), "utf8");
  return forbidden.every((token) => !source.includes(token));
}

export function runExecutiveIntentRegression(
  timestamp: string = DEFAULT_TIME
): ExecutiveIntentRegressionResult {
  const phases: ExecutiveIntentRegressionPhaseResult[] = [];
  ...
  record(
    "APP-3/1",
    EXECUTIVE_INTENT_CONTRACT_VERSION,
    EXECUTIVE_INTENT_CONTRACT_VERSION === "APP-3/1" &&
  ...
  record(
    ...
    EXECUTIVE_INTENT_STATE_ENGINE_VERSION === "APP-3/2",
    "State engine version verified."
  );
  ...
  record(
    ...
    bundle.extraction.engineVersion === EXECUTIVE_INTENT_EXTRACTION_ENGINE_VERSION,
    "Extraction engine regression passed."
  );

  record(
    "APP-3/5",
    EXECUTIVE_INTENT_SEMANTIC_MODEL_VERSION,
    bundle.semantic.model.readOnly === true &&
      bundle.semantic.model.versionMetadata.semanticModelVersion === EXECUTIVE_INTENT_SEMANTIC_MODEL_VERSION,
    "Semantic model regression passed."
  );

  record(
    "APP-3/6",
    EXECUTIVE_INTENT_CLASSIFICATION_ENGINE_VERSION,
    bundle.classification.metadata.classificationEngineVersion === EXECUTIVE_INTENT_CLASSIFICATION_ENGINE_VERSION,
    "Classification engine regression passed."
  );
  ...
  record(
    ...
    conflict.metadata.conflictEngineVersion === EXECUTIVE_INTENT_CONFLICT_ENGINE_VERSION,
    "Conflict engine regression passed."
  );
  ...
  record(
    ...
    dependency.metadata.dependencyEngineVersion === EXECUTIVE_INTENT_DEPENDENCY_ENGINE_VERSION,
    "Dependency engine regression passed."
  );

  record(
    "APP-3/9",
    EXECUTIVE_INTENT_EVOLUTION_ENGINE_VERSION,
    EXECUTIVE_INTENT_EVOLUTION_ENGINE_VERSION === "APP-3/9",
    "Evolution engine version verified."
  );

  record(
    "APP-3/10",
    EXECUTIVE_INTENT_CONFIDENCE_ENGINE_VERSION,
    EXECUTIVE_INTENT_CONFIDENCE_ENGINE_VERSION === "APP-3/10",
    "Confidence engine version verified."
  );

  record(
    "APP-3/11",
    EXECUTIVE_INTENT_REASONING_ENGINE_VERSION,
    typeof buildExecutiveIntentReasoning === "function",
    "Reasoning engine export verified."
  );

  record(
    "APP-3/12",
    EXECUTIVE_INTENT_ASSISTANT_INTEGRATION_VERSION,
    ExecutiveIntentAssistantIntegration.version === EXECUTIVE_INTENT_ASSISTANT_INTEGRATION_VERSION &&
  ...
  record(
    ...
    ExecutiveIntentDashboardIntegration.version === EXECUTIVE_INTENT_DASHBOARD_INTEGRATION_VERSION &&
