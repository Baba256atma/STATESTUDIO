...
import {
  createIntentContextDiagnostic,
  type IntentContextDiagnostic,
...
} from "./executiveIntentContextRules.ts";
import {
  createExecutiveIntentContext,
  createExecutiveIntentContextAnalysisInput,
  createIntentContextFlags,
  createIntentContextMetadata,
  createIntentContextSummary,
  EXECUTIVE_INTENT_CONTEXT_ENGINE_VERSION,
  type ExecutiveIntentContext,
  type ExecutiveIntentContextAnalysisInput,
  type IntentBusinessContext,
  type IntentConstraintContext,
  type IntentContextScope,
  type IntentContextSummary,
  type IntentContextValidationResult,
  type IntentEvidenceContext,
  type IntentObjectContext,
  type IntentRelationshipContext,
  type IntentStakeholderContext,
  type IntentWorkspaceContext,
} from "./executiveIntentContextTypes.ts";
...
export const EXECUTIVE_INTENT_CONTEXT_ENGINE_OWNER = "executive-intent-context" as const;
...
function pushDiagnostic(
  diagnostics: IntentContextDiagnostic[],
  code: Parameters<typeof createIntentContextDiagnostic>[0],
  message: string,
  timestamp: string,
  options: Parameters<typeof createIntentContextDiagnostic>[3] = Object.freeze({})
): void {
...
export function buildObjectContext(
  intent: ExecutiveIntent | null,
  semanticModel: ExecutiveIntentSemanticModel | null
): readonly IntentObjectContext[] {
  const objects: IntentObjectContext[] = [];
  ...
  const addObject = (
    ...
    label: string,
    objectType: string,
    source: IntentObjectContext["source"]
  ): void => {
  ...
  if (semanticModel?.targetEntity) {
    addObject(
      semanticModel.targetEntity.targetId,
      semanticModel.targetEntity.entityLabel,
      semanticModel.targetEntity.entityType ?? "target",
      "semantic_target"
    );
  }
...
export function buildRelationshipContext(
  intent: ExecutiveIntent | null
): readonly IntentRelationshipContext[] {
  if (!intent) return Object.freeze([]);
  return Object.freeze(
    intent.relations.map((relation) =>
      Object.freeze({
...
export function buildConstraintContext(
  intent: ExecutiveIntent | null,
  semanticModel: ExecutiveIntentSemanticModel | null
): readonly IntentConstraintContext[] {
  const constraints: IntentConstraintContext[] = [];
  const seen = new Set<string>();
  ...
  const addConstraint = (
    ...
    label: string,
    description: string,
    origin: IntentConstraintContext["origin"]
  ): void => {
...
export function buildStakeholderContext(
  intent: ExecutiveIntent | null,
  semanticModel: ExecutiveIntentSemanticModel | null
): readonly IntentStakeholderContext[] {
  const stakeholders: IntentStakeholderContext[] = [];
  const seen = new Set<string>();
  ...
  const addStakeholder = (
    ...
    name: string,
    role: string | null,
    stakeholderType: IntentStakeholderContext["stakeholderType"]
  ): void => {
  ...
  if (intent?.metadata.owner) {
    addStakeholder(
      deterministicId("stakeholder-owner", intent.metadata.owner),
      intent.metadata.owner,
      "owner",
      "owner"
    );
  }
...
function buildEvidenceContext(
  intent: ExecutiveIntent | null,
  semanticModel: ExecutiveIntentSemanticModel | null
): readonly IntentEvidenceContext[] {
  const evidence: IntentEvidenceContext[] = [];
  const seen = new Set<string>();
  ...
  const addEvidence = (
    ...
    source: string,
    summary: string,
    origin: IntentEvidenceContext["origin"]
  ): void => {
...
export function buildContextSummary(context: Omit<ExecutiveIntentContext, "summary">): IntentContextSummary {
...
export function validateContext(context: ExecutiveIntentContext): IntentContextValidationResult {
  const issues: string[] = [];
  if (context.readOnly !== true) issues.push("Context must be read-only.");
  if (!context.contextId) issues.push("Context id is required.");
...
}): ReturnType<typeof createIntentContextFlags> {
...
export function buildExecutiveIntentContext(
  input: ExecutiveIntentContextAnalysisInput
): ExecutiveIntentContext {
  const diagnostics: IntentContextDiagnostic[] = [];
  ...
  pushDiagnostic(
    ...
    "Context engine reserved for APP-3.15.1 reasoning refresh.",
    timestamp
  );

  const partialContext = {
    contextId: deterministicId(
      "intent-context",
      `${intent?.intentId ?? "none"}:${semanticModel?.modelId ?? "none"}:${timestamp}`
    ),
...
function buildPipelineFromText(
  ...
  const extraction = extractExecutiveIntent(
    Object.freeze({ text, workspaceId, owner, languageCode: "en", generatedAt: timestamp })
  );
...
export function buildContextFromExample(
  exampleId: string,
  workspaceId: string,
  owner: string,
  timestamp: string
): ExecutiveIntentContext | null {
  ...
  if (example.expectedUnknown && !example.customText && !example.semanticExampleId) {
    return buildExecutiveIntentContext(
      createExecutiveIntentContextAnalysisInput({
        intent: null,
        state: null,
        semanticModel: null,
...
export function buildContextProbe(timestamp: string = "2026-01-01T00:00:00.000Z"): ExecutiveIntentContext {
  return (
    buildContextFromExample("single-workspace", "ws-context-probe", "executive-context-probe", timestamp) ??
...
export function getExecutiveIntentContextEngineVersionMetadata(): Readonly<{
  contextEngineVersion: typeof EXECUTIVE_INTENT_CONTEXT_ENGINE_VERSION;
  owner: typeof EXECUTIVE_INTENT_CONTEXT_ENGINE_OWNER;
}> {
  return Object.freeze({
    contextEngineVersion: EXECUTIVE_INTENT_CONTEXT_ENGINE_VERSION,
    owner: EXECUTIVE_INTENT_CONTEXT_ENGINE_OWNER,
