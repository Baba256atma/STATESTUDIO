...
import type { IntentConflictPair, IntentConflictResult } from "./executiveIntentConflictTypes.ts";
...
import type {
  IntentDependency,
  IntentDependencyCategory,
  IntentDependencyEdge,
  IntentDependencyNode,
  IntentDependencyReference,
  IntentDependencyStrength,
} from "./executiveIntentDependencyTypes.ts";

export const EXECUTIVE_INTENT_DEPENDENCY_RULES_VERSION = "APP-3/8-RULES-1" as const;
...
export const DEPENDENCY_STRENGTH_ORDER = Object.freeze([
  ...
  "critical",
  "unknown",
] as const satisfies readonly IntentDependencyStrength[]);
...
export const DEPENDENCY_CATEGORY_ORDER = Object.freeze([
  ...
  "unknown",
  "custom",
] as const satisfies readonly IntentDependencyCategory[]);

const TIME_HORIZON_RANK: Readonly<Record<SemanticTimeHorizonKind, number>> = Object.freeze({
  immediate: 0,
  short_term: 1,
  medium_term: 2,
  long_term: 3,
...
export function buildDependencyReference(
...
function createDependency(input: Readonly<{
  dependent: IntentDependencyReference;
  prerequisite: IntentDependencyReference;
  ...
  return Object.freeze({
    dependencyId: deterministicId(
      "dependency",
      `${input.dependent.semanticModelId}:${input.prerequisite.semanticModelId}:${input.ruleId}`
    ),
...
function sharedBusinessObjects(
...
function targetsOverlap(left: ExecutiveIntentSemanticModel, right: ExecutiveIntentSemanticModel): boolean {
...
export type IntentDependencyAnalysisBundle = Readonly<{
...
function refFromNode(node: IntentDependencyNode): IntentDependencyReference {
  return Object.freeze({
...
function goalKeywords(model: ExecutiveIntentSemanticModel): string {
...
function detectKeywordDependencies(
  dependent: IntentDependencyAnalysisBundle,
  prerequisite: IntentDependencyAnalysisBundle,
  dependentRef: IntentDependencyReference,
  prerequisiteRef: IntentDependencyReference
): IntentDependency[] {
  const results: IntentDependency[] = [];
...
export function detectPairDependencies(input: Readonly<{
  dependent: IntentDependencyAnalysisBundle;
  prerequisite: IntentDependencyAnalysisBundle;
  conflictPair: IntentConflictPair | null;
}>): readonly IntentDependency[] {
  if (input.dependent.semanticModel.modelId === input.prerequisite.semanticModel.modelId) {
    return Object.freeze([]);
  }

  const dependencies: IntentDependency[] = [];
  ...
  if (
    (preClass === "people" || preClass === "resource" || pre.actionType === "create") &&
  ...
  if (
    (preClass === "technology" || preClass === "transformation") &&
  ...
  if (
    dep.businessDimension === "financial" &&
  ...
  if (input.conflictPair && !input.conflictPair.compatible) {
    const hasCritical = input.conflictPair.conflicts.some((entry) =>
  ...
  if (
    (dep.flags.incompleteObjective || pre.flags.incompleteObjective) &&
...
export function sortDependencies(dependencies: readonly IntentDependency[]): readonly IntentDependency[] {
  return Object.freeze(
    [...dependencies].sort((left, right) => {
      const strengthDiff =
        DEPENDENCY_STRENGTH_ORDER.indexOf(right.strength) -
        DEPENDENCY_STRENGTH_ORDER.indexOf(left.strength);
      if (strengthDiff !== 0) return strengthDiff;
      const categoryDiff =
        DEPENDENCY_CATEGORY_ORDER.indexOf(left.category) -
...
export function highestDependencyStrength(
  dependencies: readonly IntentDependency[]
): IntentDependencyStrength {
...
export function collectDependencyRulesApplied(
  dependencies: readonly IntentDependency[]
): readonly string[] {
  return Object.freeze([...new Set(dependencies.map((entry) => entry.ruleId))].sort());
}

export function findConflictPair(
  batchConflict: IntentConflictResult | null,
  leftModelId: string,
  rightModelId: string
): IntentConflictPair | null {
  if (!batchConflict) return null;
  return (
    batchConflict.matrix.pairs.find(
      (pair) =>
        (pair.leftSemanticModelId === leftModelId && pair.rightSemanticModelId === rightModelId) ||
        (pair.leftSemanticModelId === rightModelId && pair.rightSemanticModelId === leftModelId)
    ) ?? null
  );
}

export function buildDependencyNodes(
  bundles: readonly IntentDependencyAnalysisBundle[]
): readonly IntentDependencyNode[] {
  return Object.freeze(
    bundles
      .map((bundle) =>
        Object.freeze({
...
export function buildDependencyEdges(
  ...
  return Object.freeze(
    dependencies
      ...
      .filter((entry): entry is IntentDependencyEdge => entry !== null)
...
export function detectDependencyCycles(
  ...
  for (const edge of edges) {
    if (edge.category === "parallel" || edge.category === "shared_prerequisite") continue;
...
export function detectIndirectDependencies(
  dependencies: readonly IntentDependency[],
  nodes: readonly IntentDependencyNode[]
): readonly IntentDependency[] {
  ...
  for (const dep of dependencies) {
    if (dep.category === "parallel" || dep.bidirectional) continue;
...
export function pairDependencyKey(dependentId: string, prerequisiteId: string): string {
