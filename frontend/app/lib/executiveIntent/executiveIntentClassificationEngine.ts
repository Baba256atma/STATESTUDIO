...
import {
  createIntentClassificationDiagnostic,
  type IntentClassificationDiagnostic,
...
import {
  collectRulesApplied,
  resolveClassificationCandidates,
  resolvePrimaryClassFromCandidates,
  resolveSecondaryClassesFromCandidates,
  isSupportedActionType,
  type ClassificationCandidate,
} from "./executiveIntentClassificationRules.ts";
import {
  EXECUTIVE_INTENT_CLASSIFICATION_TAXONOMY_VERSION,
  getIntentTaxonomyClassDefinition,
  sortIntentClasses,
  type IntentTaxonomyClassKey,
...
import {
  createIntentClassificationResult,
  EXECUTIVE_INTENT_CLASSIFICATION_ENGINE_VERSION,
  type IntentClassificationExplanation,
  type IntentClassificationFlags,
  type IntentClassificationMetadata,
  type IntentClassificationResult,
  type IntentClassificationSummary,
  type IntentClassificationValidationResult,
  type IntentPrimaryClass,
  type IntentSecondaryClass,
} from "./executiveIntentClassificationTypes.ts";

export const EXECUTIVE_INTENT_CLASSIFICATION_ENGINE_OWNER = "executive-intent-classification" as const;
...
function pushDiagnostic(
  diagnostics: IntentClassificationDiagnostic[],
  code: Parameters<typeof createIntentClassificationDiagnostic>[0],
  message: string,
  timestamp: string,
  options: Parameters<typeof createIntentClassificationDiagnostic>[3] = Object.freeze({})
): void {
  diagnostics.push(createIntentClassificationDiagnostic(code, message, timestamp, options));
}

export function resolvePrimaryClassification(
  model: ExecutiveIntentSemanticModel,
  candidates: readonly ClassificationCandidate[] = resolveClassificationCandidates(model)
): IntentPrimaryClass | null {
...
export function resolveSecondaryClassifications(
  model: ExecutiveIntentSemanticModel,
  primaryClass: IntentPrimaryClass | null,
  candidates: readonly ClassificationCandidate[] = resolveClassificationCandidates(model)
): readonly IntentSecondaryClass[] {
  const secondaryIds = resolveSecondaryClassesFromCandidates(
    candidates,
    primaryClass?.classId ?? null
  );

  return Object.freeze(
    secondaryIds.map((classId) => {
...
export function resolveClassificationFlags(input: Readonly<{
  primaryClass: IntentPrimaryClass | null;
  secondaryClasses: readonly IntentSecondaryClass[];
  allClasses: readonly IntentTaxonomyClassKey[];
  model: ExecutiveIntentSemanticModel;
...
export function buildClassificationSummary(input: Readonly<{
  primaryClass: IntentPrimaryClass | null;
  secondaryClasses: readonly IntentSecondaryClass[];
  allClasses: readonly IntentTaxonomyClassKey[];
}>): IntentClassificationSummary {
  ...
  const headline =
    input.secondaryClasses.length > 0
...
function buildExplanations(
  ...
  primaryClass: IntentPrimaryClass | null,
  secondaryClasses: readonly IntentSecondaryClass[]
): readonly IntentClassificationExplanation[] {
  const entries: IntentClassificationExplanation[] = [];
...
export function classifySemanticModel(
  model: ExecutiveIntentSemanticModel,
...
export function classifyExecutiveIntent(
  ...
  const diagnostics: IntentClassificationDiagnostic[] = [];
  ...
  if (!validation.valid || model.readOnly !== true) {
    pushDiagnostic(
      ...
      "Semantic model failed validation for classification.",
      timestamp,
      { metadata: Object.freeze({ issues: validation.issues }) }
    );
  }
  ...
  if (!model.primaryGoal && model.goals.length === 0) {
    pushDiagnostic(
      ...
      "Classification cannot proceed without semantic goals.",
      timestamp
    );
  }
  ...
  if (model.businessDimension === "custom") {
    pushDiagnostic(
      ...
      "Semantic business dimension is custom.",
      timestamp
    );
  }
  ...
  if (!isSupportedActionType(model.actionType)) {
    pushDiagnostic(
      ...
      `Unsupported semantic action type: ${model.actionType}.`,
      timestamp
    );
  }
  ...
  if (model.actionType === "custom" || model.businessDimension === "custom") {
    pushDiagnostic(
      ...
      "Custom taxonomy class required for unmapped semantic signals.",
      timestamp
    );
  }
  ...
  const distinctPrimaryCandidates = sortIntentClasses(
    primaryCandidates.map((entry) => entry.classId)
  );
  if (distinctPrimaryCandidates.length > 1) {
    pushDiagnostic(
      diagnostics,
      "multiple_primary_classes",
      "Multiple primary class candidates detected; highest-priority rule applied.",
      timestamp,
      { metadata: Object.freeze({ candidates: distinctPrimaryCandidates }) }
    );
  }
  ...
  const allClasses = Object.freeze(
    sortIntentClasses([
  ...
  if (allClasses.length > 1) {
    pushDiagnostic(
      diagnostics,
      "multi_label_classification",
      `${allClasses.length} taxonomy classes assigned.`,
      timestamp
    );
  }
  ...
  if (flags.requiresManualReview) {
    pushDiagnostic(
      ...
      "Classification may require manual review.",
      timestamp
    );
  }

  const rulesApplied = collectRulesApplied(candidates);
  const metadata: IntentClassificationMetadata = Object.freeze({
    taxonomyVersion: EXECUTIVE_INTENT_CLASSIFICATION_TAXONOMY_VERSION,
  ...
  let status: IntentClassificationResult["status"] = "classified";
  if (!primaryClass) status = "unknown";
  else if (model.flags.incompleteObjective || flags.customClassification) status = "partial";
  else if (flags.requiresManualReview && allClasses.length === 1) status = "partial";
...
export function validateClassification(
...
): IntentClassificationValidationResult {
  const issues: string[] = [];
  if (result.readOnly !== true) issues.push("Classification result must be read-only.");
  if (result.flags.readOnly !== true) issues.push("Classification flags must be read-only.");
  if (result.flags.deterministic !== true) issues.push("Classification must be deterministic.");
  if (
    result.metadata.classificationEngineVersion !== EXECUTIVE_INTENT_CLASSIFICATION_ENGINE_VERSION
  ) {
  ...
  const primaryInAll = result.primaryClass
    ...
    : true;
  if (!primaryInAll) issues.push("Primary class must appear in allClasses.");
  return Object.freeze({
...
export function buildClassificationExample(
  exampleId: string,
  ...
  if (example.semanticExampleId) {
    semanticResult = buildExecutiveIntentSemanticModelFromExample(
      example.semanticExampleId,
      workspaceId,
      owner,
      generatedAt
    );
...
export function buildClassificationProbe(
  generatedAt: string = new Date(0).toISOString()
): IntentClassificationResult {
  const extraction = extractExecutiveIntent(
    Object.freeze({
...
export function getExecutiveIntentClassificationEngineVersionMetadata(): Readonly<{
  ...
  owner: typeof EXECUTIVE_INTENT_CLASSIFICATION_ENGINE_OWNER;
}> {
  return Object.freeze({
    classificationEngineVersion: EXECUTIVE_INTENT_CLASSIFICATION_ENGINE_VERSION,
    taxonomyVersion: EXECUTIVE_INTENT_CLASSIFICATION_TAXONOMY_VERSION,
