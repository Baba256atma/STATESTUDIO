...
import type {
  IntentConfidenceFactor,
  IntentConfidenceFactorKey,
  IntentConfidenceLevel,
} from "./executiveIntentConfidenceTypes.ts";

export const EXECUTIVE_INTENT_CONFIDENCE_RULES_VERSION = "APP-3/10-RULES-1" as const;
...
export type ConfidenceRuleId = (typeof CONFIDENCE_RULE_IDS)[number];

export const CONFIDENCE_FACTOR_WEIGHTS: Readonly<Record<IntentConfidenceFactorKey, number>> =
  Object.freeze({
...
export const CONFIDENCE_LEVEL_THRESHOLDS = Object.freeze([
  Object.freeze({ level: "very_high" as const, minimum: 90 }),
  Object.freeze({ level: "high" as const, minimum: 75 }),
  Object.freeze({ level: "medium" as const, minimum: 55 }),
  Object.freeze({ level: "low" as const, minimum: 35 }),
  Object.freeze({ level: "very_low" as const, minimum: 15 }),
] as const);
...
function buildFactor(input: Readonly<{
...
export function scoreExtractionCompleteness(
  ...
  if (extraction) {
    blocking = false;
    if (extraction.status === "success" && extraction.primaryIntent) raw = 100;
    else if (extraction.status === "partial" && extraction.primaryIntent) raw = 65;
    else if (extraction.status === "failed") raw = 20;
    else raw = 40;
...
export function scoreSemanticCompleteness(
  ...
  if (semanticModel) {
    blocking = false;
    raw = 100;
    if (semanticModel.flags.incompleteObjective) raw -= 35;
    if (semanticModel.flags.missingTarget) raw -= 20;
    if (semanticModel.flags.missingMeasure) raw -= 15;
    if (semanticModel.flags.requiresClarification) raw -= 10;
    if (semanticModel.unknowns.length > 0) raw -= Math.min(25, semanticModel.unknowns.length * 5);
...
export function scoreClassificationDeterminism(
  ...
  if (classification) {
    if (classification.status === "classified" && classification.primaryClass) raw = 100;
    else if (classification.status === "partial") raw = 60;
    else if (classification.status === "unknown") raw = 25;
    else raw = 40;

    if (classification.flags.customClassification) raw -= 15;
    if (classification.flags.requiresManualReview) raw -= 10;
    if (!classification.flags.deterministic) raw -= 20;
...
export function scoreConflictImpact(conflict: IntentConflictResult | null): IntentConfidenceFactor {
  let raw = 85;
  ...
  if (conflict) {
    if (!conflict.flags.hasConflict) {
      ...
      if (conflict.flags.duplicateIntent) raw -= 15;
      if (conflict.flags.requiresExecutiveReview) raw -= 10;
      if (conflict.summary.highestSeverity === "critical") raw -= 20;
...
export function scoreDependencyComplexity(
  ...
  if (dependency) {
    if (dependency.flags.independentIntent || !dependency.flags.hasDependencies) {
      ...
      if (dependency.flags.circularDependency) raw -= 25;
      if (dependency.flags.requiresPrerequisite) raw -= 10;
...
export function scoreEvolutionStability(evolution: IntentEvolutionResult | null): IntentConfidenceFactor {
  let raw = 85;
  let diagnostic = "confidence_medium";
...
export function scoreStateIntegrity(state: IntentResolutionResult | null): IntentConfidenceFactor {
  let raw = 50;
  ...
  if (state) {
    if (state.structuralHealth === "healthy") raw = 100;
    else if (state.structuralHealth === "warning") raw = 70;
    else if (state.structuralHealth === "invalid") raw = 30;
    else raw = 45;

    if (state.state.flags.isBlocked) raw -= 15;
...
export function scoreStructuralConsistency(input: Readonly<{
  extraction: IntentExtractionResult | null;
  semanticModel: ExecutiveIntentSemanticModel | null;
  classification: IntentClassificationResult | null;
  ...
  return buildFactor({
    ...
    explanation:
      issues.length > 0 ? `Structural issues: ${issues.join(", ")}.` : "Pipeline artifacts are consistent.",
    blocking,
  });
}

export function scoreUnknownInformation(
  semanticModel: ExecutiveIntentSemanticModel | null
): IntentConfidenceFactor {
...
export function scoreReadiness(input: Readonly<{
  semanticModel: ExecutiveIntentSemanticModel | null;
  classification: IntentClassificationResult | null;
  state: IntentResolutionResult | null;
  ...
  const semanticReady = Boolean(
    input.semanticModel?.primaryGoal && !input.semanticModel.flags.incompleteObjective
  );
...
export function scoreFutureCompatibility(
  semanticModel: ExecutiveIntentSemanticModel | null,
  classification: IntentClassificationResult | null
): IntentConfidenceFactor {
...
export function resolveConfidenceFactors(
  input: Readonly<{
    extraction: IntentExtractionResult | null;
    semanticModel: ExecutiveIntentSemanticModel | null;
    classification: IntentClassificationResult | null;
...
): readonly IntentConfidenceFactor[] {
  return Object.freeze([
...
export function resolveAggregateScore(factors: readonly IntentConfidenceFactor[]): number {
...
export function resolveConfidenceLevel(score: number): IntentConfidenceLevel {
  for (const threshold of CONFIDENCE_LEVEL_THRESHOLDS) {
    if (score >= threshold.minimum) return threshold.level;
  }
  return "unknown";
}
