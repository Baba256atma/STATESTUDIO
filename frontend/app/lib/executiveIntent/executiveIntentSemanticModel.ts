...
import {
  createIntentSemanticDiagnostic,
  type IntentSemanticDiagnostic,
...
import {
  createExecutiveIntentSemanticModel,
  EXECUTIVE_INTENT_SEMANTIC_MODEL_VERSION,
  type ExecutiveIntentSemanticModel,
  type SemanticActionType,
  type SemanticActor,
  type SemanticAssumption,
  type SemanticBusinessDimension,
  type SemanticChange,
  type SemanticConstraint,
  type SemanticFlags,
  type SemanticGoal,
  type SemanticMeasure,
  type SemanticModelValidationResult,
  type SemanticNormalizationResult,
  type SemanticObject,
  type SemanticOutcome,
  type SemanticSummary,
  type SemanticTarget,
  type SemanticTimeHorizon,
  type SemanticUnknown,
} from "./executiveIntentSemanticTypes.ts";

export const EXECUTIVE_INTENT_SEMANTIC_MODEL_OWNER = "executive-intent-semantic-model" as const;
...
function pushDiagnostic(
  diagnostics: IntentSemanticDiagnostic[],
  code: Parameters<typeof createIntentSemanticDiagnostic>[0],
  message: string,
  timestamp: string,
  options: Parameters<typeof createIntentSemanticDiagnostic>[3] = Object.freeze({})
): void {
  diagnostics.push(createIntentSemanticDiagnostic(code, message, timestamp, options));
}

export function normalizeSemanticGoal(
  goal: ExtractedGoal,
...
export function normalizeActionTypeFromVerb(verb: string): SemanticActionType {
  return mapActionVerbToActionType(verb);
}

export function normalizeSemanticOutcome(
...
export function normalizeSemanticTarget(target: ExtractedTarget | null): SemanticTarget | null {
  if (!target || !target.objectLabel) return null;
  return Object.freeze({
...
export function normalizeBusinessDimension(
  intent: ExecutiveIntent | null,
  target: ExtractedTarget | null
...
export function normalizeActors(actors: readonly ExtractedActor[]): readonly SemanticActor[] {
  return Object.freeze(
    actors.map((entry) =>
      Object.freeze({
...
export function normalizeBusinessObjects(
...
function normalizeMeasure(target: ExtractedTarget | null): SemanticMeasure | null {
  if (!target) return null;
  if (!target.valueLabel && target.numericValue === null) return null;
  return Object.freeze({
...
export function resolveSemanticUnknowns(input: Readonly<{
  target: ExtractedTarget | null;
...
export function buildSemanticSummary(model: Omit<ExecutiveIntentSemanticModel, "readOnly">): SemanticSummary {
  return Object.freeze({
...
function buildSemanticFlags(input: Readonly<{
  ...
  const requiresClarification =
    ...
    input.unknowns.some((entry) =>
      ["target_entity", "measure", "deadline", "business_dimension"].includes(entry.kind)
    );
...
export function normalizeExecutiveIntent(
  extraction: IntentExtractionResult,
...
export function buildExecutiveIntentSemanticModel(
  extraction: IntentExtractionResult,
  timestamp: string = extraction.timestamp
): SemanticNormalizationResult {
  const diagnostics: IntentSemanticDiagnostic[] = [];
  ...
  if (extraction.status === "failed" || !primaryIntent) {
    pushDiagnostic(
      ...
      "Semantic model cannot be fully normalized from failed extraction.",
      timestamp
    );
  }
  ...
  if (extraction.goals.length > 1) {
    pushDiagnostic(
      ...
      `${extraction.goals.length} goals detected in extraction.`,
      timestamp
    );
  }
  ...
  const goals = Object.freeze(
    extraction.goals.map((goal, index) =>
      normalizeSemanticGoal(goal, extraction.intents[index] ?? primaryIntent, timestamp)
    )
  );
  ...
  if (businessDimension === "custom") {
    pushDiagnostic(
      ...
      "Semantic business dimension mapped to custom.",
      timestamp
    );
  }
  ...
  else if (extraction.status === "partial" || flags.requiresClarification) status = "partial";
...
export function validateSemanticModel(model: ExecutiveIntentSemanticModel): SemanticModelValidationResult {
  const issues: string[] = [];
  if (model.readOnly !== true) issues.push("Semantic model must be read-only.");
  if (model.flags.readOnly !== true) issues.push("Semantic flags must be read-only.");
...
export function buildExecutiveIntentSemanticModelFromExample(
  semanticExampleId: string,
  workspaceId: string = "ws-example-001",
  owner: string = "executive-owner",
  generatedAt: string = new Date(0).toISOString()
): SemanticNormalizationResult | null {
...
export function buildExecutiveIntentSemanticModelProbeExample(
  generatedAt: string = new Date(0).toISOString()
): SemanticNormalizationResult {
  const extraction = extractExecutiveIntent(
    Object.freeze({
...
export function getExecutiveIntentSemanticModelVersionMetadata(): Readonly<{
  semanticModelVersion: typeof EXECUTIVE_INTENT_SEMANTIC_MODEL_VERSION;
  owner: typeof EXECUTIVE_INTENT_SEMANTIC_MODEL_OWNER;
}> {
  return Object.freeze({
    semanticModelVersion: EXECUTIVE_INTENT_SEMANTIC_MODEL_VERSION,
