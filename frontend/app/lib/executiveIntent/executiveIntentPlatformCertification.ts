...
import {
  ...
  createExecutiveIntentCertificationGate,
  createExecutiveIntentPlatformCertificationResult,
  type ExecutiveIntentCertificationGate,
  type ExecutiveIntentCertificationSummary,
  type ExecutiveIntentPlatformCertificationResult,
  type ExecutiveIntentPlatformValidationResult,
...
export const EXECUTIVE_INTENT_PLATFORM_CERTIFICATION_OWNER = "executive-intent-platform-certification" as const;

export {
  runExecutiveIntentEndToEndCertification,
} from "./executiveIntentPlatformEndToEndCertification.ts";
...
function verifySourceExcludes(path: string, forbidden: readonly string[]): boolean {
  const source = readFileSync(new URL(path, import.meta.url), "utf8");
  return forbidden.every((token) => !source.includes(token));
}

function verifyConsumerCertification(): boolean {
  ...
  return (
    verifySourceExcludes("./executiveIntentAssistantIntegration.ts", forbiddenUpstream) &&
    verifySourceExcludes("./executiveIntentDashboardIntegration.ts", forbiddenUpstream)
  );
}

export function buildExecutiveIntentCertificationSummary(input: Readonly<{
  gates: readonly ExecutiveIntentCertificationGate[];
  timestamp: string;
}>): ExecutiveIntentCertificationSummary {
...
export function validateExecutiveIntentPlatform(
  result: ExecutiveIntentPlatformCertificationResult
): ExecutiveIntentPlatformValidationResult {
  const issues: string[] = [];
  if (result.readOnly !== true) issues.push("Certification result must be read-only.");
...
export function runExecutiveIntentPlatformCertification(
  ...
  const gates: ExecutiveIntentCertificationGate[] = [];

  const addGate = (
    gateKey: ExecutiveIntentCertificationGate["gateKey"],
    label: string,
    passed: boolean,
    message: string
  ): void => {
  ...
  addGate(
    "B",
    "Contract Integrity",
    regression.phases.find((phase) => phase.phaseId === "APP-3/1")?.passed === true,
    "Contract integrity verified via regression."
  );
  addGate(
    "C",
    "State Engine",
    regression.phases.find((phase) => phase.phaseId === "APP-3/2")?.passed === true,
    "State engine verified."
  );
  addGate(
    "D",
    "Extraction Engine",
    regression.phases.find((phase) => phase.phaseId === "APP-3/4")?.passed === true,
    "Extraction engine verified."
  );
  addGate(
    "E",
    "Semantic Model",
    regression.phases.find((phase) => phase.phaseId === "APP-3/5")?.passed === true,
    "Semantic model verified."
  );
  addGate(
    "F",
    "Classification",
    regression.phases.find((phase) => phase.phaseId === "APP-3/6")?.passed === true,
    "Classification engine verified."
  );
  addGate(
    "G",
    "Conflict Detection",
    regression.phases.find((phase) => phase.phaseId === "APP-3/7")?.passed === true,
    "Conflict detection verified."
  );
  addGate(
    "H",
    "Dependency Engine",
    regression.phases.find((phase) => phase.phaseId === "APP-3/8")?.passed === true,
    "Dependency engine verified."
  );
  addGate(
    "I",
    "Evolution Engine",
    regression.phases.find((phase) => phase.phaseId === "APP-3/9")?.passed === true,
    "Evolution engine verified."
  );
  addGate(
    "J",
    "Confidence Engine",
    regression.phases.find((phase) => phase.phaseId === "APP-3/10")?.passed === true,
    "Confidence engine verified."
  );
  addGate(
    "K",
    "Reasoning Engine",
    regression.phases.find((phase) => phase.phaseId === "APP-3/11")?.passed === true,
    "Reasoning engine verified."
  );
  addGate(
    "L",
    "Assistant Integration",
    regression.phases.find((phase) => phase.phaseId === "APP-3/12")?.passed === true,
    "Assistant integration verified."
  );
  addGate(
    "M",
    "Dashboard Integration",
    regression.phases.find((phase) => phase.phaseId === "APP-3/13")?.passed === true,
    "Dashboard integration verified."
  );
  addGate(
    "N",
    "Reasoning Consumer Verification",
  ...
  addGate(
    ...
    "End-to-End Pipeline",
    endToEnd.passed,
    endToEnd.passed ? "End-to-end pipeline passed." : endToEnd.issues.join("; ")
  );
  addGate(
    "P",
    "Regression",
    regression.passed,
    regression.passed ? "Regression passed." : `${regression.failedCount} regression failure(s).`
  );
  addGate(
    "Q",
    "Architecture Rules",
  ...
  addGate(
    ...
    EXECUTIVE_INTENT_CONFIDENCE_ENGINE_RULES.readOnly === true &&
      EXECUTIVE_INTENT_REASONING_ENGINE_RULES.noMutation === true,
  ...
  addGate(
    "U",
    "No Recommendations",
    EXECUTIVE_INTENT_PLATFORM_ARCHITECTURE_RULES.noRecommendations === true,
    "No recommendations rule verified."
  );
  addGate(
    "V",
    "No Scenario Execution",
    EXECUTIVE_INTENT_PLATFORM_ARCHITECTURE_RULES.noScenarioExecution === true,
    "No scenario execution rule verified."
  );
  addGate(
    "W",
    "Backward Compatibility",
    EXECUTIVE_INTENT_PLATFORM_ARCHITECTURE_RULES.backwardCompatible === true,
    "Backward compatibility verified."
  );
  addGate(
    "X",
    "TypeScript Build",
    typeof runExecutiveIntentPlatformCertification === "function" &&
      typeof validateExecutiveIntentPlatform === "function",
    "Runtime TypeScript module integrity verified."
  );
  addGate(
    "Y",
    "Certification Tags",
    EXECUTIVE_INTENT_PLATFORM_CERTIFICATION_TAGS.includes("[PLATFORM_READY]"),
    "Certification tags verified."
  );
  ...
  addGate(
    ...
    "Platform Ready",
    allPassed,
    allPassed ? "Executive Intent Platform is certified." : "Platform certification incomplete."
  );
...
export function getExecutiveIntentPlatformCertificationVersionMetadata(): Readonly<{
  certificationVersion: typeof EXECUTIVE_INTENT_PLATFORM_CERTIFICATION_VERSION;
  owner: typeof EXECUTIVE_INTENT_PLATFORM_CERTIFICATION_OWNER;
}> {
  return Object.freeze({
    certificationVersion: EXECUTIVE_INTENT_PLATFORM_CERTIFICATION_VERSION,
