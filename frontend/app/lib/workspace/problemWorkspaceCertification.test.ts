import assert from "node:assert/strict";
import { readdirSync, readFileSync } from "node:fs";
import test from "node:test";
import { ProblemWorkspaceCertification } from "./problemWorkspaceCertification.ts";

const files = [
  "problemWorkspaceCertification.test.ts",
  "problemWorkspaceCertification.ts",
  "problemWorkspaceCertificationCompliance.ts",
  "problemWorkspaceCertificationCriteria.ts",
  "problemWorkspaceCertificationGates.ts",
  "problemWorkspaceCertificationGuarantees.ts",
  "problemWorkspaceCertificationIdentity.ts",
  "problemWorkspaceCertificationOutcomes.ts",
];

test("WS-6:7 consists of exactly eight Certification artifacts", () => {
  assert.deepEqual(
    readdirSync(new URL(".", import.meta.url))
      .filter((file) => files.includes(file))
      .sort(),
    files,
  );
});

test("WS-6:7 publishes canonical Certification identity", () => {
  const certification = ProblemWorkspaceCertification;
  assert.equal(certification.identity.id, "WS-6:7/ProblemWorkspaceCertification");
  assert.equal(
    certification.identity.namespace,
    "nexora.workspace.problem.certification",
  );
  assert.equal(certification.identity.version, "1.0.0");
  assert.equal(certification.identity.layer, "Workspace Layer");
  assert.equal(certification.status, "ReadyForFreeze");
  assert.equal(Object.isFrozen(certification), true);
});

test("WS-6:7 declares exactly 18 criteria and 16 gates", () => {
  const certification = ProblemWorkspaceCertification;
  assert.equal(certification.criteria.length, 18);
  assert.equal(certification.gates.length, 16);
  assert.equal(
    certification.criteria.every(
      ({ declaredState }) => declaredState === "Satisfied",
    ),
    true,
  );
  assert.equal(
    certification.gates.every(({ declaredState }) => declaredState === "Passed"),
    true,
  );
});

test("WS-6:7 outcomes, guarantees, and compliance are complete", () => {
  const certification = ProblemWorkspaceCertification;
  assert.deepEqual(
    certification.outcomes.map(({ name }) => name),
    ["Certified", "CertifiedWithWarnings", "NotCertified", "Blocked"],
  );
  assert.equal(certification.guarantees.length, 6);
  assert.equal(certification.compliance.length, 15);
  assert.equal(certification.compliance.every(({ present }) => !present), true);
  assert.equal(certification.certificationOutcome, "Certified");
});

test("WS-6:7 consumes Platform only and contains no runtime", () => {
  const certification = ProblemWorkspaceCertification;
  const source = readFileSync(
    new URL("./problemWorkspaceCertification.ts", import.meta.url),
    "utf8",
  );
  assert.equal(source.includes("./problemWorkspaceManifest"), false);
  assert.equal(source.includes("./problemWorkspaceValidation"), false);
  assert.deepEqual(certification.upstreamDependencies, [
    "WS-6:6 Problem Workspace Platform",
  ]);
  assert.equal(certification.dependencyVerification.state, "Verified");
  assert.equal(certification.runtime, false);
  assert.equal(certification.validators, false);
  assert.equal(certification.businessLogic, false);
});
