...
const files = [
  "decisionWorkspaceCertification.test.ts",
  "decisionWorkspaceCertification.ts",
  "decisionWorkspaceCertificationCriteria.ts",
  "decisionWorkspaceCertificationGates.ts",
  "decisionWorkspaceCertificationGuarantees.ts",
  "decisionWorkspaceCertificationIdentity.ts",
  "decisionWorkspaceCertificationReadiness.ts",
  "decisionWorkspaceCertificationResults.ts",
];

test("WS-4:7 consists of exactly eight Certification artifacts", () => {
  assert.deepEqual(
...
test("WS-4:7 publishes complete unique certification metadata", () => {
  const certification = DecisionWorkspaceCertification;
  assert.equal(
    certification.identity.id,
    "WS-4:7/DecisionWorkspaceCertification",
  );
  assert.equal(
...
test("WS-4:7 resolves all certification declarations", () => {
  const certification = DecisionWorkspaceCertification;
  assert.equal(
...
test("WS-4:7 ordering and summary counts are deterministic", () => {
  const certification = DecisionWorkspaceCertification;
  assert.deepEqual(
...
test("WS-4:7 consumes only Platform and contains no runtime", () => {
  const certification = DecisionWorkspaceCertification;
  const source = readFileSync(
    new URL("./decisionWorkspaceCertification.ts", import.meta.url),
    "utf8",
  );
