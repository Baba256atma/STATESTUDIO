/** WS-5:4 — Immutable outcomes and severities. */
...
export const ScenarioWorkspaceValidationOutcomes = Object.freeze(
  outcomeDefinitions.map(
    ([name, terminal, readinessEffect], index) => Object.freeze({
      id: `WS-5:4/Outcome/${String(index + 1).padStart(2, "0")}`,
      name,
...
export const ScenarioWorkspaceValidationSeverities = Object.freeze(
  ["Informational", "Low", "Medium", "High", "Critical"].map(
    (name, index) => Object.freeze({
      id: `WS-5:4/Severity/${String(index + 1).padStart(2, "0")}`,
      name,
