 ...
 */
import { EXECUTIVE_INTENT_DEFAULT_LIMITS, EXECUTIVE_INTENT_MANDATORY_METADATA_FIELDS, EXECUTIVE_INTENT_RESERVED_IDS, EXECUTIVE_INTENT_RESERVED_NAMESPACES, INTENT_CATEGORY_KEYS, INTENT_LIFECYCLE_KEYS, INTENT_PRIORITY_KEYS, INTENT_RELATION_TYPE_KEYS, INTENT_SCOPE_KEYS, INTENT_SOURCE_KEYS, INTENT_STATUS_KEYS, } from "./executiveIntentConstants.ts";
...
export function isIntentCategory(value) {
    return INTENT_CATEGORY_KEYS.includes(value);
}
export function isIntentPriority(value) {
    return INTENT_PRIORITY_KEYS.includes(value);
}
export function isIntentStatus(value) {
    return INTENT_STATUS_KEYS.includes(value);
}
export function isIntentScope(value) {
    return INTENT_SCOPE_KEYS.includes(value);
}
export function isIntentLifecycleStage(value) {
    return INTENT_LIFECYCLE_KEYS.includes(value);
}
export function isIntentSource(value) {
    return INTENT_SOURCE_KEYS.includes(value);
}
export function isIntentRelationType(value) {
    return INTENT_RELATION_TYPE_KEYS.includes(value);
}
export function isNonEmptyString(value) {
    return typeof value === "string" && value.trim().length > 0;
}
export function isReservedIntentId(intentId) {
    return EXECUTIVE_INTENT_RESERVED_IDS.includes(intentId);
}
export function isReservedIntentNamespace(namespace) {
    return EXECUTIVE_INTENT_RESERVED_NAMESPACES.includes(namespace);
}
export function hasDuplicateIds(ids) {
    const seen = new Set();
    for (const id of ids) {
        if (seen.has(id))
            return true;
...
export function validateStringLength(value, maxLength, field) {
    if (value.length > maxLength) {
        return issue("length_exceeded", `${field} exceeds maximum length of ${maxLength}.`, field);
    }
    return null;
}
export function validateIntentMetadataShape(input) {
    const issues = [];
    ...
    if (input.title !== undefined) {
        const lengthIssue = validateStringLength(input.title, EXECUTIVE_INTENT_DEFAULT_LIMITS.maxTitleLength, "title");
        if (lengthIssue)
            issues.push(lengthIssue);
    }
    if (input.summary !== undefined) {
        const lengthIssue = validateStringLength(input.summary, EXECUTIVE_INTENT_DEFAULT_LIMITS.maxSummaryLength, "summary");
        if (lengthIssue)
            issues.push(lengthIssue);
    }
    if (input.description !== undefined) {
        const lengthIssue = validateStringLength(input.description, EXECUTIVE_INTENT_DEFAULT_LIMITS.maxDescriptionLength, "description");
        if (lengthIssue)
            issues.push(lengthIssue);
    }
...
export function validateExecutiveIntentShape(input) {
    const issues = [];
    if (!isNonEmptyString(input.intentId)) {
    ...
    if (input.metadata !== undefined) {
        ...
        if (input.intentId && input.metadata.intentId && input.intentId !== input.metadata.intentId) {
            issues.push(issue("identity_mismatch", "intentId must match metadata.intentId.", "intentId"));
        }
        if (input.workspaceId &&
            input.metadata.workspaceId &&
            input.workspaceId !== input.metadata.workspaceId) {
            issues.push(issue("identity_mismatch", "workspaceId must match metadata.workspaceId.", "workspaceId"));
        }
    }
    else {
    ...
    if (input.relations !== undefined) {
        ...
        for (const relation of input.relations) {
            if (!isIntentRelationType(relation.relationType)) {
                issues.push(issue("invalid_relation_type", "relationType is not recognized.", "relations"));
            }
        }
    }
    if (input.readOnly !== undefined && input.readOnly !== true) {
...
export function validateCustomMetadataKeys(customMetadata) {
    const issues = [];
    ...
    for (const key of keys) {
        if (isReservedIntentNamespace(key)) {
            issues.push(issue("reserved_namespace", `customMetadata key "${key}" uses a reserved namespace.`, "customMetadata"));
        }
        const value = customMetadata[key];
        if (value.length > EXECUTIVE_INTENT_DEFAULT_LIMITS.maxCustomMetadataValueLength) {
            issues.push(issue("length_exceeded", `customMetadata value for "${key}" exceeds maximum length.`, "customMetadata"));
        }
    }
