...
export const EXECUTIVE_INTENT_CLASSIFICATION_RULES_VERSION = "APP-3/6-RULES-1";
...
const DIMENSION_TO_CLASS = Object.freeze({
    financial: "financial",
...
const ACTION_TO_CLASS = Object.freeze({
...
const KEYWORD_TO_CLASS = Object.freeze({
    profit: "financial",
    revenue: "financial",
    cost: "financial",
    cash: "financial",
...
function addCandidate(candidates, classId, ruleId, source, priority) {
    candidates.push(Object.freeze({ classId, ruleId, source, priority }));
}
export function resolveDimensionClass(dimension) {
    return DIMENSION_TO_CLASS[dimension] ?? "custom";
}
export function resolveActionClass(actionType) {
    return ACTION_TO_CLASS[actionType] ?? null;
}
export function resolveGoalClasses(goal) {
...
export function resolveKeywordClasses(model) {
    const corpus = [
...
export function resolveClassificationCandidates(model) {
    const candidates = [];
    ...
    const dimensionGroups = new Set(uniqueClasses.map((classId) => {
        if (classId === "growth" || classId === "strategic")
            return "strategic";
        if (classId === "transformation" || classId === "technology")
...
export function resolvePrimaryClassFromCandidates(candidates) {
    if (candidates.length === 0)
        return null;
    const sorted = [...candidates].sort((left, right) => {
        if (right.priority !== left.priority)
            return right.priority - left.priority;
        return (sortIntentClasses([left.classId, right.classId]).indexOf(left.classId) -
...
export function resolveSecondaryClassesFromCandidates(candidates, primaryClass) {
    const all = sortIntentClasses(candidates.map((entry) => entry.classId));
    if (!primaryClass)
        return all;
    return all.filter((classId) => classId !== primaryClass);
}
export function isSupportedActionType(actionType) {
    return SUPPORTED_ACTION_TYPES.includes(actionType);
}
export function collectRulesApplied(candidates) {
