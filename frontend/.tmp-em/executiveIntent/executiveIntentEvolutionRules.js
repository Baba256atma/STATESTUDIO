 ...
 */
export const EXECUTIVE_INTENT_EVOLUTION_RULES_VERSION = "APP-3/9-RULES-1";
...
export function extractExplicitLineageEdges(records) {
    const edges = [];
...
function edgesFromRelation(ownerIntentId, relation, knownIds) {
    const edges = [];
    const addBroken = (fromId, toId) => {
...
export function sortEdges(edges) {
    return Object.freeze([...edges].sort((left, right) => left.edgeId.localeCompare(right.edgeId)));
}
export function resolveIntentVersions(records, activeIntentId) {
...
export function resolveActiveIntentId(records, lineageIntentIds) {
    const candidates = records
        .filter((record) => lineageIntentIds.includes(record.intent.intentId))
        .filter((record) => record.intent.metadata.status === "active")
        .sort((left, right) => {
        const versionDiff = compareSemanticVersions(right.intent.metadata.version.semanticVersion, left.intent.metadata.version.semanticVersion);
        if (versionDiff !== 0)
            return versionDiff;
...
export function resolveRootIntentId(focusIntentId, edges, records) {
    ...
    if (current)
        return current;
    const explicitRoot = records.find((record) => record.intent.metadata.customMetadata["lineage.rootIntentId"])?.intent.metadata.customMetadata["lineage.rootIntentId"];
    return explicitRoot ?? focusIntentId;
}
export function resolveAncestors(focusIntentId, edges) {
    const ancestors = [];
    const queue = [{ intentId: focusIntentId, depth: 0 }];
    const visited = new Set();
    while (queue.length > 0) {
        const current = queue.shift();
        for (const edge of edges) {
            if (edge.toIntentId !== current.intentId)
                continue;
            if (edge.relationship !== "parent" &&
                edge.relationship !== "supersedes" &&
                edge.relationship !== "split_from" &&
    ...
    return Object.freeze(ancestors.sort((left, right) => left.depth - right.depth || left.intentId.localeCompare(right.intentId)));
}
export function resolveDescendants(focusIntentId, edges) {
    const descendants = [];
    const queue = [{ intentId: focusIntentId, depth: 0 }];
    const visited = new Set();
    while (queue.length > 0) {
        const current = queue.shift();
        for (const edge of edges) {
            if (edge.fromIntentId !== current.intentId)
                continue;
            if (edge.relationship !== "parent" &&
                edge.relationship !== "supersedes" &&
                edge.relationship !== "split_from" &&
    ...
    return Object.freeze(descendants.sort((left, right) => left.depth - right.depth || left.intentId.localeCompare(right.intentId)));
}
function mapToAncestorRelationship(relationship) {
    if (relationship === "supersedes")
        return "ancestor";
    if (relationship === "merged_into")
        return "ancestor";
...
function mapToDescendantRelationship(relationship) {
    if (relationship === "supersedes")
        return "descendant";
    if (relationship === "merged_into")
        return "descendant";
...
export function resolveSplitHistory(records) {
    ...
    for (const record of records) {
        const parentId = record.intent.metadata.customMetadata["lineage.splitFrom"] ??
    ...
    return Object.freeze([...splitMap.entries()]
        .map(([parentIntentId, childIntentIds]) => Object.freeze({
        splitId: deterministicId("split", parentIntentId),
        parentIntentId,
        childIntentIds: Object.freeze([...childIntentIds].sort()),
        timestamp: records.find((record) => record.intent.intentId === parentIntentId)?.intent.metadata.updatedAt ??
            "",
        readOnly: true,
    }))
        .sort((left, right) => left.splitId.localeCompare(right.splitId)));
}
export function resolveReplacementHistory(records, edges) {
    const replacements = [];
    for (const edge of edges) {
        ...
        replacements.push(Object.freeze({
            ...
            timestamp: records.find((record) => record.intent.intentId === edge.toIntentId)?.intent.metadata.updatedAt ??
                "",
            readOnly: true,
        }));
    }
    return Object.freeze(replacements.sort((left, right) => left.replacementId.localeCompare(right.replacementId)));
}
export function buildEvolutionEvents(records, edges, focusIntentId, timestamp) {
    const events = [];
    ...
    for (const edge of edges) {
        if (edge.toIntentId !== focusIntentId && edge.fromIntentId !== focusIntentId)
            continue;
        if (edge.relationship === "supersedes" || edge.relationship === "replaced_by") {
            events.push(createEvent({
                intentId: edge.toIntentId,
            ...
            events.push(createEvent({
                ...
                timestamp: intentMap.get(edge.fromIntentId)?.metadata.updatedAt ?? timestamp,
            }));
        }
...
export function sortEvents(events) {
    return Object.freeze([...events].sort((left, right) => {
        ...
        if (timeDiff !== 0)
            return timeDiff;
        const kindDiff = EVOLUTION_EVENT_ORDER.indexOf(left.kind) - EVOLUTION_EVENT_ORDER.indexOf(right.kind);
        if (kindDiff !== 0)
            return kindDiff;
        return left.eventId.localeCompare(right.eventId);
    }));
}
export function buildEvolutionRevisions(events, records) {
    const versionByIntent = new Map(records.map((record) => [record.intent.intentId, record.intent.metadata.version.semanticVersion]));
...
export function buildLineageGraph(request) {
...
function collectLineageIntentIds(focusIntentId, edges) {
    const ids = new Set([focusIntentId]);
    for (const edge of edges) {
...
export function buildEvolutionTimelineFromParts(input) {
    const orderedIntentIds = Object.freeze([...new Set(input.events.map((event) => event.intentId))].sort());
    return Object.freeze({
...
export function collectEvolutionRulesApplied(edges, events) {
    return Object.freeze([...new Set([...edges.map((edge) => edge.ruleId), ...events.map((event) => event.ruleId)])].sort());
}
export function hasMultipleParents(focusIntentId, edges) {
    const parents = edges.filter((edge) => edge.toIntentId === focusIntentId &&
...
export function hasBrokenLineage(edges) {
    return edges.some((edge) => edge.ruleId === "RULE_BROKEN_LINEAGE" || edge.relationship === "unknown");
}
export function resolveIntentVersionForRecord(record, isActive) {
    return Object.freeze({
        versionId: record.intent.metadata.version.versionId,
        intentId: record.intent.intentId,
        semanticVersion: record.intent.metadata.version.semanticVersion,
...
export function resolveSiblingIntentIds(focusIntentId, edges) {
    const parents = edges
