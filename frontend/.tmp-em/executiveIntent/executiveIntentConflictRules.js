 ...
 */
export const EXECUTIVE_INTENT_CONFLICT_RULES_VERSION = "APP-3/7-RULES-1";
...
const OPPOSED_ACTIONS = Object.freeze({
...
export function buildConflictReference(model) {
...
export function measuresOverlap(left, right) {
...
export function actionsOppose(left, right) {
    return (OPPOSED_ACTIONS[left] ?? []).includes(right) || (OPPOSED_ACTIONS[right] ?? []).includes(left);
}
export function actorsOverlap(left, right) {
...
export function timeHorizonsOverlap(left, right) {
    if (left.timeHorizon.kind === "unknown" || right.timeHorizon.kind === "unknown")
        return false;
    return left.timeHorizon.kind === right.timeHorizon.kind;
}
function createConflict(left, right, category, severity, ruleId, summary, explanation) {
    return Object.freeze({
...
export function detectPairConflicts(input) {
    const conflicts = [];
    ...
    if (leftGoal &&
        ...
        sameTarget) {
        conflicts.push(createConflict(leftRef, rightRef, "duplicate", "high", "RULE_DUPLICATE_INTENT", "Duplicate executive intent detected.", "Both intents express the same goal, action, and target entity."));
    }
    if (sameTarget && actionsOppose(left.actionType, right.actionType)) {
        conflicts.push(createConflict(leftRef, rightRef, "target", sameMeasure ? "critical" : "high", "RULE_GOAL_CONTRADICTION", "Opposing actions target the same entity.", `Action ${left.actionType} conflicts with ${right.actionType} on shared target.`));
    }
    if (sameMeasure && actionsOppose(left.actionType, right.actionType)) {
        conflicts.push(createConflict(leftRef, rightRef, "target", "critical", "RULE_ACTION_OPPOSITION", "Opposing actions apply to the same measure.", `Measure-level opposition between ${left.actionType} and ${right.actionType}.`));
    }
    if (sameTarget && !actionsOppose(left.actionType, right.actionType) && leftGoal !== rightGoal) {
        conflicts.push(createConflict(leftRef, rightRef, "target", "medium", "RULE_TARGET_SHARED", "Shared target entity with distinct goals.", "Intents overlap on target entity but actions are not strictly opposed."));
    }
    if (actorsOverlap(left, right)) {
        conflicts.push(createConflict(leftRef, rightRef, "resource", "medium", "RULE_RESOURCE_ACTOR_OVERLAP", "Shared actors referenced by both intents.", "Both intents reference the same explicit actor."));
    }
    ...
    for (const [markerA, markerB] of CONSTRAINT_OPPOSITION_MARKERS) {
        ...
        if ((leftHasA && rightHasB) || (leftHasB && rightHasA)) {
            conflicts.push(createConflict(leftRef, rightRef, "constraint", "medium", "RULE_CONSTRAINT_OPPOSITION", "Explicit constraints oppose each other.", `Constraint marker "${markerA}" conflicts with "${markerB}".`));
        }
    }
    if ((combinedLeft.includes("budget") || combinedLeft.includes("cost")) &&
        (combinedRight.includes("budget") || combinedRight.includes("cost")) &&
        actionsOppose(left.actionType, right.actionType)) {
        conflicts.push(createConflict(leftRef, rightRef, "financial", "high", "RULE_RESOURCE_KEYWORD", "Shared budget or cost resource tension.", "Both intents reference financial resources with opposing actions."));
    }
    if (timeHorizonsOverlap(left, right) && (sameTarget || sameMeasure)) {
        conflicts.push(createConflict(leftRef, rightRef, "time", "medium", "RULE_TIME_SAME_HORIZON", "Same time horizon with overlapping targets.", `Both intents target the same horizon (${left.timeHorizon.label}).`));
    }
    else if (timeHorizonsOverlap(left, right)) {
        conflicts.push(createConflict(leftRef, rightRef, "time", "informational", "RULE_TIME_OVERLAP", "Timeline overlap detected.", "Intents share the same time horizon."));
    }
    ...
    for (const leftAssumption of leftAssumptions) {
        for (const rightAssumption of rightAssumptions) {
            if (leftAssumption &&
                ...
                conflicts.push(createConflict(leftRef, rightRef, "assumption", "low", "RULE_ASSUMPTION_OPPOSITION", "Assumptions may be incompatible.", "Explicit assumptions contain opposing market signals."));
            }
        }
    }
    ...
    if (leftClass?.primaryClass && rightClass?.primaryClass) {
        ...
        const growthVsCost = (leftPrimary === "growth" && rightPrimary === "financial" && right.actionType === "reduce") ||
            (rightPrimary === "growth" && leftPrimary === "financial" && left.actionType === "reduce");
        if (growthVsCost) {
            conflicts.push(createConflict(leftRef, rightRef, "strategic", "high", "RULE_GROWTH_VS_COST", "Growth objective conflicts with cost reduction.", "Classification and action types indicate growth vs cost tension."));
        }
        const techReplace = leftPrimary === "technology" &&
        ...
        if (techReplace && left.modelId !== right.modelId) {
            conflicts.push(createConflict(leftRef, rightRef, "technology", "medium", "RULE_TECHNOLOGY_REPLACEMENT", "Technology replacement overlap detected.", "Both intents involve technology replacement or transformation."));
        }
        const complianceTension = leftPrimary === "compliance" &&
            rightPrimary === "compliance" &&
            left.actionType !== right.actionType;
        if (complianceTension) {
            conflicts.push(createConflict(leftRef, rightRef, "compliance", "medium", "RULE_COMPLIANCE_TENSION", "Compliance objectives use different action types.", "Multiple compliance intents with differing actions may conflict."));
        }
        if (leftPrimary !== rightPrimary &&
            actionsOppose(left.actionType, right.actionType) &&
            left.businessDimension === right.businessDimension) {
            conflicts.push(createConflict(leftRef, rightRef, "strategic", "low", "RULE_CLASSIFICATION_TENSION", "Classification categories differ with opposing actions.", `Classes ${leftPrimary} and ${rightPrimary} tension detected.`));
        }
    }
    ...
    if (leftState?.state.flags.isBlocked && rightState?.state.flags.isBlocked) {
        conflicts.push(createConflict(leftRef, rightRef, "operational", "informational", "RULE_STATE_BLOCKED", "Both intents are in blocked state.", "Blocked state on both intents may indicate operational conflict."));
    }
    if ((left.flags.incompleteObjective || right.flags.incompleteObjective) &&
        conflicts.length === 0) {
        conflicts.push(createConflict(leftRef, rightRef, "unknown", "unknown", "RULE_UNKNOWN_COMPARISON", "Insufficient semantic information for full conflict analysis.", "One or both intents are incomplete; conflict status is unknown."));
    }
    return sortConflicts(conflicts);
}
export function sortConflicts(conflicts) {
    return Object.freeze([...conflicts].sort((left, right) => {
        const severityDiff = CONFLICT_SEVERITY_ORDER.indexOf(right.severity) -
            CONFLICT_SEVERITY_ORDER.indexOf(left.severity);
        if (severityDiff !== 0)
            return severityDiff;
        const categoryDiff = CONFLICT_CATEGORY_ORDER.indexOf(left.category) -
...
export function highestConflictSeverity(conflicts) {
    if (conflicts.length === 0)
        return "none";
    const sorted = sortConflicts(conflicts);
    return sorted[0].severity;
}
export function collectConflictRulesApplied(conflicts) {
