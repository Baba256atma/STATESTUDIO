...
import { createExecutiveIntentDiagnostic, } from "./executiveIntentDiagnostics.ts";
import { resolveLifecycleTransition } from "./executiveIntentLifecycleMatrix.ts";
import { hasDuplicateIds, isIntentRelationType, validateExecutiveIntentShape, validateIntentMetadataShape, } from "./executiveIntentValidation.ts";
import { resolveExecutiveIntentExample } from "./executiveIntentContract.ts";
import { createExecutiveIntentState, createIntentResolutionResult, createIntentStateSummary, EXECUTIVE_INTENT_STATE_ENGINE_VERSION, } from "./executiveIntentStateTypes.ts";
export const EXECUTIVE_INTENT_STATE_ENGINE_OWNER = "executive-intent-state-engine";
...
function hasBlockingRelation(intent) {
    return intent.relations.some((relation) => relation.relationType === "blocks");
}
function hasBlockingDependency(intent) {
    return intent.metadata.dependencies.some((dependency) => dependency.relationType === "blocks");
}
function isStatusLifecycleConsistent(intent) {
    ...
    if (status === "draft")
        return lifecycle === "created" || lifecycle === "validated";
    if (status === "active") {
...
export function resolveIntentStructuralHealth(intent, diagnostics) {
    if (!intent)
        return "unknown";
    ...
    if (intent.contractVersion !== EXECUTIVE_INTENT_CONTRACT_VERSION)
        return "invalid";
    if (!validateExecutiveIntentShape(intent).valid)
        return "invalid";
    if (!validateIntentMetadataShape(intent.metadata).valid)
        return "invalid";
    if (diagnostics.some((entry) => entry.blocking && entry.severity === "error"))
        return "invalid";
    if (diagnostics.some((entry) => entry.severity === "warning"))
        return "warning";
    return "healthy";
}
export function resolveIntentFreshness(intent, evaluatedAt) {
    if (!intent)
        return "unknown";
    const evaluatedMs = parseTimestamp(evaluatedAt);
    const updatedMs = parseTimestamp(intent.metadata.updatedAt);
    if (evaluatedMs === null || updatedMs === null)
        return "unknown";
    const ageMs = Math.max(0, evaluatedMs - updatedMs);
    if (ageMs <= FRESHNESS_FRESH_MS)
        return "fresh";
    if (ageMs <= FRESHNESS_RECENT_MS)
        return "recent";
    if (ageMs <= FRESHNESS_AGING_MS)
        return "aging";
    if (ageMs <= FRESHNESS_STALE_MS)
        return "stale";
    return "expired";
}
export function resolveIntentDiagnostics(request) {
    return evaluateExecutiveIntentStateContext(request).diagnostics;
}
function resolveExecutionState(intent, readiness, structuralHealth) {
    if (!intent)
        return "unknown";
    if (readiness === "archived" || readiness === "completed")
        return "terminal";
    ...
    if (intent.metadata.lifecycle === "approved")
        return "awaiting_activation";
    if (intent.metadata.status === "paused")
        return "paused";
    if (readiness === "ready")
        return "active";
    return "idle";
}
function resolveReadiness(intent, structuralHealth, diagnostics, workspaceIsolated) {
    if (!intent)
        return "not_ready";
    ...
    if (intent.metadata.status === "paused")
        return "waiting";
    if (intent.metadata.status === "active" &&
...
function resolveStateCategory(intent, readiness, structuralHealth) {
    if (!intent)
        return "unknown";
    if (structuralHealth === "invalid" || structuralHealth === "corrupted")
        return "invalid";
    ...
    if (intent.metadata.status === "paused")
        return "paused";
    if (intent.metadata.status === "draft")
        return "draft";
    if (readiness === "ready")
        return "ready";
    if (structuralHealth === "healthy" || structuralHealth === "warning")
        return "valid";
    return "unknown";
}
export function resolveIntentReadiness(request) {
    const context = evaluateExecutiveIntentStateContext(request);
    return context.readiness;
}
export function isIntentReady(request) {
    return resolveIntentReadiness(request) === "ready";
}
export function isIntentBlocked(request) {
    const readiness = resolveIntentReadiness(request);
    return readiness === "blocked";
}
export function isIntentArchived(request) {
    const readiness = resolveIntentReadiness(request);
    return readiness === "archived";
}
export function isIntentActionable(request) {
    const context = evaluateExecutiveIntentStateContext(request);
    return (context.flags.isActionable &&
        context.readiness === "ready" &&
        context.structuralHealth === "healthy" &&
        context.freshness !== "stale" &&
        context.freshness !== "expired");
}
export function evaluateExecutiveIntentStateContext(request) {
    const diagnostics = [];
    ...
    let lifecycleValidation = request.proposedLifecycleTransition
        ? resolveLifecycleTransition(request.proposedLifecycleTransition.from, request.proposedLifecycleTransition.to)
        : null;
    if (!intent) {
        pushDiagnostic(diagnostics, "intent_missing", "Executive Intent is unavailable.", evaluatedAt, Object.freeze({
    ...
    if (intent.readOnly !== true || intent.metadata.readOnly !== true) {
        pushDiagnostic(diagnostics, "intent_read_only_violation", "Executive Intent must remain read-only.", evaluatedAt, Object.freeze({ blocking: true }));
    }
    if (intent.contractVersion !== EXECUTIVE_INTENT_CONTRACT_VERSION) {
        pushDiagnostic(diagnostics, "intent_unsupported_version", `Unsupported contract version "${intent.contractVersion}".`, evaluatedAt, Object.freeze({
            blocking: true,
            metadata: Object.freeze({ expected: EXECUTIVE_INTENT_CONTRACT_VERSION }),
        }));
    }
    if (intent.intentId !== request.intentId) {
        pushDiagnostic(diagnostics, "intent_invalid_metadata", "Request intentId does not match ExecutiveIntent.intentId.", evaluatedAt, Object.freeze({ blocking: true }));
    }
    if (intent.workspaceId !== workspaceId || intent.metadata.workspaceId !== workspaceId) {
        workspaceIsolated = false;
        pushDiagnostic(diagnostics, "intent_workspace_mismatch", "Workspace isolation violation detected.", evaluatedAt, Object.freeze({
            blocking: true,
    ...
    if (!metadataValidation.valid) {
        pushDiagnostic(diagnostics, "intent_invalid_metadata", "Executive Intent metadata failed structural validation.", evaluatedAt, Object.freeze({
            blocking: true,
    ...
    if (!intentValidation.valid) {
        pushDiagnostic(diagnostics, "intent_invalid_metadata", "Executive Intent failed structural validation.", evaluatedAt, Object.freeze({
            blocking: true,
            metadata: Object.freeze({ issueCount: intentValidation.issues.length }),
        }));
    }
    if (intent.metadata.dependencies.length > 0 &&
        hasDuplicateIds(intent.metadata.dependencies.map((entry) => entry.dependencyId))) {
        pushDiagnostic(diagnostics, "intent_duplicate_dependency", "Duplicate dependency identifiers detected.", evaluatedAt, Object.freeze({ blocking: true }));
    }
    for (const relation of intent.relations) {
        if (!isIntentRelationType(relation.relationType)) {
            pushDiagnostic(diagnostics, "intent_invalid_relation", `Invalid relation type "${relation.relationType}".`, evaluatedAt, Object.freeze({ blocking: true, metadata: Object.freeze({ relationId: relation.relationId }) }));
        }
    }
    if (hasBlockingRelation(intent) || hasBlockingDependency(intent)) {
        pushDiagnostic(diagnostics, "intent_blocked", "Intent has blocking relations or dependencies.", evaluatedAt, Object.freeze({
            blocking: true,
            recommendedNextState: "blocked",
        }));
    }
    if (intent.metadata.status === "archived" || intent.metadata.lifecycle === "archived") {
        pushDiagnostic(diagnostics, "intent_archived", "Intent is archived.", evaluatedAt, Object.freeze({ recommendedNextState: "archived" }));
    }
    if (!isStatusLifecycleConsistent(intent)) {
        pushDiagnostic(diagnostics, "intent_status_lifecycle_mismatch", "Intent status and lifecycle stage are inconsistent.", evaluatedAt, Object.freeze({
            metadata: Object.freeze({
    ...
    if (intent.metadata.title.trim().length === 0 ||
        intent.metadata.summary.trim().length === 0) {
        pushDiagnostic(diagnostics, "intent_incomplete", "Intent title or summary is incomplete.", evaluatedAt, Object.freeze({ recommendedNextState: "valid" }));
    }
    const freshness = resolveIntentFreshness(intent, evaluatedAt);
    if (freshness === "stale" || freshness === "expired") {
        pushDiagnostic(diagnostics, "intent_stale", `Intent freshness is ${freshness}.`, evaluatedAt, Object.freeze({
            recommendedNextState: "valid",
            metadata: Object.freeze({ freshness }),
        }));
    }
    if (lifecycleValidation && !lifecycleValidation.allowed) {
        pushDiagnostic(diagnostics, "intent_illegal_lifecycle_transition", lifecycleValidation.reason, evaluatedAt, Object.freeze({
            blocking: true,
            metadata: Object.freeze({
    ...
    if (diagnostics.length === 0 ||
        ...
        pushDiagnostic(diagnostics, "intent_ok", "Executive Intent state resolved successfully.", evaluatedAt, Object.freeze({ recommendedNextState: "ready" }));
    }
    ...
    const flags = Object.freeze({
        ...
        isDownstreamReady: workspaceIsolated &&
            structuralHealth !== "invalid" &&
            structuralHealth !== "corrupted" &&
            structuralHealth !== "unknown",
        workspaceIsolated,
...
export function resolveIntentStateSummary(request) {
    const context = evaluateExecutiveIntentStateContext(request);
    const blockingDiagnosticCount = context.diagnostics.filter((entry) => entry.blocking).length;
    const headline = context.request.intent?.metadata.title ??
...
export function resolveExecutiveIntentState(request) {
    const context = evaluateExecutiveIntentStateContext(request);
...
export function resolveExecutiveIntentStateResult(request) {
    const context = evaluateExecutiveIntentStateContext(request);
...
export function resolveExecutiveIntentStateProbeExample(evaluatedAt = new Date(0).toISOString()) {
    const intent = resolveExecutiveIntentExample(evaluatedAt);
    return resolveExecutiveIntentStateResult(Object.freeze({
        intent,
...
export function getExecutiveIntentStateEngineVersionMetadata() {
    return Object.freeze({
