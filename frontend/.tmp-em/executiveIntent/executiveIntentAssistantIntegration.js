 ...
 */
import { createAssistantIntentDiagnostic, } from "./executiveIntentAssistantDiagnostics.ts";
import { getExecutiveIntentAssistantCanonicalExample } from "./executiveIntentAssistantExamples.ts";
import { CLARIFICATION_QUESTION_TEMPLATES, buildSectionFromTemplate, mapReasoningStatusToAssistantStatus, templateClassificationExplanation, templateConfidenceExplanation, templateConflictExplanation, templateDependencyExplanation, templateDiagnosticsBody, templateEvolutionExplanation, templateHighlights, templateIntentExplanation, templateKnownInformation, templateNoIntentSummary, templateOpenIssues, templateOverviewSummary, templateStateExplanation, templateUnknownInformation, } from "./executiveIntentAssistantTemplates.ts";
import { createAssistantIntentResponse, EXECUTIVE_INTENT_ASSISTANT_INTEGRATION_VERSION, } from "./executiveIntentAssistantTypes.ts";
...
export const EXECUTIVE_INTENT_ASSISTANT_INTEGRATION_OWNER = "executive-intent-assistant";
...
export function buildIntentStatus(reasoning) {
    if (!reasoning)
        return "unknown";
    return mapReasoningStatusToAssistantStatus(reasoning.readinessAssessment.state);
}
export function buildIntentSummary(reasoning) {
    if (!reasoning)
        return templateNoIntentSummary();
    return templateOverviewSummary(reasoning);
}
export function buildIntentHighlights(reasoning) {
    if (!reasoning)
        return Object.freeze([]);
    return templateHighlights(reasoning);
}
export function buildIntentClarificationQuestions(reasoning) {
    if (!reasoning)
        return Object.freeze([]);
    const questions = [];
    const seen = new Set();
    const addQuestion = (questionType, context) => {
    ...
    if (reasoning.readinessAssessment.state === "needs_clarification" &&
        questions.length === 0) {
        addQuestion("general_unknown", reasoning.readinessAssessment.explanation);
    }
    return Object.freeze(questions);
}
export function buildIntentWarnings(reasoning) {
    if (!reasoning)
        return Object.freeze([]);
    const warnings = [];
...
export function buildIntentExplanation(reasoning) {
    if (!reasoning)
        return Object.freeze([]);
    const explanations = [
        Object.freeze({
...
function buildAssistantSections(reasoning, questions) {
    if (!reasoning) {
        return Object.freeze([
    ...
    return Object.freeze([
        ...
        buildSectionFromTemplate({
            ...
            body: templateClassificationExplanation(reasoning),
            available: reasoning.sections.some((entry) => entry.sectionKey === "primary_classification" && entry.available),
        }),
        buildSectionFromTemplate({
            sectionId: deterministicId("assistant-section", `confidence:${timestamp}`),
            sectionKey: "confidence",
            body: templateConfidenceExplanation(reasoning),
            available: reasoning.sections.some((entry) => entry.sectionKey === "confidence_summary" && entry.available),
        }),
        buildSectionFromTemplate({
        ...
        buildSectionFromTemplate({
            ...
            available: reasoning.sections.some((entry) => entry.sectionKey === "known_information" && entry.available),
        }),
        buildSectionFromTemplate({
...
function buildClarifications(reasoning, questions) {
    if (!reasoning || questions.length === 0)
        return null;
    return Object.freeze({
        clarificationId: deterministicId("assistant-clarification", reasoning.reasoningId),
        required: reasoning.readinessAssessment.state === "needs_clarification" ||
...
function resolveAssistantFlags(input) {
    return Object.freeze({
...
export function buildAssistantIntentResponse(reasoning, timestamp = reasoning?.timestamp ?? new Date(0).toISOString()) {
    const diagnostics = [];
    ...
    if (!reasoning) {
        pushDiagnostic(diagnostics, "no_executive_intent", "No executive intent reasoning model was provided.", timestamp);
        pushDiagnostic(diagnostics, "reasoning_unavailable", "Assistant cannot present intent without APP-3:11 reasoning.", timestamp);
    }
    else {
        ...
        if (flags.clarificationRequired) {
            pushDiagnostic(diagnostics, "clarification_required", "Clarification questions are available.", timestamp);
        }
    ...
    pushDiagnostic(diagnostics, "assistant_response_success", "Assistant response built deterministically from reasoning model.", timestamp);
    return createAssistantIntentResponse({
        responseId: deterministicId("assistant-intent-response", `${reasoning?.reasoningId ?? "none"}:${timestamp}`),
...
export function validateAssistantIntentResponse(response) {
    const issues = [];
    if (response.readOnly !== true)
        issues.push("Assistant response must be read-only.");
    if (response.flags.readOnly !== true)
        issues.push("Assistant flags must be read-only.");
    if (response.flags.deterministic !== true)
        issues.push("Assistant integration must be deterministic.");
...
export function buildAssistantExample(exampleId, workspaceId = "ws-example-001", owner = "executive-owner", generatedAt = new Date(0).toISOString()) {
    ...
    if (exampleId === "multiple-intents") {
        const diagnostics = Object.freeze([
            ...
            createAssistantIntentDiagnostic("multiple_intents_context", "Assistant presents one executive intent at a time.", generatedAt),
        ]);
        return createAssistantIntentResponse({
            ...response,
            diagnostics,
...
export function buildAssistantProbe(generatedAt = new Date(0).toISOString()) {
    const reasoning = buildReasoningProbe(generatedAt);
    return buildAssistantIntentResponse(reasoning, generatedAt);
}
export function getExecutiveIntentAssistantIntegrationVersionMetadata() {
    return Object.freeze({
        assistantIntegrationVersion: EXECUTIVE_INTENT_ASSISTANT_INTEGRATION_VERSION,
