 ...
 */
import { createDashboardIntentDiagnostic, } from "./executiveIntentDashboardDiagnostics.ts";
import { getExecutiveIntentDashboardCanonicalExample } from "./executiveIntentDashboardExamples.ts";
import { DASHBOARD_SECTION_TITLES, EXECUTIVE_INTENT_DASHBOARD_LAYOUT, } from "./executiveIntentDashboardLayouts.ts";
import { createDashboardIntentModel, EXECUTIVE_INTENT_DASHBOARD_INTEGRATION_VERSION, } from "./executiveIntentDashboardTypes.ts";
...
export const EXECUTIVE_INTENT_DASHBOARD_INTEGRATION_OWNER = "executive-intent-dashboard";
...
function sectionContent(reasoning, sectionKey) {
...
function sectionAvailable(reasoning, sectionKey) {
    if (sectionKey === "executive_summary" || sectionKey === "intent_overview")
        return true;
    if (sectionKey === "highlights")
        return reasoning.highlights.items.length > 0;
    if (sectionKey === "issues")
        return reasoning.issues.length > 0;
    if (sectionKey === "readiness")
        return true;
    if (sectionKey === "diagnostics")
        return reasoning.diagnostics.length > 0;
    if (sectionKey === "conflicts")
        return reasoning.flags.hasConflicts;
    if (sectionKey === "dependencies")
        return reasoning.flags.hasDependencies;
    if (sectionKey === "evolution")
        return reasoning.flags.hasEvolutionHistory;
    if (sectionKey === "unknown_information")
        return reasoning.unknowns.length > 0;
    const mapping = Object.freeze({
        current_state: "current_state",
        classification: "primary_classification",
        confidence: "confidence_summary",
...
function layoutPanelForSection(sectionKey) {
    const panel = EXECUTIVE_INTENT_DASHBOARD_LAYOUT.panels.find((entry) => entry.sectionKeys.includes(sectionKey));
    return panel?.panelId ?? null;
}
export function mapReasoningStatusToDashboardStatus(readinessState) {
...
export function buildDashboardStatus(reasoning) {
    if (!reasoning)
        return "unknown";
    return mapReasoningStatusToDashboardStatus(reasoning.readinessAssessment.state);
}
export function buildDashboardSections(reasoning) {
    if (!reasoning) {
    ...
    const sectionKeys = Object.keys(DASHBOARD_SECTION_TITLES);
    return Object.freeze(sectionKeys.map((sectionKey) => Object.freeze({
...
export function buildDashboardMetrics(reasoning) {
    if (!reasoning)
        return Object.freeze([]);
    const metrics = [];
    const addMetric = (metricKey, label, value, numericValue, unit) => {
    ...
    const confidenceSection = reasoning.sections.find((entry) => entry.sectionKey === "confidence_summary");
    const aggregateScoreMatch = confidenceSection?.content.match(/(\d+)\/100/);
    addMetric("confidence_score", "Confidence Score", aggregateScoreMatch?.[1] ?? reasoning.summary.confidenceLevel ?? "unknown", aggregateScoreMatch ? Number(aggregateScoreMatch[1]) : null, aggregateScoreMatch ? "score" : null);
    addMetric("conflict_count", "Conflict Count", String(reasoning.flags.hasConflicts ? reasoning.issues.filter((i) => i.issueKey === "conflicting_objectives").length || 1 : 0), reasoning.flags.hasConflicts ? Math.max(1, reasoning.issues.filter((i) => i.issueKey === "conflicting_objectives").length) : 0, "count");
    addMetric("dependency_count", "Dependency Count", String(reasoning.flags.hasDependencies ? 1 : 0), reasoning.flags.hasDependencies ? 1 : 0, "count");
    addMetric("unknown_count", "Unknown Count", String(reasoning.unknowns.length), reasoning.unknowns.length, "count");
    addMetric("evolution_depth", "Evolution Depth", String(reasoning.flags.hasEvolutionHistory ? 1 : 0), reasoning.flags.hasEvolutionHistory ? 1 : 0, "depth");
    addMetric("classification_count", "Classification Count", String(reasoning.summary.primaryClassification ? 1 + (reasoning.sections.find((s) => s.sectionKey === "secondary_classifications")?.content.split(",").length ?? 0) : 0), reasoning.summary.primaryClassification ? 1 : 0, "count");
    addMetric("readiness_state", "Readiness State", reasoning.readinessAssessment.state, null, null);
    addMetric("highlight_count", "Highlight Count", String(reasoning.highlights.items.length), reasoning.highlights.items.length, "count");
    addMetric("issue_count", "Issue Count", String(reasoning.issues.length), reasoning.issues.length, "count");
    return Object.freeze(metrics);
}
export function buildDashboardBadges(reasoning) {
    if (!reasoning)
        return Object.freeze([]);
    const badges = [];
    const addBadge = (badgeKey, label, active) => {
    ...
    addBadge("needs_clarification", "Needs Clarification", reasoning.readinessAssessment.state === "needs_clarification");
    ...
    addBadge("recently_updated", "Recently Updated", reasoning.highlights.items.some((item) => item.highlightKey === "recent_strategy_shift"));
...
function cardEmphasis(cardKey, reasoning) {
    if (cardKey === "conflict" && reasoning.flags.hasConflicts)
        return "warning";
    if (cardKey === "readiness" && reasoning.readinessAssessment.state === "blocked")
        return "critical";
    if (cardKey === "confidence" && reasoning.flags.lowConfidence)
        return "warning";
    if (cardKey === "executive_summary" || cardKey === "intent")
        return "primary";
    return "neutral";
}
export function buildDashboardCards(reasoning) {
    if (!reasoning)
        return Object.freeze([]);
    const cards = [
        {
...
export function buildDashboardWidgets(reasoning) {
    const widgets = [
        {
            widgetKey: "summary",
    ...
    return Object.freeze(widgets.map((widget) => Object.freeze({
...
export function buildDashboardSummary(reasoning, metrics, cards, badges) {
    if (!reasoning) {
        return Object.freeze({
...
function withSyntheticReadinessState(reasoning, state) {
    const readinessAssessment = Object.freeze({
        ...reasoning.readinessAssessment,
        state,
        headline: state === "archived"
            ? "Intent is archived."
            : state === "blocked"
                ? "Intent is blocked."
                : state === "ready"
...
function resolveDashboardFlags(input) {
    return Object.freeze({
...
export function buildDashboardIntentModel(reasoning, timestamp = reasoning?.timestamp ?? new Date(0).toISOString()) {
    const diagnostics = [];
    ...
    if (!reasoning) {
        pushDiagnostic(diagnostics, "reasoning_unavailable", "No executive intent reasoning model was provided.", timestamp);
    }
    else {
    ...
    pushDiagnostic(diagnostics, "dashboard_model_success", "Dashboard model built deterministically from reasoning.", timestamp);
    return createDashboardIntentModel({
        modelId: deterministicId("dashboard-intent-model", `${reasoning?.reasoningId ?? "none"}:${timestamp}`),
...
export function validateDashboardModel(model) {
    const issues = [];
    if (model.readOnly !== true)
        issues.push("Dashboard model must be read-only.");
    if (model.flags.readOnly !== true)
        issues.push("Dashboard flags must be read-only.");
    if (model.flags.deterministic !== true)
        issues.push("Dashboard integration must be deterministic.");
...
export function buildDashboardExample(exampleId, workspaceId = "ws-example-001", owner = "executive-owner", generatedAt = new Date(0).toISOString()) {
    ...
    const reasoning = buildReasoningExample(example.reasoningExampleId, workspaceId, owner, generatedAt);
    if (!reasoning)
        return null;
    const presentationReasoning = example.syntheticStatus
        ? withSyntheticReadinessState(reasoning, example.syntheticStatus)
        : reasoning;
    return buildDashboardIntentModel(presentationReasoning, generatedAt);
}
export function buildDashboardProbe(generatedAt = new Date(0).toISOString()) {
    const reasoning = buildReasoningProbe(generatedAt);
    return buildDashboardIntentModel(reasoning, generatedAt);
}
export function getExecutiveIntentDashboardIntegrationVersionMetadata() {
    return Object.freeze({
        dashboardIntegrationVersion: EXECUTIVE_INTENT_DASHBOARD_INTEGRATION_VERSION,
