 ...
 */
export const EXECUTIVE_INTENT_EXTRACTION_RULES_VERSION = "APP-3/4";
...
export const INTENT_EXTRACTION_NEUTRAL_LANGUAGE_ADAPTER = Object.freeze({
...
export const INTENT_EXTRACTION_ENGLISH_LANGUAGE_ADAPTER = Object.freeze({
    languageCode: "en",
    actionVerbs: Object.freeze([
        "increase",
        "decrease",
    ...
    normalizeText: (text) => text.trim().replace(/\s+/g, " ").toLowerCase(),
});
export const INTENT_EXTRACTION_SPANISH_LANGUAGE_ADAPTER = Object.freeze({
    languageCode: "es",
    actionVerbs: Object.freeze([
        "aumentar",
        "reducir",
...
export function resolveIntentExtractionLanguageAdapter(languageCode) {
    const normalized = languageCode.trim().toLowerCase();
    const adapter = INTENT_EXTRACTION_LANGUAGE_ADAPTERS.find((entry) => entry.languageCode === normalized);
    return adapter ?? INTENT_EXTRACTION_ENGLISH_LANGUAGE_ADAPTER;
}
export function findFirstMatchingKeyword(text, keywords) {
...
export function findActionVerb(text, adapter) {
...
export function resolveExplicitCategory(text, adapter) {
...
export function resolveExplicitPriority(text, adapter) {
    for (const [priority, keywords] of Object.entries(adapter.priorityKeywords)) {
        if (!keywords)
            continue;
        if (findFirstMatchingKeyword(text, keywords))
            return priority;
    }
    return null;
}
export function resolveExplicitScope(text, adapter) {
    for (const [scope, keywords] of Object.entries(adapter.scopeKeywords)) {
        if (!keywords)
            continue;
        if (findFirstMatchingKeyword(text, keywords))
            return scope;
