...
import { EXECUTIVE_MEMORY_CATEGORY_KEYS, EXECUTIVE_MEMORY_CONTRACT_VERSION, EXECUTIVE_MEMORY_FUTURE_COMPATIBILITY, EXECUTIVE_MEMORY_FUTURE_PHASE_KEYS, EXECUTIVE_MEMORY_MANDATORY_METADATA_FIELDS, EXECUTIVE_MEMORY_MUST_NOT_OWN, EXECUTIVE_MEMORY_RESERVED_MEMORY_IDS, EXECUTIVE_MEMORY_RESERVED_PROVIDER_IDS, EXECUTIVE_MEMORY_TAGS, } from "./executiveMemoryConstants.ts";
import { EXECUTIVE_MEMORY_FREEZE_RULES, EXECUTIVE_MEMORY_IDENTITY, EXECUTIVE_MEMORY_PUBLIC_API_RULES, EXECUTIVE_MEMORY_SELF_MANIFEST, ExecutiveMemoryContract, getExecutiveMemoryContractVersionMetadata, getExecutiveMemoryFutureCompatibility, isExecutiveMemoryCategory, resolveExecutiveMemoryExample, resolveExecutiveMemoryMetadataExample, resolveExecutiveMemoryProviderExample, validateExecutiveMemoryMetadataShape, validateExecutiveMemoryShape, } from "./executiveMemoryContracts.ts";
import { getExecutiveMemoryPlatformState, initializeExecutiveMemoryPlatform, isExecutiveMemoryPlatformInitialized, resetExecutiveMemoryFoundationForTests, } from "./executiveMemoryFoundation.ts";
import { ExecutiveMemoryPlatform, getExecutiveMemoryProvider, getExecutiveMemoryProviders, isExecutiveMemoryRegistered, registerExecutiveMemoryProvider, resetExecutiveMemoryPlatformForTests, } from "./executiveMemoryPlatform.ts";
import { getExecutiveMemoryRegistryMetadata, listExecutiveMemoryProviderIds, } from "./executiveMemoryRegistry.ts";
import { hasDuplicateProviderIds, isReservedExecutiveMemoryId, isReservedExecutiveMemoryProviderId, validateExecutiveMemoryProviderRegistration, } from "./executiveMemoryValidation.ts";
...
test("exports APP-4 identity and contract vocabulary", () => {
    ...
    for (const tag of [
        ...
        assert.ok(EXECUTIVE_MEMORY_TAGS.includes(tag), tag);
    }
});
...
test("registers and retrieves executive memory providers", () => {
    ...
    const registration = registerExecutiveMemoryProvider(Object.freeze({
        providerId: "provider-decision-archive",
        label: "Decision Archive Provider",
        version: "1.0.0",
        supportedCategories: Object.freeze(["decision", "evidence"]),
...
test("blocks duplicate provider registration", () => {
    const input = Object.freeze({
        ...
        version: "1.0.0",
        supportedCategories: Object.freeze(["goal"]),
    });
...
test("rejects reserved provider IDs", () => {
    for (const providerId of EXECUTIVE_MEMORY_RESERVED_PROVIDER_IDS) {
        ...
        const result = registerExecutiveMemoryProvider(Object.freeze({
            providerId,
            label: "Reserved Provider",
            version: "1.0.0",
            supportedCategories: Object.freeze(["metadata"]),
        }), FIXED_TIME);
...
test("rejects empty provider category list", () => {
    const validation = validateExecutiveMemoryProviderRegistration(Object.freeze({
...
test("provider lookup works and registry remains stable", () => {
    initializeExecutiveMemoryPlatform(FIXED_TIME);
    registerExecutiveMemoryProvider(Object.freeze({
        providerId: "provider-alpha",
        label: "Alpha Provider",
        version: "1.0.0",
        supportedCategories: Object.freeze(["scenario"]),
    }), FIXED_TIME);
    registerExecutiveMemoryProvider(Object.freeze({
        providerId: "provider-beta",
        label: "Beta Provider",
        version: "1.0.0",
        supportedCategories: Object.freeze(["intent"]),
    }), FIXED_TIME);
