 ...
 */
import { EXECUTIVE_MEMORY_CATEGORY_KEYS, EXECUTIVE_MEMORY_DEFAULT_LIMITS, EXECUTIVE_MEMORY_MANDATORY_METADATA_FIELDS, EXECUTIVE_MEMORY_RESERVED_MEMORY_IDS, EXECUTIVE_MEMORY_RESERVED_PROVIDER_IDS, } from "./executiveMemoryConstants.ts";
...
export function isExecutiveMemoryCategory(value) {
    return EXECUTIVE_MEMORY_CATEGORY_KEYS.includes(value);
}
export function isNonEmptyString(value) {
    return typeof value === "string" && value.trim().length > 0;
}
export function isReservedExecutiveMemoryProviderId(providerId) {
    return EXECUTIVE_MEMORY_RESERVED_PROVIDER_IDS.includes(providerId);
}
export function isReservedExecutiveMemoryId(memoryId) {
    return EXECUTIVE_MEMORY_RESERVED_MEMORY_IDS.includes(memoryId);
}
export function validateExecutiveMemoryCategories(categories) {
    const issues = [];
...
export function validateExecutiveMemoryProviderRegistration(input) {
    const issues = [];
...
export function validateExecutiveMemoryMetadataShape(metadata) {
    const issues = [];
...
export function validateExecutiveMemoryShape(memory) {
    const issues = [];
...
export function validateExecutiveMemoryProviderShape(provider) {
    const issues = [];
...
export function hasDuplicateProviderIds(providerIds) {
