/**
 * MVP:1 — Release Hardening & Baseline Freeze certification.
 *
 * Re-certifies the hydrated /executive manager MVP after P1 hardening.
 * Does not start EXI:1 and does not infer Outcome, Learning, or CC:11.
 */
...
const OUT =
  "/Users/bahadoors/Documents/StateStudio/frontend/.certification/mvp-release-baseline";
  ...
  return page.evaluate(() => {
    ...
    return {
      ...
      context: {
        ...
        dataStatusKind: attr(shell, "data-data-status-kind"),
...
const report = {
  certification: "MVP:1 — Release Hardening & Baseline Freeze",
...
report.captures.firstOpen = await capture(main, "01-overview");
report.captures.dataStatus = await capture(main, "06-data-status");
...
report.checks.dataStatus =
  first.context.dataStatusKind === "local" &&
  /Data\s*·\s*Local/i.test(first.context.dataLabel ?? "") &&
  !/Evidence strong/i.test(first.advisor.evidence ?? "");
...
report.captures.attention = await capture(main, "extra-attention");
const overviewPrimary = main.locator(
  '[data-advisor-action-priority="primary"]',
);
await overviewPrimary.click();
await waitFocused(main, "obj-risk");
report.captures.investigation = await capture(main, "extra-investigation");
...
report.captures.problem = await capture(main, "02-problem");
...
report.captures.evidence = await capture(main, "extra-evidence");
...
report.captures.scenario = await capture(main, "03-scenario");
...
report.captures.decision = await capture(main, "04-decision");
...
report.captures.execution = await capture(main, "05-execution");
...
await waitCollection(main, "problem");
report.captures.backNavigation = await capture(main, "extra-back-navigation");
...
report.captures.returnOverview = await capture(main, "09-overview-return");
...
report.captures.narrow = await capture(main, "extra-narrow-desktop");
...
await main.waitForTimeout(250);
report.captures.consoleClean = await capture(main, "10-console-clean");
