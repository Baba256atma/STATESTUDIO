/**
 * MRA:2 live /executive journeys A–H. Does not rewrite MRA:1 live-audit.json.
 */
...
const out = join(process.cwd(), "artifacts/mra/MRA-2");
...
try {
  ...
  const live = {
    identity: "NPA-T MRA:2/LiveExecutiveJourneys",
    url,
    errors,
    journeys: {},
  };
  ...
  for (const utterance of [
    ...
    live.journeys.A_stage.push(await turn(utterance));
  }

  await openExecutivePage(page, url);
  live.journeys.B_data = [];
  for (const utterance of [
    ...
    live.journeys.B_data.push(await turn(utterance));
  }

  await openExecutivePage(page, url);
  live.journeys.C_problem = [];
  for (const utterance of [
    "show me problems",
    ...
    live.journeys.C_problem.push(await turn(utterance));
  }

  await openExecutivePage(page, url);
  live.journeys.D_scenario = [];
  for (const utterance of [
    "show me scenarios",
    ...
    live.journeys.D_scenario.push(await turn(utterance));
  }

  await openExecutivePage(page, url);
  live.journeys.E_decision = [];
  for (const utterance of [
    "show me scenarios",
    "I prefer Demand Surge.",
    ...
    live.journeys.E_decision.push(await turn(utterance));
  }

  await openExecutivePage(page, url);
  live.journeys.F_execution = [];
  for (const utterance of [
    "Approve Demand Surge",
    "are we ready to execute?",
    "start it",
  ]) {
    live.journeys.F_execution.push(await turn(utterance));
  }

  await openExecutivePage(page, url);
  live.journeys.G_mutation = [];
  for (const utterance of [
    ...
    live.journeys.G_mutation.push(await turn(utterance));
  }

  await openExecutivePage(page, url);
  live.journeys.H_context = [];
  for (const utterance of [
    "what is Capacity Gap?",
    "What CSV do you have?",
    ...
    live.journeys.H_context.push(await turn(utterance));
  }
