/**
 * MRA:3 independent recertification (RECERT-2). Validation only. Does not patch product code.
 */
...
const outDir = join(dirname(fileURLToPath(import.meta.url)), "../artifacts/mra/MRA-3-FINAL-RECERT-2");
...
function snapshot(utterance: string, turn: Turn, extra?: Record<string, unknown>) {
  ...
  return {
    ...
    compositionResolvedId: turn.trace.compositionResolvedSubjectId ?? null,
    compositionResolved: turn.trace.compositionResolvedSubjectKind ?? null,
    compositionSelectedId: turn.trace.compositionSelectedSubjectId ?? null,
...
function runTurn(utterance: string, previous?: Turn, runtimeState = previous?.nextRuntimeState): Turn {
  return executeNexoraConversationalExperience({
    ...
    messageIdSeed: `mra-3-recert-2-${utterance.slice(0, 56)}`,
  });
}
...
const jDeictic = runJourney("J-deictic", "Demand Surge deictic family and explicit change", [
  "Demand Surge",
  "explain it",
  "tell me more about it",
  "investigate it",
...
function runHistoricalS1() {
  ...
  previous = executeNexoraConversationalExperience({
    ...
    lastAppliedCommandId: previous.commandResult?.command?.commandId ?? null,
    messageIdSeed: "mra-3-recert-2-look-at-capacity-gap",
  });
  ...
  previous = runTurn("tell me more about it", previous);
  turns.push(snapshot("tell me more about it", previous));
  ...
  return {
    id: "S1-HIST",
    title: "Historical S1: Stage click then Capacity Gap explain/more, then Demand Surge explain/more",
    turns,
    last: previous,
  };
}
...
const journeys = [
  j1, j2, j3, j4, j5, j6, jDeictic, j7, j8, j9, j10,
  historicalS1,
  longSession,
...
writeFileSync(
  ...
  JSON.stringify(
    {
      identity: "NPA-T MRA:3-RECERT-2/RuntimeManagerSimulation",
