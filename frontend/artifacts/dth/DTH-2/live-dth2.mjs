/**
 * DTH:2 live /executive certification.
 * Live data does not invent Cost/Time Iconic values. Certifies safe-empty Iconic projection.
 */
...
async function snapshot(page) {
  return page.evaluate(() => {
    ...
    const queue = [...document.querySelectorAll("[data-queue-count], [data-testid^='nexora-queue']")].map((el) =>
      el.textContent?.trim() ?? "",
    );
    const advisor = document.querySelector('[data-testid="nexora-advisor-view"], [data-testid="nexora-conversational-input-field"]');
    const breadcrumb = [...document.querySelectorAll('[data-testid^="nexora-stage-breadcrumb"], [data-testid="nexora-stage-interaction-breadcrumb"]')]
...
const architectureLeak = turns.some((turn) =>
  /DTH:2|DTH:1|DIR:1|NEX-MVP|NexoGraph|Scene Intent/i.test(turn.reply),
);
const dthConsole = consoleErrors.filter((text) => /DTH:2|DTH:1|decision theatre|decisionTheatre|iconic/i.test(text));
const hydration = consoleErrors.filter((text) => /hydrat/i.test(text));
const report = {
  identity: "DTH:2/LiveExecutiveCertification",
  url,
  runtimeCommand: "npx next start -p 3012",
  liveIconicPolicy: "safe-empty-no-fabricated-values",
  pageErrors,
